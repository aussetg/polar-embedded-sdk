...
#define ASSERT_EQ_INT(expected, actual)                                        \
  do {                                                                         \
    ...
    int _a = (actual);                                                         \
    if (_e != _a)                                                              \
      fail_int(__LINE__, #actual, _e, _a);                                     \
  } while (0)
...
static void fail_int(int line, const char *expr, int expected, int actual) {
  fprintf(
      stderr,
      "test_btstack_security:%d assertion failed: %s expected=%d actual=%d\n",
...
void sm_request_pairing(uint16_t conn_handle) {
  ...
  g_pairing_calls += 1u;
  if (g_promote_on_pair_call != 0u &&
      g_pairing_calls >= g_promote_on_pair_call) {
...
static void test_invalid_handle(void) {
  ...
  ASSERT_EQ_INT(
      0, polar_sdk_btstack_security_encryption_key_size(0xffffu, 0xffffu));
  ASSERT_TRUE(!polar_sdk_btstack_security_ready(0xffffu, 0xffffu));
  polar_sdk_btstack_security_request_pairing(0xffffu, 0xffffu);
  ASSERT_EQ_INT(POLAR_SDK_SECURITY_RESULT_NOT_CONNECTED,
                polar_sdk_btstack_security_ensure(
                    &g_active_handle, 0xffffu, &policy, fake_sleep_ms, NULL));
...
static void test_already_secure_link(void) {
  ...
  ASSERT_EQ_INT(16, polar_sdk_btstack_security_encryption_key_size(
                        g_active_handle, 0xffffu));
  ASSERT_TRUE(polar_sdk_btstack_security_ready(g_active_handle, 0xffffu));
  ASSERT_EQ_INT(POLAR_SDK_SECURITY_RESULT_OK,
                polar_sdk_btstack_security_ensure(
                    &g_active_handle, 0xffffu, &policy, fake_sleep_ms, NULL));
  ASSERT_EQ_INT(0, (int)g_auth_policy_calls);
  ASSERT_EQ_INT(0, (int)g_pairing_calls);
...
static void test_timeout_without_security(void) {
  ...
  ASSERT_EQ_INT(POLAR_SDK_SECURITY_RESULT_TIMEOUT,
                polar_sdk_btstack_security_ensure(
                    &g_active_handle, 0xffffu, &policy, fake_sleep_ms, NULL));
  ASSERT_EQ_INT(1, (int)g_auth_policy_calls);
