...
static void fail_int(int line, const char *expr, int expected, int actual) {
  fprintf(stderr,
          "test_pmd_start:%d assertion failed: %s expected=%d actual=%d\n",
...
static int fake_start_ecg_and_wait_response(void *ctx, const uint8_t *start_cmd,
...
static void test_already_secure_success(void) {
  ...
  polar_sdk_pmd_start_result_t r = polar_sdk_pmd_start_ecg_with_policy(
      &policy, &ops, &response_status, &last_ccc_att);
...
static void test_initial_security_recovery_success(void) {
  ...
  polar_sdk_pmd_start_result_t r =
      polar_sdk_pmd_start_ecg_with_policy(&policy, &ops, NULL, NULL);
...
static void test_security_att_recovery_success(void) {
  ...
  int last_ccc_att = -1;
  polar_sdk_pmd_start_result_t r =
      polar_sdk_pmd_start_ecg_with_policy(&policy, &ops, NULL, &last_ccc_att);
