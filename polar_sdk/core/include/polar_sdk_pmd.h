...
size_t polar_sdk_pmd_build_ecg_start_command(
    const polar_sdk_pmd_ecg_start_config_t *cfg, uint8_t *out,
    size_t out_capacity);

size_t polar_sdk_pmd_build_acc_start_command(
    const polar_sdk_pmd_acc_start_config_t *cfg, uint8_t *out,
    size_t out_capacity);

bool polar_sdk_pmd_parse_cp_response(const uint8_t *value, size_t value_len,
...
typedef struct {
  ...
  // BTstack-backed callers should typically use
  ...
  // return 0 on transport success and set *out_status to PMD CP response
  // status.
  int (*start_ecg_and_wait_response)(void *ctx, const uint8_t *start_cmd,
                                     size_t start_cmd_len, uint8_t *out_status);
