...
static bool polar_sdk_transport_ops_ready(const polar_sdk_transport_connect_ops_t *ops) {
    return ops != 0 &&
        ops->now_ms != 0 &&
        ops->sleep_ms != 0 &&
        ops->start_attempt != 0 &&
...
polar_sdk_transport_connect_result_t polar_sdk_transport_connect_blocking(
    ...
    if (policy == 0 || !polar_sdk_transport_ops_ready(ops) || policy->timeout_ms == 0) {
    ...
    while (polar_sdk_connect_has_time_left(policy, &connect_state, ops->now_ms(ops->ctx))) {
        if (out_stats != 0) {
        ...
        uint32_t budget_ms = polar_sdk_connect_attempt_budget_ms(policy, &connect_state, ops->now_ms(ops->ctx));
        if (budget_ms == 0) {
            break;
        }

        polar_sdk_transport_attempt_result_t attempt_result = ops->wait_attempt(ops->ctx, budget_ms);
        ...
        uint32_t backoff_ms = polar_sdk_connect_next_backoff_ms(policy, &connect_state, ops->now_ms(ops->ctx));
        if (backoff_ms == 0) {
            break;
        }
