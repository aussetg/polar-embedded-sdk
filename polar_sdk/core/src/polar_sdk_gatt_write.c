...
static bool
polar_sdk_gatt_write_ops_ready(const polar_sdk_gatt_write_ops_t *ops) {
  return ops != 0 && ops->is_connected != 0 && ops->write_value != 0 &&
         ops->wait_complete != 0;
}
...
polar_sdk_gatt_write_result_t polar_sdk_gatt_write_with_wait(
  ...
  if (!polar_sdk_gatt_write_ops_ready(ops) || data == 0 || len == 0) {
    return POLAR_SDK_GATT_WRITE_FAILED;
  }
  if (out_att_status) {
  ...
  if (!ops->is_connected(ops->ctx)) {
    return POLAR_SDK_GATT_WRITE_NOT_CONNECTED;
  }
  ...
  if (!ops->wait_complete(ops->ctx, timeout_ms, &att_status)) {
    return POLAR_SDK_GATT_WRITE_TIMEOUT;
  }
  if (out_att_status) {
    *out_att_status = att_status;
  }
  if (att_status != att_success) {
    return POLAR_SDK_GATT_WRITE_ATT_REJECTED;
  }

  return POLAR_SDK_GATT_WRITE_OK;
}
