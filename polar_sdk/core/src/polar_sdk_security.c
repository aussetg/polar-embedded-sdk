...
static bool polar_sdk_security_ops_ready(const polar_sdk_security_ops_t *ops) {
  return ops != 0 && ops->is_connected != 0 && ops->is_secure != 0 &&
         ops->request_pairing != 0 && ops->sleep_ms != 0;
}

polar_sdk_security_result_t
polar_sdk_security_request_with_retry(const polar_sdk_security_policy_t *policy,
