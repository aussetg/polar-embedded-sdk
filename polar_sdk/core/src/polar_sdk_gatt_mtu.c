...
static bool polar_sdk_gatt_mtu_ops_ready(const polar_sdk_gatt_mtu_ops_t *ops) {
    return ops != 0 &&
        ops->is_connected != 0 &&
        ops->read_mtu != 0 &&
        ops->request_exchange != 0 &&
        ops->wait_exchange_complete != 0 &&
        ops->current_mtu != 0;
}

polar_sdk_gatt_mtu_result_t polar_sdk_gatt_mtu_ensure_minimum(
    const polar_sdk_gatt_mtu_ops_t *ops,
    ...
    return mtu >= minimum_mtu
        ? POLAR_SDK_GATT_MTU_RESULT_OK
