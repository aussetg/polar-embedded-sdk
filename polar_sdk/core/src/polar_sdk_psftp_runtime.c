...
static bool
polar_sdk_psftp_prepare_ops_ready(const polar_sdk_psftp_prepare_ops_t *ops) {
  return ops != 0 && ops->is_connected_ready != 0 &&
         ops->has_required_characteristics != 0 && ops->security_ready != 0 &&
         ops->ensure_security != 0 && ops->mtu_notify_enabled != 0 &&
         ops->enable_mtu_notify != 0 && ops->d2h_notify_supported != 0 &&
         ops->d2h_notify_enabled != 0 && ops->enable_d2h_notify != 0;
}
...
bool polar_sdk_psftp_prepare_failure_is_security_related(int prepare_status,
                                                         bool security_ready) {
  if (prepare_status > 0 &&
      polar_sdk_pmd_att_status_requires_security((uint8_t)prepare_status)) {
    return true;
  }
...
int polar_sdk_psftp_prepare_channels(
  ...
  if (!ops->security_ready(ops->ctx)) {
    int sec =
        polar_sdk_psftp_map_security_result(ops->ensure_security(ops->ctx));
  ...
  if (ops->d2h_notify_supported(ops->ctx) &&
      !ops->d2h_notify_enabled(ops->ctx)) {
...
static bool
polar_sdk_psftp_get_ops_ready(const polar_sdk_psftp_get_ops_t *ops) {
  return ops != 0 && ops->prepare_channels != 0 && ops->frame_capacity != 0 &&
         ops->write_frame != 0 && ops->begin_response != 0 &&
         ops->wait_response != 0 && ops->response_result != 0 &&
         ops->rx_state != 0;
}

static polar_sdk_psftp_trans_result_t polar_sdk_psftp_execute_rfc60_stream(
    const polar_sdk_psftp_get_ops_t *ops, const uint8_t *request_stream,
    size_t request_len, uint8_t *response, size_t response_capacity,
    uint32_t timeout_ms, size_t *out_response_len, uint16_t *out_error_code,
    int *out_prepare_status, int *out_write_status) {
  ...
  if (!polar_sdk_psftp_get_ops_ready(ops) || request_stream == 0 ||
      request_len == 0u) {
  ...
  if (prep == POLAR_SDK_PSFTP_OP_MISSING_CHAR ||
      prep == POLAR_SDK_PSFTP_OP_NO_NOTIFY_PROP) {
  ...
  if (rr == POLAR_SDK_PSFTP_RX_ERROR_FRAME) {
    ...
    if (ops->is_remote_success != 0 &&
        ops->is_remote_success(ops->ctx, error_code)) {
      if (out_response_len != 0) {
        *out_response_len = rx_state->length;
      }
      return POLAR_SDK_PSFTP_TRANS_OK;
...
polar_sdk_psftp_trans_result_t polar_sdk_psftp_execute_proto_operation(
    const polar_sdk_psftp_get_ops_t *ops, const uint8_t *proto_payload,
    size_t proto_payload_len, uint8_t *response, size_t response_capacity,
    uint32_t timeout_ms, size_t *out_response_len, uint16_t *out_error_code,
    int *out_prepare_status, int *out_write_status) {
  if (!polar_sdk_psftp_get_ops_ready(ops) || proto_payload == 0 ||
      proto_payload_len == 0) {
    return POLAR_SDK_PSFTP_TRANS_NOTIFY_FAILED;
  }
  ...
  return polar_sdk_psftp_execute_rfc60_stream(
      ops, request_stream, request_len, response, response_capacity, timeout_ms,
      out_response_len, out_error_code, out_prepare_status, out_write_status);
}

polar_sdk_psftp_trans_result_t polar_sdk_psftp_execute_query_operation(
    const polar_sdk_psftp_get_ops_t *ops, uint16_t query_id,
    const uint8_t *query_payload, size_t query_payload_len, uint8_t *response,
    size_t response_capacity, uint32_t timeout_ms, size_t *out_response_len,
    uint16_t *out_error_code, int *out_prepare_status, int *out_write_status) {
  ...
  size_t request_len = polar_sdk_psftp_build_rfc60_query(
      query_id, query_payload, query_payload_len, request_stream,
...
polar_sdk_psftp_trans_result_t polar_sdk_psftp_execute_get_operation(
    const polar_sdk_psftp_get_ops_t *ops, const char *path, size_t path_len,
    uint8_t *response, size_t response_capacity, uint32_t timeout_ms,
    size_t *out_response_len, uint16_t *out_error_code, int *out_prepare_status,
  ...
  return polar_sdk_psftp_execute_proto_operation(
      ...
      timeout_ms, out_response_len, out_error_code, out_prepare_status,
      out_write_status);
}
