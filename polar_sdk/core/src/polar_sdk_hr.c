...
static uint16_t polar_sdk_hr_rr_raw_to_ms(uint16_t rr_raw) {
...
bool polar_sdk_hr_parse_measurement(
    polar_sdk_hr_state_t *state,
    const uint8_t *value,
    size_t value_len,
    uint32_t now_ms) {
    ...
    if ((flags & 0x10u) != 0) {
        while (offset + 1 < value_len && rr_count < 4) {
            ...
            offset += 2;
            rr_ms[rr_count] = polar_sdk_hr_rr_raw_to_ms(rr_raw);
            rr_count += 1;
        }
    }
