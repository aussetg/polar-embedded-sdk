...
static void polar_sdk_imu_ring_push_bytes(polar_sdk_imu_ring_t *ring, const uint8_t *data, uint16_t len) {
...
void polar_sdk_imu_ring_init(
    polar_sdk_imu_ring_t *ring,
    uint8_t *storage,
    uint16_t capacity) {
    if (ring == 0) {
        return;
...
uint16_t polar_sdk_imu_ring_pop_bytes(
    polar_sdk_imu_ring_t *ring,
    uint8_t *out,
    uint16_t max_len) {
...
void polar_sdk_imu_parse_pmd_notification(
    polar_sdk_imu_ring_t *ring,
    ...
    for (uint16_t i = 0; i < sample_count; ++i) {
        ...
        polar_sdk_imu_ring_push_bytes(ring, payload + o, 6);
    }
