...
polar_sdk_disc_service_kind_t polar_sdk_btstack_classify_service(
    ...
    if (uuid16 == 0 && polar_sdk_btstack_uuid128_matches_any_order(uuid128, pmd_service_uuid_be)) {
...
polar_sdk_disc_char_kind_t polar_sdk_btstack_classify_char(
    ...
    if (stage == POLAR_SDK_DISC_STAGE_HR_CHARS) {
        return uuid16 == hr_measurement_uuid16 ? POLAR_SDK_DISC_CHAR_HR_MEAS : POLAR_SDK_DISC_CHAR_NONE;
    }

    if (stage == POLAR_SDK_DISC_STAGE_PMD_CHARS) {
        if (uuid16 == 0 && polar_sdk_btstack_uuid128_matches_any_order(uuid128, pmd_cp_uuid_be)) {
            return POLAR_SDK_DISC_CHAR_PMD_CP;
        }
        if (uuid16 == 0 && polar_sdk_btstack_uuid128_matches_any_order(uuid128, pmd_data_uuid_be)) {
    ...
    if (stage == POLAR_SDK_DISC_STAGE_PSFTP_CHARS) {
        if (uuid16 == 0 && polar_sdk_btstack_uuid128_matches_any_order(uuid128, psftp_mtu_uuid_be)) {
            return POLAR_SDK_DISC_CHAR_PSFTP_MTU;
        }
        if (uuid16 == 0 && polar_sdk_btstack_uuid128_matches_any_order(uuid128, psftp_d2h_uuid_be)) {
            return POLAR_SDK_DISC_CHAR_PSFTP_D2H;
        }
        if (uuid16 == 0 && polar_sdk_btstack_uuid128_matches_any_order(uuid128, psftp_h2d_uuid_be)) {
