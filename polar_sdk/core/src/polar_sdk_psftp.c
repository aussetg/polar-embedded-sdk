...
size_t polar_sdk_psftp_build_rfc60_request(const uint8_t *payload,
...
size_t polar_sdk_psftp_build_rfc60_query(uint16_t query_id,
...
void polar_sdk_psftp_tx_init(polar_sdk_psftp_tx_state_t *state,
...
size_t polar_sdk_psftp_tx_build_next_frame(polar_sdk_psftp_tx_state_t *state,
                                           size_t frame_capacity,
                                           uint8_t *out_frame,
                                           size_t out_frame_capacity,
                                           bool *out_is_last) {
  if (state == 0 || out_frame == 0 || frame_capacity == 0 ||
      out_frame_capacity == 0) {
    return 0;
  }
  ...
  if (frame_capacity > 1u && out_frame_capacity > 1u) {
    max_payload =
        polar_sdk_psftp_min_size(frame_capacity - 1u, out_frame_capacity - 1u);
  }
  ...
  out_frame[0] = (uint8_t)((more ? 0x06u : 0x02u) | (state->next_bit & 0x01u) |
                           ((state->sequence & 0x0fu) << 4));
  if (chunk_len > 0u) {
...
void polar_sdk_psftp_rx_reset(polar_sdk_psftp_rx_state_t *state,
...
polar_sdk_psftp_rx_feed_frame(polar_sdk_psftp_rx_state_t *state,
  ...
  return status == 0x01u ? POLAR_SDK_PSFTP_RX_COMPLETE
                         : POLAR_SDK_PSFTP_RX_MORE;
}

static bool polar_sdk_psftp_encode_operation(int command, const char *path,
                                             size_t path_len,
                                             uint8_t *out_payload,
  ...
  pb_ostream_t stream =
      pb_ostream_from_buffer(out_payload, out_payload_capacity);
...
bool polar_sdk_psftp_encode_get_operation(const char *path, size_t path_len,
                                          uint8_t *out_payload,
                                          size_t out_payload_capacity,
                                          size_t *out_payload_len) {
#if !POLAR_CFG_ENABLE_PSFTP
  ...
  return polar_sdk_psftp_encode_operation(
      protocol_PbPFtpOperation_Command_GET, path, path_len, out_payload,
      out_payload_capacity, out_payload_len);
#endif
}

bool polar_sdk_psftp_encode_remove_operation(const char *path, size_t path_len,
                                             uint8_t *out_payload,
                                             size_t out_payload_capacity,
                                             size_t *out_payload_len) {
#if !POLAR_CFG_ENABLE_PSFTP
...
#else
  return polar_sdk_psftp_encode_operation(
      protocol_PbPFtpOperation_Command_REMOVE, path, path_len, out_payload,
      out_payload_capacity, out_payload_len);
#endif
}
...
static bool polar_sdk_psftp_encode_string_cb(pb_ostream_t *stream,
  ...
  const polar_sdk_psftp_string_view_t *view =
      (const polar_sdk_psftp_string_view_t *)(*arg);
...
static bool polar_sdk_psftp_decode_string_cb(pb_istream_t *stream,
                                             const pb_field_t *field,
                                             void **arg) {
  (void)field;
  ...
  polar_sdk_psftp_string_decode_ctx_t *ctx =
      (polar_sdk_psftp_string_decode_ctx_t *)(*arg);
...
bool polar_sdk_psftp_encode_h10_start_recording_params(
    const char *recording_id, size_t recording_id_len,
    polar_sdk_h10_recording_sample_type_t sample_type,
    polar_sdk_h10_recording_interval_t interval, uint8_t *out_payload,
    size_t out_payload_capacity, size_t *out_payload_len) {
  if (out_payload_len != 0) {
    *out_payload_len = 0;
  }
  ...
  pb_ostream_t stream =
      pb_ostream_from_buffer(out_payload, out_payload_capacity);
  if (!pb_encode(&stream, protocol_PbPFtpRequestStartRecordingParams_fields,
                 &params)) {
    return false;
  }
...
bool polar_sdk_psftp_decode_h10_recording_status_result(
    const uint8_t *payload, size_t payload_len, bool *out_recording_on,
    char *out_recording_id, size_t out_recording_id_capacity,
  ...
  protocol_PbRequestRecordingStatusResult result =
      protocol_PbRequestRecordingStatusResult_init_zero;
  ...
  if (!pb_decode(&stream, protocol_PbRequestRecordingStatusResult_fields,
                 &result)) {
    return false;
  }
  if (id_ctx.overflow) {
...
static bool polar_sdk_psftp_decode_directory_entry_cb(pb_istream_t *stream,
                                                      const pb_field_t *field,
                                                      void **arg) {
  (void)field;
  ...
  polar_sdk_psftp_dir_decode_ctx_t *ctx =
      (polar_sdk_psftp_dir_decode_ctx_t *)(*arg);
  ...
  size_t name_len =
      polar_sdk_psftp_bounded_strlen(entry.name, sizeof(entry.name));
...
polar_sdk_psftp_decode_directory(const uint8_t *payload, size_t payload_len,
  ...
  if ((payload_len > 0u && payload == 0) ||
      (out_entries_capacity > 0u && out_entries == 0)) {
