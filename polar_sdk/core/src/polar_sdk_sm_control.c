...
bool polar_sdk_sm_control_apply(
    ...
    if (!polar_sdk_sm_event_matches_handle(event, active_conn_handle, invalid_conn_handle)) {
        return false;
    }

    switch (event->type) {
        case POLAR_SDK_SM_EVENT_JUST_WORKS_REQUEST:
            if (ops->on_just_works_request) {
                ops->on_just_works_request(ops->ctx, event->handle);
            }
            return true;

        case POLAR_SDK_SM_EVENT_NUMERIC_COMPARISON_REQUEST:
            if (ops->on_numeric_comparison_request) {
                ops->on_numeric_comparison_request(ops->ctx, event->handle);
            }
            return true;

        case POLAR_SDK_SM_EVENT_AUTHORIZATION_REQUEST:
            if (ops->on_authorization_request) {
                ops->on_authorization_request(ops->ctx, event->handle);
            }
            return true;

        case POLAR_SDK_SM_EVENT_PAIRING_COMPLETE:
