...
bool polar_sdk_pmd_response_status_ok(uint8_t status) {
  return status == POLAR_SDK_PMD_RESPONSE_SUCCESS ||
         status == POLAR_SDK_PMD_RESPONSE_ALREADY_IN_STATE;
}
...
size_t polar_sdk_pmd_build_ecg_start_command(
  ...
  out[0] = POLAR_SDK_PMD_OPCODE_START_MEASUREMENT;
  out[1] = POLAR_SDK_PMD_MEASUREMENT_ECG;
...
size_t polar_sdk_pmd_build_acc_start_command(
  ...
  out[0] = POLAR_SDK_PMD_OPCODE_START_MEASUREMENT;
  out[1] = POLAR_SDK_PMD_MEASUREMENT_ACC;
...
bool polar_sdk_pmd_parse_cp_response(const uint8_t *value, size_t value_len,
  ...
  if (value[0] != POLAR_SDK_PMD_CP_RESPONSE_CODE) {
    return false;
  }
