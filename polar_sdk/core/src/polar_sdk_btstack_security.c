...
bool polar_sdk_btstack_security_ready(uint16_t conn_handle,
                                      uint16_t invalid_conn_handle) {
  return polar_sdk_btstack_security_encryption_key_size(
             conn_handle, invalid_conn_handle) > 0u;
}
...
static bool polar_sdk_btstack_security_is_connected_cb(const void *ctx) {
  const polar_sdk_btstack_security_ctx_t *state =
      (const polar_sdk_btstack_security_ctx_t *)ctx;
  return state != 0 && polar_sdk_btstack_security_current_handle(state) !=
...
static void polar_sdk_btstack_security_sleep_ms_cb(const void *ctx,
                                                   uint32_t ms) {
  const polar_sdk_btstack_security_ctx_t *state =
      (const polar_sdk_btstack_security_ctx_t *)ctx;
