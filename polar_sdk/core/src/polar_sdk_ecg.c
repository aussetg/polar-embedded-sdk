...
static void polar_sdk_ecg_ring_push_bytes(polar_sdk_ecg_ring_t *ring,
                                          const uint8_t *data, uint16_t len) {
  if (ring == 0 || ring->storage == 0 || ring->capacity == 0 || data == 0 ||
      len == 0) {
    return;
  }
...
void polar_sdk_ecg_ring_init(polar_sdk_ecg_ring_t *ring, uint8_t *storage,
                             uint16_t capacity) {
  if (ring == 0) {
    return;
  }
...
uint16_t polar_sdk_ecg_ring_pop_bytes(polar_sdk_ecg_ring_t *ring, uint8_t *out,
                                      uint16_t max_len) {
...
void polar_sdk_ecg_parse_pmd_notification(polar_sdk_ecg_ring_t *ring,
  ...
  for (uint16_t i = 0; i < sample_count; ++i) {
    ...
    int32_t sample =
        (int32_t)((uint32_t)payload[o] | ((uint32_t)payload[o + 1] << 8) |
