...
bool polar_sdk_btstack_decode_mtu_event(uint8_t packet_type, uint8_t *packet,
...
bool polar_sdk_btstack_decode_query_complete_att_status(
    uint8_t packet_type, uint8_t *packet, uint8_t *out_att_status) {
...
bool polar_sdk_btstack_decode_service_query_result(uint8_t packet_type,
                                                   uint8_t *packet,
                                                   void *out_service) {
  ...
  gatt_event_service_query_result_get_service(
      packet, (gatt_client_service_t *)out_service);
  return true;
}

bool polar_sdk_btstack_decode_characteristic_query_result(
    uint8_t packet_type, uint8_t *packet, void *out_characteristic) {
  if (packet_type != HCI_EVENT_PACKET || packet == 0 ||
      out_characteristic == 0) {
    return false;
  }
  ...
  gatt_event_characteristic_query_result_get_characteristic(
      packet, (gatt_client_characteristic_t *)out_characteristic);
  return true;
}
