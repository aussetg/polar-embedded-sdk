...
bool polar_sdk_discovery_required_services_present(
    ...
    if (polar_sdk_need(required_services_mask, service_hr_bit) && !hr_service_found) {
        return false;
    }
    if (polar_sdk_need(required_services_mask, service_ecg_bit) && !pmd_service_found) {
        return false;
    }
    if (polar_sdk_need(required_services_mask, service_psftp_bit) && !psftp_service_found) {
        return false;
    }
    return true;
}
...
bool polar_sdk_discovery_required_handles_ready(
    ...
    uint8_t service_ecg_bit,
    uint8_t service_psftp_bit) {
    if (polar_sdk_need(required_services_mask, service_hr_bit) && hr_measurement_handle == 0) {
        return false;
    }
    if (polar_sdk_need(required_services_mask, service_ecg_bit) && (pmd_cp_handle == 0 || pmd_data_handle == 0)) {
        return false;
    }
...
polar_sdk_discovery_action_t polar_sdk_discovery_next_action(
    ...
    switch (stage) {
        ...
        case POLAR_SDK_DISC_STAGE_HR_CHARS:
            ...
            return have_required_handles ? POLAR_SDK_DISC_ACTION_READY : POLAR_SDK_DISC_ACTION_FAIL;
