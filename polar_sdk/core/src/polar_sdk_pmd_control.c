...
int polar_sdk_pmd_map_notify_result(
    polar_sdk_gatt_notify_runtime_result_t result, uint8_t att_status,
    int op_ok, int op_not_connected, int op_timeout, int op_transport) {
...
int polar_sdk_pmd_enable_notify_pair(const polar_sdk_pmd_notify_pair_ops_t *ops,
                                     int op_ok, int op_not_connected,
                                     int op_transport,
                                     int *out_last_att_status) {
  if (ops == 0 || ops->is_connected == 0 || ops->enable_cp_notify == 0 ||
...
int polar_sdk_pmd_start_command_and_wait(
    const polar_sdk_pmd_start_cmd_ops_t *ops, const uint8_t *cmd,
    size_t cmd_len, uint8_t opcode, uint8_t measurement_type,
    uint32_t timeout_ms, int op_ok, int op_not_connected, int op_timeout,
    int op_transport, uint8_t *out_response_status) {
  if (ops == 0 || ops->is_connected == 0 || ops->expect_response == 0 ||
      ops->write_command == 0 || ops->wait_response == 0 || cmd == 0 ||
      cmd_len == 0 || cmd_len > 0xffffu) {
    return op_transport;
  }
