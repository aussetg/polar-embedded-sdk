...
bool polar_sdk_transport_adapter_handle_link_event(
  ...
  case POLAR_SDK_LINK_EVENT_DISCONNECT:
    if (link->conn_handle == invalid_conn_handle ||
        event->handle == link->conn_handle) {
      polar_sdk_runtime_on_disconnect(link, invalid_conn_handle, event->status,
                                      event->reason, user_disconnect_requested,
                                      connect_intent);
    }
    return false;
