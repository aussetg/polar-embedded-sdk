...
bool polar_sdk_btstack_adv_runtime_on_report(
    ...
    const polar_sdk_btstack_adv_report_t *report, int hci_success_status,
    const polar_sdk_btstack_adv_runtime_ops_t *ops) {
  if (filter == 0 || report == 0 || ops == 0 || ops->is_scanning == 0 ||
      ops->stop_scan == 0 || ops->connect == 0) {
    return false;
  }
  ...
  if (!polar_sdk_btstack_adv_prepare_connect(
          link, filter, report->addr, report->adv_data, report->adv_len)) {
    return true;
  }
