...
bool polar_sdk_btstack_route_gatt_event(
  ...
  if (polar_sdk_btstack_decode_value_event(packet_type, packet, &out->value) &&
      state->connected && state->conn_handle != HCI_CON_HANDLE_INVALID &&
      out->value.handle == state->conn_handle) {
    if (out->value.value_handle == state->hr_value_handle &&
        state->hr_enabled) {
      out->kind = POLAR_SDK_GATT_ROUTE_HR_VALUE;
      return true;
    }
    if (out->value.value_handle == state->pmd_cp_value_handle &&
        state->pmd_cp_listening) {
      out->kind = POLAR_SDK_GATT_ROUTE_PMD_CP_VALUE;
      return true;
    }
    if (out->value.value_handle == state->pmd_data_value_handle &&
        state->pmd_data_listening) {
      out->kind = POLAR_SDK_GATT_ROUTE_PMD_DATA_VALUE;
      return true;
    }
    if (out->value.value_handle == state->psftp_mtu_value_handle &&
        state->psftp_mtu_listening) {
      out->kind = POLAR_SDK_GATT_ROUTE_PSFTP_MTU_VALUE;
      return true;
    }
    if (out->value.value_handle == state->psftp_d2h_value_handle &&
        state->psftp_d2h_listening) {
  ...
  if (polar_sdk_btstack_decode_query_complete_att_status(
          packet_type, packet, &out->query_complete_att_status)) {
