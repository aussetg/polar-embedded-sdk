...
static bool polar_sdk_pmd_ops_ready(const polar_sdk_pmd_start_ops_t *ops) {
  return ops != 0 && ops->is_connected != 0 && ops->security_ready != 0 &&
         ops->ensure_security != 0 && ops->enable_notifications != 0 &&
         ops->ensure_minimum_mtu != 0 && ops->start_ecg_and_wait_response != 0;
}

static polar_sdk_pmd_start_result_t
polar_sdk_pmd_map_security_result(polar_sdk_security_result_t security_result) {
...
static polar_sdk_pmd_start_result_t
polar_sdk_pmd_require_security(const polar_sdk_pmd_start_ops_t *ops) {
  if (ops == 0) {
  ...
  polar_sdk_pmd_start_result_t mapped =
      polar_sdk_pmd_map_security_result(ops->ensure_security(ops->ctx));
...
static polar_sdk_pmd_start_result_t
polar_sdk_pmd_recover_security(const polar_sdk_pmd_start_ops_t *ops) {
  if (ops == 0) {
    return POLAR_SDK_PMD_START_RESULT_TRANSPORT_ERROR;
  }
  if (!ops->is_connected(ops->ctx)) {
...
static polar_sdk_pmd_start_result_t
polar_sdk_pmd_start_with_command(const polar_sdk_pmd_start_policy_t *policy,
                                 const polar_sdk_pmd_start_ops_t *ops,
                                 const uint8_t *start_cmd, size_t start_cmd_len,
                                 uint8_t *out_pmd_response_status,
                                 int *out_last_ccc_att_status) {
  if (policy == 0 || !polar_sdk_pmd_ops_ready(ops) || start_cmd == 0 ||
      start_cmd_len == 0) {
    return POLAR_SDK_PMD_START_RESULT_TRANSPORT_ERROR;
  }
  ...
  for (size_t i = 0; i < ccc_attempts; ++i) {
    ...
    polar_sdk_pmd_start_result_t security_result =
        polar_sdk_pmd_require_security(ops);
  ...
  if (ops->ensure_minimum_mtu(ops->ctx, policy->minimum_mtu) !=
      POLAR_SDK_PMD_OP_OK) {
    if (!ops->is_connected(ops->ctx)) {
      return POLAR_SDK_PMD_START_RESULT_NOT_CONNECTED;
    }
  ...
  int start_status = ops->start_ecg_and_wait_response(
      ops->ctx, start_cmd, start_cmd_len, &response_status);
...
polar_sdk_pmd_start_ecg_with_policy(const polar_sdk_pmd_start_policy_t *policy,
                                    const polar_sdk_pmd_start_ops_t *ops,
                                    uint8_t *out_pmd_response_status,
                                    int *out_last_ccc_att_status) {
  ...
  size_t start_cmd_len =
      polar_sdk_pmd_build_ecg_start_command(&cfg, start_cmd, sizeof(start_cmd));
  if (start_cmd_len == 0) {
    return POLAR_SDK_PMD_START_RESULT_TRANSPORT_ERROR;
  }

  return polar_sdk_pmd_start_with_command(policy, ops, start_cmd, start_cmd_len,
                                          out_pmd_response_status,
                                          out_last_ccc_att_status);
}

polar_sdk_pmd_start_result_t
polar_sdk_pmd_start_acc_with_policy(const polar_sdk_pmd_start_policy_t *policy,
                                    const polar_sdk_pmd_start_ops_t *ops,
                                    uint8_t *out_pmd_response_status,
                                    int *out_last_ccc_att_status) {
  ...
  size_t start_cmd_len =
      polar_sdk_pmd_build_acc_start_command(&cfg, start_cmd, sizeof(start_cmd));
  if (start_cmd_len == 0) {
    return POLAR_SDK_PMD_START_RESULT_TRANSPORT_ERROR;
  }
