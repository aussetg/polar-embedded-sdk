...
static bool polar_sdk_runtime_context_has_link_ops(const polar_sdk_runtime_context_link_ops_t *ops) {
    return ops != 0;
}
...
bool polar_sdk_runtime_context_handle_link_event(
    ...
    if (event->type == POLAR_SDK_LINK_EVENT_CONN_COMPLETE) {
        if (connected_now && polar_sdk_runtime_context_has_link_ops(ops) && ops->on_connected_ready) {
    ...
    if (event->type == POLAR_SDK_LINK_EVENT_CONN_UPDATE_COMPLETE) {
        if (polar_sdk_runtime_context_has_link_ops(ops) && ops->on_conn_update_complete) {
...
static bool polar_sdk_runtime_context_has_cleanup_ops(const polar_sdk_runtime_context_cleanup_ops_t *ops) {
...
void polar_sdk_runtime_context_cleanup_attempt(
    ...
    polar_sdk_runtime_state_t recovering_state,
    ...
    if (link->state == POLAR_SDK_RUNTIME_STATE_CONNECTING && ops->cancel_connect) {
