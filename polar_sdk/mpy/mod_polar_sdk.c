...
#define POLAR_SDK_SERVICE_ALL                                                  \
  (POLAR_SDK_SERVICE_HR | POLAR_SDK_SERVICE_ECG | POLAR_SDK_SERVICE_PSFTP)
#define POLAR_SDK_CAP_STREAM_PMD_MASK                                          \
  (POLAR_CAP_STREAM_ECG | POLAR_CAP_STREAM_ACC | POLAR_CAP_STREAM_PPG |        \
   POLAR_CAP_STREAM_PPI | POLAR_CAP_STREAM_GYRO | POLAR_CAP_STREAM_MAG)
...
#define POLAR_SDK_PMD_OP_REQUEST_MEASUREMENT_START                             \
  (POLAR_SDK_PMD_OPCODE_START_MEASUREMENT)
#define POLAR_SDK_PMD_OP_STOP_MEASUREMENT                                      \
  (POLAR_SDK_PMD_OPCODE_STOP_MEASUREMENT)
...
static void polar_fill_btstack_host_key(sm_key_t out_key, uint8_t salt_a,
                                        uint8_t salt_b) {
  ...
  for (size_t i = 0; i < sizeof(sm_key_t); ++i) {
    ...
    uint8_t mix = (uint8_t)(src ^ (uint8_t)(salt_a + (uint8_t)(i * 29u)) ^
                            (uint8_t)(salt_b * (uint8_t)(i + 1u)));
    if (mix == 0) {
...
static uint8_t
polar_capability_mask_to_service_mask(polar_capability_mask_t mask) {
  ...
  if (mask &
      (POLAR_CAP_RECORDING | POLAR_CAP_PSFTP_READ | POLAR_CAP_PSFTP_DELETE)) {
...
static uint8_t polar_build_default_services_mask(void) {
  return polar_capability_mask_to_service_mask(
      polar_build_available_capabilities_mask());
}

static bool polar_capability_name_to_bit(const char *name, size_t len,
                                         polar_capability_mask_t *out_bit) {
...
static bool polar_required_capabilities_parse(mp_obj_t caps_obj,
  ...
  if (caps_obj == mp_const_none) {
    ...
    if (!mp_obj_is_type(caps_obj, &mp_type_tuple) &&
        !mp_obj_is_type(caps_obj, &mp_type_list)) {
      mp_raise_TypeError(
          MP_ERROR_TEXT("required_capabilities must be tuple/list or None"));
    }
    ...
    for (size_t i = 0; i < n_items; i++) {
      if (!mp_obj_is_str(items[i])) {
        mp_raise_TypeError(
            MP_ERROR_TEXT("required_capabilities entries must be str"));
      }
      size_t len = 0;
  ...
  if ((caps & ~polar_build_available_capabilities_mask()) != 0) {
    mp_raise_ValueError(
        MP_ERROR_TEXT("required capabilities unavailable in this build"));
  }
...
static mp_obj_t
polar_capabilities_tuple_from_mask(polar_capability_mask_t mask) {
  mp_obj_t items[10];
  size_t n = 0;
  if (mask & POLAR_CAP_STREAM_HR) {
...
static polar_stream_format_t
polar_default_stream_format_for_kind(polar_stream_kind_key_t kind) {
  (void)kind;
  return POLAR_STREAM_FORMAT_DECODED;
}

static polar_stream_format_t
polar_parse_stream_format_or_default(mp_obj_t format_obj,
...
static mp_obj_t polar_build_decoded_chunk_v1(
    uint8_t kind_code, uint8_t unit_code, uint8_t time_base_code, uint8_t flags,
    int64_t t0_ns, int32_t dt_ns, uint32_t sample_size_bytes,
  ...
  tmp[6] = 0;
  tmp[7] = 0;
  polar_store_u32_le(&tmp[8],
                     sample_size_bytes == 0
                         ? 0u
                         : (uint32_t)(payload_len / sample_size_bytes));
...
static mp_obj_t
polar_build_decoded_hr_chunk(const polar_sdk_hr_state_t *state) {
  ...
  return polar_build_decoded_chunk_v1(0, 1, 0, 0, 0, 0, 12, payload,
                                      sizeof(payload));
}
...
static MP_DEFINE_CONST_OBJ_TYPE(polar_type_Error, MP_QSTR_Error,
                                MP_TYPE_FLAG_NONE, make_new,
                                mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
                                &mp_type_Exception);

static MP_DEFINE_CONST_OBJ_TYPE(polar_type_TimeoutError, MP_QSTR_TimeoutError,
                                MP_TYPE_FLAG_NONE, make_new,
                                mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
...
static MP_DEFINE_CONST_OBJ_TYPE(polar_type_NotConnectedError,
                                MP_QSTR_NotConnectedError, MP_TYPE_FLAG_NONE,
                                make_new, mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
                                &polar_type_Error);

static MP_DEFINE_CONST_OBJ_TYPE(polar_type_ProtocolError, MP_QSTR_ProtocolError,
                                MP_TYPE_FLAG_NONE, make_new,
                                mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
...
static MP_DEFINE_CONST_OBJ_TYPE(polar_type_BufferOverflowError,
                                MP_QSTR_BufferOverflowError, MP_TYPE_FLAG_NONE,
                                make_new, mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
                                &polar_type_Error);

static MP_DEFINE_CONST_OBJ_TYPE(polar_type_SecurityError, MP_QSTR_SecurityError,
                                MP_TYPE_FLAG_NONE, make_new,
                                mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
...
static MP_DEFINE_CONST_OBJ_TYPE(polar_type_UnsupportedError,
                                MP_QSTR_UnsupportedError, MP_TYPE_FLAG_NONE,
                                make_new, mp_obj_exception_make_new, print,
                                mp_obj_exception_print, attr,
                                mp_obj_exception_attr, parent,
                                &polar_type_Error);

static NORETURN void polar_raise_exc(const mp_obj_type_t *type,
...
static polar_sdk_psftp_trans_result_t polar_psftp_execute_get(
    polar_h10_obj_t *self, const char *path, size_t path_len, uint8_t *response,
    size_t response_capacity, uint32_t timeout_ms, size_t *out_response_len);
static polar_sdk_psftp_trans_result_t
polar_psftp_execute_remove(polar_h10_obj_t *self, const char *path,
                           size_t path_len, uint32_t timeout_ms);
static NORETURN void
polar_raise_for_psftp_result(polar_h10_obj_t *self,
                             polar_sdk_psftp_trans_result_t result);
...
static void polar_require_ready(polar_h10_obj_t *self) {
  if (self->runtime_link.state != POLAR_SDK_STATE_READY ||
    ...
    polar_raise_exc(&polar_type_NotConnectedError,
...
static void polar_require_no_active_download_handle(polar_h10_obj_t *self) {
  ...
  if (self->download_handle_active) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("download handle already active"));
  }
#else
  (void)self;
#endif
...
static inline uint32_t polar_now_ms(void) { return mp_hal_ticks_ms(); }
...
static void polar_download_blob_compact_for_frame(polar_h10_obj_t *self,
                                                  size_t incoming_frame_len) {
  if (self == NULL || self->download_blob == NULL ||
      self->download_blob_alloc == 0) {
    return;
  }
  if (self->download_blob_off == 0 ||
      self->download_blob_off > self->psftp_rx_state.length) {
    return;
  }
  ...
  if (remaining > 0) {
    memmove(self->download_blob, self->download_blob + self->download_blob_off,
            remaining);
  }
  self->download_blob_off = 0;
...
static void polar_hci_packet_handler_and_discovery(uint8_t packet_type,
                                                   uint16_t channel,
                                                   uint8_t *packet,
                                                   uint16_t size);
static void polar_sm_packet_handler(uint8_t packet_type, uint16_t channel,
...
static bool polar_psftp_reconnect_after_security_failure(polar_h10_obj_t *self,
                                                         uint32_t timeout_ms);
static void polar_wait_conn_update_settle(polar_h10_obj_t *self,
                                          uint32_t timeout_ms);

static void polar_hr_parse_notification(polar_h10_obj_t *self,
                                        const uint8_t *value,
                                        uint16_t value_len) {
  (void)polar_sdk_hr_parse_measurement(&self->hr_state, value, value_len,
                                       polar_now_ms());
}
...
static void polar_btstack_pump_sleep_ms(uint32_t ms) { mp_event_wait_ms(ms); }

static bool polar_wait_flag_is_connected(void *ctx) {
  polar_wait_flag_ctx_t *w = (polar_wait_flag_ctx_t *)ctx;
  return w->self->runtime_link.connected &&
         w->self->runtime_link.conn_handle != HCI_CON_HANDLE_INVALID;
}

static bool polar_wait_for_flag_or_disconnect(polar_h10_obj_t *self,
                                              bool *done_flag,
                                              uint32_t timeout_ms) {
...
static bool polar_hr_wait_cfg_complete(polar_h10_obj_t *self,
                                       uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->hr_cfg_done,
                                           timeout_ms);
}
...
static void polar_hr_notify_start_listener(void *ctx) {
  ...
  gatt_client_listen_for_characteristic_value_updates(
      &h->self->hr_notification, polar_hci_packet_handler_and_discovery,
      h->self->runtime_link.conn_handle, &h->self->hr_measurement_char);
...
static int polar_hr_notify_write_ccc(void *ctx, uint16_t ccc_cfg) {
  ...
  uint8_t err = gatt_client_write_client_characteristic_configuration(
      polar_hci_packet_handler_and_discovery, h->self->runtime_link.conn_handle,
      &h->self->hr_measurement_char, ccc_cfg);
...
static bool polar_hr_notify_wait_complete(void *ctx, uint32_t timeout_ms,
...
static void polar_raise_for_notify_runtime_result(
    polar_sdk_gatt_notify_runtime_result_t r, mp_rom_error_text_t missing_msg,
    mp_rom_error_text_t no_prop_msg, mp_rom_error_text_t write_failed_msg,
    mp_rom_error_text_t timeout_msg, mp_rom_error_text_t rejected_msg) {
...
static void polar_hr_set_notify(polar_h10_obj_t *self, bool enable) {
  ...
  polar_sdk_gatt_notify_runtime_args_t args = {
      .ops = &ops,
      .has_value_handle =
          self->hr_char_found && self->hr_measurement_char.value_handle != 0,
      .enable = enable,
  ...
  polar_raise_for_notify_runtime_result(
      ...
      MP_ERROR_TEXT("failed to write HR CCC"), MP_ERROR_TEXT("HR CCC timeout"),
...
static void polar_hr_rearm_best_effort(polar_h10_obj_t *self) {
  ...
  if (self->hr_last_resubscribe_ms != 0 &&
      (uint32_t)(now - self->hr_last_resubscribe_ms) < 1500) {
    return;
  }
...
static uint16_t polar_ecg_ring_pop_bytes(polar_h10_obj_t *self, uint8_t *out,
                                         uint16_t max_len) {
...
static uint16_t polar_imu_ring_pop_bytes(polar_h10_obj_t *self, uint8_t *out,
                                         uint16_t max_len) {
  return polar_sdk_imu_ring_pop_bytes(&self->imu_ring, out, max_len);
}

static bool polar_pmd_wait_cfg_complete(polar_h10_obj_t *self,
                                        uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->pmd_cfg_done,
                                           timeout_ms);
}

static bool polar_pmd_wait_write_complete(polar_h10_obj_t *self,
                                          uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->pmd_write_done,
                                           timeout_ms);
}

static bool polar_pmd_wait_response(polar_h10_obj_t *self,
                                    uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->pmd_cp_response_done,
                                           timeout_ms);
}

static int polar_pmd_write_command_result(polar_h10_obj_t *self,
                                          const uint8_t *cmd, uint16_t cmd_len);
static int polar_pmd_set_notify_for_char_result(
    polar_h10_obj_t *self, gatt_client_characteristic_t *chr,
    gatt_client_notification_t *notification, bool *listening, bool enable);
static void polar_raise_for_pmd_notify_status(int status, bool missing_char);
static void polar_pmd_expect_response(polar_h10_obj_t *self, uint8_t opcode,
                                      uint8_t type);
...
static polar_recording_kind_key_t
polar_parse_recording_kind_key(mp_obj_t kind_obj) {
...
static bool
polar_recording_kind_is_supported_build(polar_recording_kind_key_t kind) {
  switch (kind) {
...
static bool polar_h10_recording_is_supported(polar_h10_obj_t *self) {
  return self != NULL && POLAR_CFG_ENABLE_PSFTP && POLAR_CFG_ENABLE_HR &&
         self->psftp_mtu_char_found && self->psftp_h2d_char_found &&
         self->psftp_mtu_handle != 0 && self->psftp_h2d_handle != 0;
}

static polar_sdk_h10_recording_sample_type_t
polar_parse_h10_recording_sample_type(mp_obj_t obj) {
...
static void polar_h10_recording_store_id(polar_h10_obj_t *self,
...
static bool polar_psftp_dir_contains_entry_or_raise(polar_h10_obj_t *self,
                                                    const char *path,
                                                    const char *entry_name,
                                                    size_t entry_name_len) {
  if (self == NULL || path == NULL || entry_name == NULL ||
      entry_name_len == 0u) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("invalid recording id lookup"));
  }
  ...
  if (normalized_len == 0u || normalized_len > POLAR_SDK_PSFTP_MAX_PATH_BYTES) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("invalid recording directory lookup path"));
  }
  ...
  polar_sdk_psftp_trans_result_t r = polar_psftp_execute_get(
      self, normalized_path, normalized_len, response,
      POLAR_SDK_PSFTP_MAX_DIR_RESPONSE_BYTES,
      POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS, &response_len);
  ...
  polar_sdk_psftp_dir_entry_t *entries =
      m_new(polar_sdk_psftp_dir_entry_t, POLAR_SDK_PSFTP_MAX_DIR_ENTRIES);
  size_t entry_count = 0;
  polar_sdk_psftp_dir_decode_result_t decode_result =
      polar_sdk_psftp_decode_directory(response, response_len, entries,
  ...
  if (decode_result == POLAR_SDK_PSFTP_DIR_DECODE_TOO_MANY_ENTRIES) {
    m_del(polar_sdk_psftp_dir_entry_t, entries,
          POLAR_SDK_PSFTP_MAX_DIR_ENTRIES);
    polar_raise_exc(&polar_type_BufferOverflowError,
                    MP_ERROR_TEXT("PSFTP directory entry overflow"));
  }
  if (decode_result != POLAR_SDK_PSFTP_DIR_DECODE_OK) {
    m_del(polar_sdk_psftp_dir_entry_t, entries,
          POLAR_SDK_PSFTP_MAX_DIR_ENTRIES);
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("failed to decode PSFTP directory"));
  }
  ...
  for (size_t i = 0; i < entry_count; ++i) {
    ...
    if (name_len == entry_name_len &&
        memcmp(entries[i].name, entry_name, entry_name_len) == 0) {
      found = true;
      break;
    }
  }
...
static void polar_psftp_fetch_directory_entries_or_raise(
    polar_h10_obj_t *self, const char *path,
    polar_sdk_psftp_dir_entry_t *entries, size_t entries_capacity,
    size_t *out_entry_count) {
  if (self == NULL || path == NULL || entries == NULL ||
      entries_capacity == 0u || out_entry_count == NULL) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("invalid PSFTP directory fetch"));
  }
  ...
  if (normalized_len == 0u || normalized_len > POLAR_SDK_PSFTP_MAX_PATH_BYTES) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("invalid PSFTP directory path"));
  }
...
static void polar_h10_recording_make_auto_id(polar_h10_obj_t *self, char *out,
                                             size_t out_capacity) {
  if (out == NULL || out_capacity < 25u) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("recording id buffer too small"));
  }
  ...
  pico_get_unique_board_id(&board_id);
  uint32_t board_tag =
      ((uint32_t)board_id.id[0] << 24) | ((uint32_t)board_id.id[1] << 16) |
      ((uint32_t)board_id.id[2] << 8) | (uint32_t)board_id.id[3];
#else
...
static void polar_h10_recording_make_unique_auto_id_or_raise(
    polar_h10_obj_t *self, char *out, size_t out_capacity) {
  ...
  polar_raise_exc(&polar_type_Error,
                  MP_ERROR_TEXT("failed to allocate unique recording id"));
}

static bool polar_h10_recording_local_config_matches(
    const polar_h10_obj_t *self,
    polar_sdk_h10_recording_sample_type_t sample_type, uint8_t interval_s) {
  return self->recording_hr_active && self->recording_hr_config_known &&
...
static void polar_h10_recording_prepare_or_raise(polar_h10_obj_t *self) {
  ...
  if (self->psftp_mtu_enabled &&
      (self->psftp_d2h_enabled || !self->psftp_d2h_char_found) &&
  ...
  self->psftp_last_security_related_failure =
      polar_sdk_psftp_prepare_failure_is_security_related(
          prepare_status, polar_psftp_security_ready(self));
  if (self->psftp_last_security_related_failure) {
    polar_raise_exc(&polar_type_SecurityError,
                    MP_ERROR_TEXT("PSFTP security setup failed"));
  }
  polar_raise_exc(&polar_type_Error,
                  MP_ERROR_TEXT("PSFTP channel preparation failed"));
}

static bool polar_h10_recording_find_any_stored_id_or_raise(
    polar_h10_obj_t *self, char *out_recording_id,
    size_t out_recording_id_capacity) {
  polar_sdk_psftp_dir_entry_t *root_entries =
      m_new(polar_sdk_psftp_dir_entry_t, POLAR_SDK_PSFTP_MAX_DIR_ENTRIES);
  size_t root_count = 0;
  polar_psftp_fetch_directory_entries_or_raise(
      self, "/", root_entries, POLAR_SDK_PSFTP_MAX_DIR_ENTRIES, &root_count);
  ...
  for (size_t i = 0; i < root_count && !found; ++i) {
    ...
    size_t sub_count = 0;
    polar_psftp_fetch_directory_entries_or_raise(
        self, dir_path, sub_entries, POLAR_SDK_PSFTP_MAX_DIR_ENTRIES,
        &sub_count);
      ...
      if (strcmp(sub_entries[j].name, "SAMPLES.BPB") == 0) {
        if (out_recording_id != NULL && out_recording_id_capacity > 0u) {
          if (dir_name_len >= out_recording_id_capacity) {
            m_del(polar_sdk_psftp_dir_entry_t, root_entries,
                  POLAR_SDK_PSFTP_MAX_DIR_ENTRIES);
            polar_raise_exc(&polar_type_Error,
                            MP_ERROR_TEXT("recording id buffer too small"));
          }
...
static bool polar_wait_for_mtu_exchange(polar_h10_obj_t *self,
                                        uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->mtu_exchange_done,
                                           timeout_ms);
}
...
static bool polar_pmd_mtu_is_connected(void *ctx) {
  ...
  return m->self->runtime_link.connected &&
         m->self->runtime_link.conn_handle != HCI_CON_HANDLE_INVALID;
}
...
static int polar_pmd_mtu_read(void *ctx, uint16_t *out_mtu) {
  ...
  if (gatt_client_get_mtu(m->self->runtime_link.conn_handle, &mtu) !=
      ERROR_CODE_SUCCESS) {
    return -1;
  }
...
static int polar_pmd_mtu_request_exchange(void *ctx) {
  ...
  gatt_client_send_mtu_negotiation(polar_hci_packet_handler_and_discovery,
                                   m->self->runtime_link.conn_handle);
  return 0;
}

static bool polar_pmd_mtu_wait_exchange_complete(void *ctx,
                                                 uint32_t timeout_ms) {
  polar_pmd_mtu_ctx_t *m = (polar_pmd_mtu_ctx_t *)ctx;
...
static bool polar_pmd_ensure_mtu(polar_h10_obj_t *self, uint16_t minimum_mtu,
                                 uint32_t timeout_ms) {
  ...
  polar_sdk_gatt_mtu_result_t r = polar_sdk_gatt_mtu_ensure_minimum(
      &ops, minimum_mtu, timeout_ms, &self->att_mtu);
...
static void polar_pmd_notify_start_listener(void *ctx) {
  polar_pmd_notify_ctx_t *p = (polar_pmd_notify_ctx_t *)ctx;
  gatt_client_listen_for_characteristic_value_updates(
      p->notification, polar_hci_packet_handler_and_discovery,
      p->self->runtime_link.conn_handle, p->chr);
  *p->listening = true;
}
...
static int polar_pmd_notify_write_ccc(void *ctx, uint16_t ccc_cfg) {
  ...
  uint8_t err = gatt_client_write_client_characteristic_configuration(
      polar_hci_packet_handler_and_discovery, p->self->runtime_link.conn_handle,
      p->chr, ccc_cfg);
  if (err != ERROR_CODE_SUCCESS) {
...
static bool polar_pmd_notify_wait_complete(void *ctx, uint32_t timeout_ms,
                                           uint8_t *out_att_status) {
  polar_pmd_notify_ctx_t *p = (polar_pmd_notify_ctx_t *)ctx;
...
static int polar_pmd_set_notify_for_char_result(
  ...
  return polar_sdk_pmd_map_notify_result(
      r, self->pmd_cfg_att_status, POLAR_SDK_PMD_OP_OK,
...
static void polar_raise_for_pmd_notify_status(int status, bool missing_char) {
  ...
  if (status == POLAR_SDK_PMD_OP_NOT_CONNECTED) {
    ...
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PMD CCC rejected"));
  } else if (missing_char) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PMD characteristic missing"));
  } else {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("failed to write PMD CCC"));
  }
}
...
static void polar_pmd_set_notify_for_char(
    polar_h10_obj_t *self, gatt_client_characteristic_t *chr,
    gatt_client_notification_t *notification, bool *listening, bool enable) {
  int status = polar_pmd_set_notify_for_char_result(self, chr, notification,
                                                    listening, enable);
...
static int polar_pmd_write_value_cb(void *ctx, const uint8_t *data,
                                    uint16_t len) {
  polar_pmd_write_ctx_t *w = (polar_pmd_write_ctx_t *)ctx;
  ...
  uint8_t err = gatt_client_write_value_of_characteristic(
      polar_hci_packet_handler_and_discovery, w->self->runtime_link.conn_handle,
      w->self->pmd_cp_handle, len, (uint8_t *)data);
  if (err != ERROR_CODE_SUCCESS) {
...
static bool polar_pmd_write_wait_cb(void *ctx, uint32_t timeout_ms,
                                    uint8_t *out_att_status) {
  polar_pmd_write_ctx_t *w = (polar_pmd_write_ctx_t *)ctx;
...
static int polar_pmd_write_command_result(polar_h10_obj_t *self,
  ...
  polar_sdk_gatt_write_result_t r =
      polar_sdk_gatt_write_with_wait(&ops, cmd, cmd_len, ATT_ERROR_SUCCESS,
                                     POLAR_SDK_GATT_OP_TIMEOUT_MS, &att_status);
...
static void polar_pmd_write_command(polar_h10_obj_t *self, const uint8_t *cmd,
                                    uint16_t cmd_len) {
  ...
  if (r == POLAR_SDK_PMD_OP_NOT_CONNECTED) {
    ...
    polar_raise_exc(&polar_type_TimeoutError,
                    MP_ERROR_TEXT("PMD write timeout"));
  } else if (r > 0) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PMD write rejected"));
  } else {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("failed to write PMD command"));
  }
}

static void polar_pmd_parse_cp_notification(polar_h10_obj_t *self,
                                            const uint8_t *value,
                                            uint16_t value_len) {
  ...
  if (self->pmd_cp_response_waiting &&
      ...
      (self->pmd_cp_response_expected_type == 0xffu ||
...
static void polar_pmd_parse_data(polar_h10_obj_t *self, const uint8_t *value,
                                 uint16_t value_len) {
  if (self->ecg_enabled) {
    polar_sdk_ecg_parse_pmd_notification(
        &self->ecg_ring, POLAR_SDK_PMD_MEAS_ECG, value, value_len);
  }
  if (self->imu_enabled) {
    polar_sdk_imu_parse_pmd_notification(
        &self->imu_ring, POLAR_SDK_PMD_MEAS_ACC, value, value_len);
  }
}

static bool polar_psftp_wait_cfg_complete(polar_h10_obj_t *self,
                                          uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->psftp_cfg_done,
                                           timeout_ms);
}

static bool polar_psftp_wait_write_complete(polar_h10_obj_t *self,
                                            uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->psftp_write_done,
                                           timeout_ms);
}

static bool polar_psftp_wait_response(polar_h10_obj_t *self,
                                      uint32_t timeout_ms) {
  return polar_wait_for_flag_or_disconnect(self, &self->psftp_response_done,
                                           timeout_ms);
}
...
static bool polar_psftp_security_ready(polar_h10_obj_t *self) {
  ...
  return polar_sdk_btstack_security_ready(self->runtime_link.conn_handle,
...
static void polar_clear_local_bond_for_addr_once(polar_h10_obj_t *self,
...
static void polar_psftp_clear_local_bond_once(polar_h10_obj_t *self) {
  ...
  polar_clear_local_bond_for_addr_once(self, self->peer_addr_type,
                                       self->peer_addr);
}
...
static bool polar_psftp_request_pairing_and_wait(polar_h10_obj_t *self) {
  ...
  for (size_t round = 0; round < POLAR_SDK_PMD_SECURITY_ROUNDS; ++round) {
    ...
    polar_sdk_security_result_t r = polar_sdk_btstack_security_ensure(
        &self->runtime_link.conn_handle, HCI_CON_HANDLE_INVALID, &policy,
        polar_psftp_security_sleep_ms_cb, self);
    ...
    bool pairing_failed = self->sm_pairing_complete_total != pair_before &&
                          self->sm_last_pairing_status != 0;
    if (pairing_failed && !dropped_stale_bond &&
...
static bool polar_psftp_notify_wait_complete(void *ctx, uint32_t timeout_ms,
                                             uint8_t *out_att_status) {
  polar_psftp_notify_ctx_t *p = (polar_psftp_notify_ctx_t *)ctx;
...
static bool polar_psftp_prepare_has_required_characteristics(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return self->psftp_mtu_char_found && self->psftp_h2d_char_found &&
         self->psftp_mtu_handle != 0 && self->psftp_h2d_handle != 0;
}

static polar_sdk_security_result_t
polar_psftp_prepare_ensure_security(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static int polar_psftp_prepare_enable_mtu_notify(void *ctx) {
  ...
  int status = polar_psftp_set_notify_for_char_result(
      self, &self->psftp_mtu_char, &self->psftp_mtu_notification,
      &self->psftp_mtu_notification_listening, true);
...
static int polar_psftp_prepare_enable_d2h_notify(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  int status = polar_psftp_set_notify_for_char_result(
      self, &self->psftp_d2h_char, &self->psftp_d2h_notification,
      &self->psftp_d2h_notification_listening, true);
  if (status == POLAR_SDK_PSFTP_OP_OK) {
...
static int polar_psftp_prepare_result(polar_h10_obj_t *self) {
  ...
  if (!self->psftp_mtu_enabled && !self->psftp_d2h_enabled &&
      !polar_psftp_security_ready(self)) {
  ...
  polar_sdk_psftp_prepare_ops_t ops = {
      ...
      .has_required_characteristics =
          polar_psftp_prepare_has_required_characteristics,
...
static int polar_psftp_write_value_cb(void *ctx, const uint8_t *data,
                                      uint16_t len) {
  polar_psftp_write_ctx_t *w = (polar_psftp_write_ctx_t *)ctx;
  ...
  uint8_t err = gatt_client_write_value_of_characteristic(
      polar_hci_packet_handler_and_discovery, w->self->runtime_link.conn_handle,
      w->self->psftp_mtu_handle, len, (uint8_t *)data);
  if (err != ERROR_CODE_SUCCESS) {
...
static bool polar_psftp_write_wait_cb(void *ctx, uint32_t timeout_ms,
                                      uint8_t *out_att_status) {
  polar_psftp_write_ctx_t *w = (polar_psftp_write_ctx_t *)ctx;
...
static int polar_psftp_write_frame_result(polar_h10_obj_t *self,
                                          const uint8_t *frame,
                                          uint16_t frame_len) {
  ...
  uint8_t att_status = ATT_ERROR_SUCCESS;
  polar_sdk_gatt_write_result_t r =
      polar_sdk_gatt_write_with_wait(&ops, frame, frame_len, ATT_ERROR_SUCCESS,
                                     POLAR_SDK_GATT_OP_TIMEOUT_MS, &att_status);
...
static int polar_psftp_get_write_frame_cb(void *ctx, const uint8_t *frame,
                                          uint16_t frame_len) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_psftp_get_begin_response_cb(void *ctx, uint8_t *response,
...
static polar_sdk_psftp_rx_result_t
polar_psftp_get_response_result_cb(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return self->psftp_response_result;
}

static const polar_sdk_psftp_rx_state_t *
polar_psftp_get_rx_state_cb(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return &self->psftp_rx_state;
}

static bool polar_psftp_get_is_remote_success_cb(void *ctx,
                                                 uint16_t error_code) {
  (void)ctx;
#if POLAR_CFG_ENABLE_PSFTP
...
static polar_sdk_psftp_trans_result_t
polar_psftp_map_prepare_status_to_result(int prepare_status) {
  if (prepare_status == POLAR_SDK_PSFTP_OP_NOT_CONNECTED) {
    return POLAR_SDK_PSFTP_TRANS_NOT_CONNECTED;
  }
  if (prepare_status == POLAR_SDK_PSFTP_OP_MISSING_CHAR ||
      prepare_status == POLAR_SDK_PSFTP_OP_NO_NOTIFY_PROP) {
...
static polar_sdk_psftp_trans_result_t
polar_psftp_map_write_status_to_result(int write_status) {
...
static polar_sdk_psftp_trans_result_t
polar_psftp_begin_get_stream(polar_h10_obj_t *self, const char *path,
                             size_t path_len) {
  if (self == NULL || path == NULL || self->download_blob == NULL ||
      self->download_blob_alloc == 0) {
  ...
  if (!polar_sdk_psftp_encode_get_operation(path, path_len, proto_payload,
                                            sizeof(proto_payload),
                                            &proto_payload_len)) {
    return POLAR_SDK_PSFTP_TRANS_ENCODE_FAILED;
  }
  ...
  size_t request_len = polar_sdk_psftp_build_rfc60_request(
      proto_payload, proto_payload_len, request_stream, sizeof(request_stream));
  if (request_len == 0) {
    return POLAR_SDK_PSFTP_TRANS_ENCODE_FAILED;
  }
  ...
  self->psftp_response_result = POLAR_SDK_PSFTP_RX_MORE;
  self->download_blob_off = 0;
  polar_sdk_psftp_rx_reset(&self->psftp_rx_state, self->download_blob,
                           self->download_blob_alloc);
  ...
  while (polar_sdk_psftp_tx_has_more(&tx)) {
    ...
    size_t frame_len = polar_sdk_psftp_tx_build_next_frame(
        &tx, frame_capacity, frame, sizeof(frame), &is_last);
    (void)is_last;
    ...
    int write_status =
        polar_psftp_write_frame_result(self, frame, (uint16_t)frame_len);
  ...
  for (;;) {
    ...
    polar_sdk_psftp_trans_result_t result =
        polar_sdk_psftp_execute_get_operation(
            &ops, path, path_len, response, response_capacity, timeout_ms,
            out_response_len, &error_code, &prepare_status, &write_status);
    ...
    if (prepare_status > 0 || write_status > 0) {
      self->psftp_last_att_status =
          (uint8_t)(write_status > 0 ? write_status : prepare_status);
    }
    ...
    if (!retried_after_reconnect && security_related &&
        polar_psftp_reconnect_after_security_failure(
            self, POLAR_SDK_PSFTP_RECONNECT_TIMEOUT_MS)) {
...
static polar_sdk_psftp_trans_result_t
polar_psftp_execute_proto(polar_h10_obj_t *self, const uint8_t *proto_payload,
                          size_t proto_payload_len, uint8_t *response,
                          size_t response_capacity, uint32_t timeout_ms,
  ...
  for (;;) {
    ...
    polar_sdk_psftp_trans_result_t result =
        polar_sdk_psftp_execute_proto_operation(
            &ops, proto_payload, proto_payload_len, response, response_capacity,
            timeout_ms, out_response_len, &error_code, &prepare_status,
            &write_status);
...
static polar_sdk_psftp_trans_result_t polar_psftp_execute_query(
    polar_h10_obj_t *self, uint16_t query_id, const uint8_t *query_payload,
    size_t query_payload_len, uint8_t *response, size_t response_capacity,
    uint32_t timeout_ms, size_t *out_response_len) {
  if (out_response_len != NULL) {
    *out_response_len = 0;
  }
  ...
  for (;;) {
    ...
    polar_sdk_psftp_trans_result_t result =
        polar_sdk_psftp_execute_query_operation(
            &ops, query_id, query_payload, query_payload_len, response,
            response_capacity, timeout_ms, out_response_len, &error_code,
            &prepare_status, &write_status);
...
polar_psftp_execute_remove(polar_h10_obj_t *self, const char *path,
  ...
  uint8_t proto_payload[POLAR_SDK_PSFTP_RUNTIME_MAX_PROTO_REQUEST_BYTES];
  size_t proto_payload_len = 0;
  if (!polar_sdk_psftp_encode_remove_operation(path, path_len, proto_payload,
                                               sizeof(proto_payload),
                                               &proto_payload_len)) {
    return POLAR_SDK_PSFTP_TRANS_ENCODE_FAILED;
  }
  ...
  size_t response_len = 0;
  return polar_psftp_execute_proto(self, proto_payload, proto_payload_len,
...
static NORETURN void polar_raise_psftp_remote_error(uint16_t error_code) {
  ...
  if (error_code == protocol_PbPFtpError_TIMEOUT) {
    mp_raise_msg_varg(&polar_type_TimeoutError,
                      MP_ERROR_TEXT("PSFTP error code %u"),
                      (unsigned)error_code);
  }
  if (error_code == protocol_PbPFtpError_NO_SUCH_FILE_OR_DIRECTORY ||
    ...
    mp_raise_msg_varg(&polar_type_ProtocolError,
                      MP_ERROR_TEXT("PSFTP error code %u"),
                      (unsigned)error_code);
  }
#endif
  mp_raise_msg_varg(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PSFTP error code %u"), (unsigned)error_code);
}
...
polar_raise_for_psftp_result(polar_h10_obj_t *self,
  ...
  if (result == POLAR_SDK_PSFTP_TRANS_MISSING_CHARACTERISTICS) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PSFTP characteristics missing"));
  }
  ...
  if (result == POLAR_SDK_PSFTP_TRANS_NOTIFY_ATT_REJECTED ||
      result == POLAR_SDK_PSFTP_TRANS_WRITE_ATT_REJECTED) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PSFTP ATT rejected"));
  }
  if (result == POLAR_SDK_PSFTP_TRANS_OVERFLOW) {
    polar_raise_exc(&polar_type_BufferOverflowError,
                    MP_ERROR_TEXT("PSFTP response overflow"));
  }
  if (result == POLAR_SDK_PSFTP_TRANS_SEQUENCE_ERROR) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PSFTP sequence error"));
  }
  if (result == POLAR_SDK_PSFTP_TRANS_PROTOCOL_ERROR) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PSFTP protocol error"));
  }
...
static void polar_h10_recording_query_status_or_raise(polar_h10_obj_t *self) {
  ...
  polar_sdk_psftp_trans_result_t result = polar_psftp_execute_query(
      self, POLAR_SDK_PSFTP_QUERY_REQUEST_RECORDING_STATUS, NULL, 0, response,
      sizeof(response), POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS, &response_len);
  if (result != POLAR_SDK_PSFTP_TRANS_OK) {
  ...
  if (!polar_sdk_psftp_decode_h10_recording_status_result(
          response, response_len, &recording_on, recording_id,
          sizeof(recording_id), &recording_id_len)) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("failed to decode recording status"));
  }
  ...
  bool keep_local_config =
      self->recording_hr_active && self->recording_hr_config_known &&
...
static NORETURN void
polar_raise_for_psftp_rx_result(polar_h10_obj_t *self,
                                polar_sdk_psftp_rx_result_t result) {
...
static void polar_link_on_connected_ready(void *ctx,
  ...
  uint8_t conn_update_err = gap_update_connection_parameters(
      self->runtime_link.conn_handle, POLAR_SDK_POST_CONN_INTERVAL_MIN,
      POLAR_SDK_POST_CONN_INTERVAL_MAX, POLAR_SDK_POST_CONN_LATENCY,
      POLAR_SDK_POST_CONN_SUPERVISION_TIMEOUT_10MS);
  self->runtime_link.conn_update_pending =
      (conn_update_err == ERROR_CODE_SUCCESS);
...
static void polar_link_on_disconnected(void *ctx,
                                       const polar_sdk_link_event_t *event) {
  (void)event;
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void
polar_adv_runtime_on_report(void *ctx,
...
static void
polar_adv_runtime_on_match(void *ctx,
                           const polar_sdk_btstack_adv_report_t *report) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static int polar_adv_runtime_connect(void *ctx, const uint8_t *addr,
                                     uint8_t addr_type) {
  (void)ctx;
  bd_addr_t a;
...
static void polar_hci_dispatch_on_adv_report(
  ...
  if (self->addr != mp_const_none) {
    ...
    const uint8_t *prefix =
        (const uint8_t *)mp_obj_str_get_data(self->name_prefix, &prefix_len);
  ...
  (void)polar_sdk_btstack_adv_runtime_on_report(
      &self->runtime_link, &filter, adv_report, ERROR_CODE_SUCCESS, &ops);
}

static void
polar_hci_dispatch_on_link_event(void *ctx,
  ...
  (void)polar_sdk_runtime_context_handle_link_event(
      &self->runtime_link, HCI_CON_HANDLE_INVALID, link_event,
      self->user_disconnect_requested, self->connect_intent, &ops);
}

static void
polar_hci_dispatch_on_sm_event(void *ctx,
                               const polar_sdk_sm_event_t *sm_event) {
  (void)ctx;
  (void)sm_event;
}

static void polar_hci_packet_handler(uint8_t packet_type, uint16_t channel,
                                     uint8_t *packet, uint16_t size) {
  (void)channel;
  (void)size;
...
static void polar_sm_on_pairing_complete(void *ctx,
...
static void polar_sm_packet_handler(uint8_t packet_type, uint16_t channel,
  ...
  (void)polar_sdk_sm_control_apply(&sm_event, self->runtime_link.conn_handle,
                                   HCI_CON_HANDLE_INVALID, &ops);
}

static polar_sdk_discovery_snapshot_t
polar_discovery_snapshot_from_obj(polar_h10_obj_t *self) {
...
static void
polar_discovery_runtime_set_stage(void *ctx,
...
static void polar_discovery_runtime_mark_att_fail(void *ctx,
                                                  uint8_t att_status) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_discovery_runtime_mark_hci_fail(void *ctx,
                                                  uint8_t hci_status) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_discovery_advance(polar_h10_obj_t *self, uint8_t att_status) {
  polar_sdk_discovery_snapshot_t snapshot =
      polar_discovery_snapshot_from_obj(self);
  ...
  polar_sdk_discovery_runtime_on_query_complete(
      &snapshot, POLAR_SDK_SERVICE_HR, POLAR_SDK_SERVICE_ECG,
      POLAR_SDK_SERVICE_PSFTP, att_status, ATT_ERROR_ATTRIBUTE_NOT_FOUND,
      ERROR_CODE_SUCCESS, &ops);
}

static uint8_t polar_discovery_cmd_hr_chars(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return gatt_client_discover_characteristics_for_service(
      polar_hci_packet_handler_and_discovery, self->runtime_link.conn_handle,
      &self->hr_service);
}
...
static void
polar_discovery_handle_service_result(polar_h10_obj_t *self,
                                      const gatt_client_service_t *svc) {
  polar_sdk_disc_service_kind_t kind = polar_sdk_btstack_classify_service(
      svc->uuid16, svc->uuid128, ORG_BLUETOOTH_SERVICE_HEART_RATE,
      POLAR_UUID16_PSFTP_SERVICE, POLAR_SDK_UUID_PMD_SERVICE_BE);
  ...
  polar_sdk_discovery_apply_service_kind(kind, &self->hr_service_found,
                                         &self->pmd_service_found,
                                         &self->psftp_service_found);
}

static void
polar_discovery_handle_char_result(polar_h10_obj_t *self,
                                   const gatt_client_characteristic_t *chr) {
  polar_sdk_disc_char_kind_t kind = polar_sdk_btstack_classify_char(
      self->discovery_stage, chr->uuid16, chr->uuid128,
      ORG_BLUETOOTH_CHARACTERISTIC_HEART_RATE_MEASUREMENT,
      POLAR_SDK_UUID_PMD_CP_BE, POLAR_SDK_UUID_PMD_DATA_BE,
      POLAR_SDK_UUID_PSFTP_MTU_BE, POLAR_SDK_UUID_PSFTP_D2H_BE,
  ...
  polar_sdk_discovery_apply_char_kind(
      kind, chr->value_handle, &self->hr_char_found,
      &self->hr_measurement_handle, &self->pmd_cp_char_found,
      &self->pmd_cp_handle, &self->pmd_data_char_found, &self->pmd_data_handle,
      &self->psftp_mtu_handle, &self->psftp_d2h_handle,
      &self->psftp_h2d_handle);
}

static bool polar_handle_gatt_route_event(
    polar_h10_obj_t *self, const polar_sdk_btstack_gatt_route_result_t *route) {
  if (route->kind == POLAR_SDK_GATT_ROUTE_MTU_EVENT) {
    if (self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID ||
        route->mtu.handle == self->runtime_link.conn_handle) {
  ...
  if (route->kind == POLAR_SDK_GATT_ROUTE_HR_VALUE) {
    ...
    polar_hr_parse_notification(self, route->value.value,
                                route->value.value_len);
    return true;
  }

  if (route->kind == POLAR_SDK_GATT_ROUTE_PMD_CP_VALUE) {
    polar_pmd_parse_cp_notification(self, route->value.value,
                                    route->value.value_len);
    return true;
  }
  ...
  if (route->kind == POLAR_SDK_GATT_ROUTE_PSFTP_MTU_VALUE) {
    ...
    if (self->psftp_response_waiting) {
      ...
      polar_sdk_psftp_rx_result_t r = polar_sdk_psftp_rx_feed_frame(
          &self->psftp_rx_state, route->value.value, route->value.value_len);
      self->psftp_response_result = r;

      if (self->download_handle_active &&
          (r == POLAR_SDK_PSFTP_RX_MORE || r == POLAR_SDK_PSFTP_RX_COMPLETE) &&
          route->value.value_len > 1u) {
        self->psftp_last_response_bytes +=
            (uint32_t)(route->value.value_len - 1u);
      }

      if (r == POLAR_SDK_PSFTP_RX_SEQUENCE_ERROR) {
        self->psftp_rx_seq_errors_total += 1;
        self->psftp_last_error_code =
            POLAR_SDK_PSFTP_PFTP_AIR_PACKET_LOST_ERROR;
  ...
  if (route->kind == POLAR_SDK_GATT_ROUTE_QUERY_COMPLETE) {
    ...
    if (polar_sdk_gatt_apply_query_complete(route->query_complete_att_status,
                                            slots, MP_ARRAY_SIZE(slots),
                                            &self->last_att_status)) {
      return true;
    }

    if (self->runtime_link.state == POLAR_SDK_STATE_DISCOVERING &&
        self->runtime_link.conn_handle != HCI_CON_HANDLE_INVALID) {
...
static void polar_handle_discovery_decode_apply(polar_h10_obj_t *self,
                                                uint8_t packet_type,
                                                uint8_t *packet) {
  if (self->runtime_link.state != POLAR_SDK_STATE_DISCOVERING ||
      self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID) {
  ...
  if (!polar_sdk_discovery_btstack_decode_result(
          packet_type, packet, self->discovery_stage, &disc_result)) {
    return;
  }
...
static void polar_hci_packet_handler_and_discovery(uint8_t packet_type,
  ...
  if (!polar_sdk_btstack_route_gatt_event(packet_type, packet, &route_state,
                                          &route)) {
...
static void polar_discovery_start(polar_h10_obj_t *self) {
  ...
  uint8_t err = gatt_client_discover_primary_services(
      polar_hci_packet_handler_and_discovery, self->runtime_link.conn_handle);
  if (err != ERROR_CODE_SUCCESS) {
...
static bool polar_wait_until_ready_or_fail(polar_h10_obj_t *self,
                                           uint32_t budget_ms) {
...
static void polar_wait_conn_update_settle(polar_h10_obj_t *self,
                                          uint32_t timeout_ms) {
  uint32_t start_ms = polar_now_ms();
  while (self->runtime_link.conn_update_pending &&
         !polar_elapsed_ms(start_ms, timeout_ms)) {
    if (!self->runtime_link.connected ||
        self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID) {
      break;
    }
...
static void polar_cleanup_after_attempt(polar_h10_obj_t *self) {
  ...
  polar_sdk_runtime_context_cleanup_attempt(
      &self->runtime_link, HCI_CON_HANDLE_INVALID, POLAR_SDK_DISCONNECT_WAIT_MS,
      POLAR_SDK_RUNTIME_STATE_RECOVERING, &ops);
}

static void polar_ensure_ble_ready(void) {
  // Ensure the high-level bluetooth.BLE singleton exists so modbluetooth
  // ringbuf callbacks always have a valid object, even though we don't use
  // Python IRQs here.
  if (MP_STATE_VM(bluetooth) == MP_OBJ_NULL) {
    mp_obj_t bt_module = mp_import_name(MP_QSTR_bluetooth, mp_const_none,
  ...
  if (!mp_bluetooth_is_active()) {
    ...
    if (err != 0) {
      polar_raise_exc(&polar_type_Error,
                      MP_ERROR_TEXT("failed to initialize Bluetooth stack"));
    }
...
static void polar_register_hci_handler_refresh(void) {
  ...
  // reinit sequences where our C static state may outlive btstack
  // registrations.
...
static bool polar_transport_start_attempt(void *ctx) {
  ...
  if (self->addr != mp_const_none) {
    ...
    const char *addr_str = mp_obj_str_get_str(self->addr);
    if (sscanf_bd_addr(addr_str, target_addr) != 0) {
      polar_clear_local_bond_for_addr_once(self, BD_ADDR_TYPE_UNKNOWN,
                                           target_addr);
    }
  }

  gap_set_scan_params(1, POLAR_SDK_SCAN_INTERVAL_UNITS,
                      POLAR_SDK_SCAN_WINDOW_UNITS, 0);
  gap_start_scan();
  return true;
}

static polar_sdk_transport_attempt_result_t
polar_transport_wait_attempt(void *ctx, uint32_t attempt_budget_ms) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static polar_sdk_transport_connect_result_t
polar_transport_connect_with_timeout(polar_h10_obj_t *self,
                                     uint32_t timeout_ms) {
  ...
  polar_sdk_transport_connect_result_t connect_result =
      polar_sdk_transport_connect_blocking(&connect_policy, &transport_ops,
                                           &transport_stats);
...
static bool polar_psftp_reconnect_after_security_failure(polar_h10_obj_t *self,
  ...
  polar_sdk_transport_connect_result_t connect_result =
      polar_transport_connect_with_timeout(self, timeout_ms);
...
static mp_obj_t polar_h10_make_new(const mp_obj_type_t *type, size_t n_args,
  ...
  static const mp_arg_t allowed_args[] = {
      ...
      {MP_QSTR_name_prefix,
       ...
       {.u_obj = MP_OBJ_NEW_QSTR(MP_QSTR_Polar)}},
      {MP_QSTR_required_capabilities,
       MP_ARG_KW_ONLY | MP_ARG_OBJ,
  ...
  if (parsed_args[ARG_addr].u_obj != mp_const_none &&
      !mp_obj_is_str(parsed_args[ARG_addr].u_obj)) {
    mp_raise_TypeError(MP_ERROR_TEXT("addr must be str or None"));
  }
  if (parsed_args[ARG_name_prefix].u_obj != mp_const_none &&
      !mp_obj_is_str(parsed_args[ARG_name_prefix].u_obj)) {
  ...
  if (!polar_required_capabilities_parse(
          parsed_args[ARG_required_capabilities].u_obj, &required_caps,
          &required_services_mask)) {
  ...
  self->ecg_enabled = false;
  self->ecg_companion_active = false;
  polar_sdk_ecg_ring_init(&self->ecg_ring, self->ecg_ring_storage,
                          POLAR_SDK_ECG_RING_BYTES);

  self->imu_enabled = false;
  polar_sdk_imu_ring_init(&self->imu_ring, self->imu_ring_storage,
                          POLAR_SDK_IMU_RING_BYTES);
  self->recording_hr_active = false;
  self->recording_hr_config_known = false;
  self->recording_hr_interval_s = 1u;
  self->recording_hr_sample_type = POLAR_SDK_H10_RECORDING_SAMPLE_HEART_RATE;
  ...
  memset(&self->psftp_mtu_notification, 0,
         sizeof(self->psftp_mtu_notification));
  memset(&self->psftp_d2h_notification, 0,
         sizeof(self->psftp_d2h_notification));
  self->psftp_mtu_notification_listening = false;
  self->psftp_d2h_notification_listening = false;
  self->psftp_mtu_enabled = false;
  self->psftp_d2h_enabled = false;
...
static mp_obj_t polar_h10_state(mp_obj_t self_in) {
  ...
  return mp_obj_new_str(polar_state_strs[self->runtime_link.state],
                        strlen(polar_state_strs[self->runtime_link.state]));
}
...
static MP_DEFINE_CONST_FUN_OBJ_1(polar_h10_is_connected_obj,
                                 polar_h10_is_connected);

static mp_obj_t polar_h10_set_required_capabilities(mp_obj_t self_in,
                                                    mp_obj_t caps_in) {
  polar_h10_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_set_required_capabilities_obj,
                                 polar_h10_set_required_capabilities);
...
static MP_DEFINE_CONST_FUN_OBJ_1(polar_h10_required_capabilities_obj,
                                 polar_h10_required_capabilities);

static mp_obj_t polar_stream_kind_capabilities_dict(
    bool supported, bool has_raw, bool has_decoded, const char *default_format,
    const char *units, mp_obj_t decoded_sample_size, mp_obj_t config) {
  mp_obj_t dict = mp_obj_new_dict(config == MP_OBJ_NULL ? 5 : 6);
  mp_obj_dict_store(dict, mp_obj_new_str("supported", 9),
                    mp_obj_new_bool(supported));
  ...
  mp_obj_dict_store(dict, mp_obj_new_str("default_format", 14),
                    mp_obj_new_str(default_format, strlen(default_format)));
  mp_obj_dict_store(dict, mp_obj_new_str("units", 5),
                    mp_obj_new_str(units, strlen(units)));
  mp_obj_dict_store(dict, mp_obj_new_str("decoded_sample_size_bytes", 25),
                    decoded_sample_size);
...
static mp_obj_t polar_config_desc_range(mp_int_t min_v, mp_int_t max_v,
                                        mp_int_t step_v) {
  mp_obj_t dict = mp_obj_new_dict(4);
  mp_obj_dict_store(dict, mp_obj_new_str("type", 4),
                    mp_obj_new_str("range", 5));
...
static mp_obj_t polar_config_desc_enum(const char *const *choices,
                                       size_t choice_count) {
...
static mp_obj_t polar_capability_bool_dict4(bool control, bool status,
                                            bool list, bool delete_) {
  mp_obj_t dict = mp_obj_new_dict(4);
  mp_obj_dict_store(dict, mp_obj_new_str("control", 7),
                    mp_obj_new_bool(control));
  ...
  mp_obj_dict_store(dict, mp_obj_new_str("delete", 6),
                    mp_obj_new_bool(delete_));
  return dict;
}
...
static mp_obj_t polar_h10_capabilities(mp_obj_t self_in) {
  ...
  mp_obj_dict_store(root, mp_obj_new_str("schema_version", 14),
                    mp_obj_new_int(1));
  ...
  mp_obj_dict_store(device, mp_obj_new_str("model", 5),
                    mp_obj_new_str("unknown", 7));
  mp_obj_dict_store(device, mp_obj_new_str("family", 6),
                    mp_obj_new_str("polar", 5));
  ...
  if (POLAR_CFG_ENABLE_HR) {
    ...
    mp_obj_dict_store(
        by_kind, mp_obj_new_str("hr", 2),
        polar_stream_kind_capabilities_dict(true, true, true, "decoded", "bpm",
                                            mp_obj_new_int(12), MP_OBJ_NULL));
  }
  if (POLAR_CFG_ENABLE_ECG) {
    mp_obj_t ecg_cfg = mp_obj_new_dict(1);
    mp_obj_dict_store(ecg_cfg, mp_obj_new_str("sample_rate_hz", 14),
                      polar_config_desc_range(1, 65535, 1));
    mp_obj_list_append(kinds, mp_obj_new_str("ecg", 3));
    mp_obj_dict_store(
        by_kind, mp_obj_new_str("ecg", 3),
        polar_stream_kind_capabilities_dict(true, false, true, "decoded", "uV",
                                            mp_obj_new_int(4), ecg_cfg));

    mp_obj_t acc_cfg = mp_obj_new_dict(2);
    mp_obj_dict_store(acc_cfg, mp_obj_new_str("sample_rate_hz", 14),
                      polar_config_desc_range(1, 65535, 1));
    mp_obj_dict_store(acc_cfg, mp_obj_new_str("range", 5),
                      polar_config_desc_range(1, 65535, 1));
    mp_obj_list_append(kinds, mp_obj_new_str("acc", 3));
    mp_obj_dict_store(
        by_kind, mp_obj_new_str("acc", 3),
        polar_stream_kind_capabilities_dict(true, false, true, "decoded", "mg",
                                            mp_obj_new_int(6), acc_cfg));
  }

  mp_obj_t streams = mp_obj_new_dict(3);
  mp_obj_dict_store(streams, mp_obj_new_str("kinds", 5), kinds);
  mp_obj_dict_store(streams, mp_obj_new_str("max_parallel_pmd_streams", 24),
                    mp_obj_new_int(POLAR_CFG_ENABLE_ECG ? 2 : 0));
  ...
  mp_obj_dict_store(recording, mp_obj_new_str("supported", 9),
                    mp_obj_new_bool(recording_supported));
  ...
  if (recording_supported) {
    ...
    mp_obj_dict_store(
        hr_cfg, mp_obj_new_str("sample_type", 11),
        polar_config_desc_enum(h10_recording_sample_type_choices, 2));
    mp_obj_dict_store(hr_cfg, mp_obj_new_str("interval_s", 10),
                      polar_config_desc_range(1, 5, 4));
  ...
  mp_obj_dict_store(recording, mp_obj_new_str("max_parallel_kinds", 18),
                    mp_obj_new_int(recording_supported ? 1 : 0));
  mp_obj_dict_store(
      recording, mp_obj_new_str("features", 8),
      polar_capability_bool_dict4(recording_supported, recording_supported,
  ...
  mp_obj_dict_store(psftp, mp_obj_new_str("read", 4),
                    mp_obj_new_bool(POLAR_CFG_ENABLE_PSFTP));
  mp_obj_dict_store(psftp, mp_obj_new_str("delete", 6),
                    mp_obj_new_bool(POLAR_CFG_ENABLE_PSFTP));
  mp_obj_dict_store(psftp, mp_obj_new_str("chunked_download", 16),
                    mp_obj_new_bool(POLAR_CFG_ENABLE_PSFTP));
  mp_obj_dict_store(
      psftp, mp_obj_new_str("max_chunk_bytes", 15),
      mp_obj_new_int_from_uint(POLAR_SDK_PSFTP_MAX_DOWNLOAD_BYTES));
...
#endif
  mp_obj_dict_store(security, mp_obj_new_str("bonded", 6),
                    mp_obj_new_bool(bonded));
  mp_obj_dict_store(security, mp_obj_new_str("encryption_key_size", 19),
                    mp_obj_new_int_from_uint(key_size));
  mp_obj_dict_store(security, mp_obj_new_str("pairing_method", 14),
                    mp_const_none);
  mp_obj_dict_store(root, mp_obj_new_str("security", 8), security);

  return root;
}
static MP_DEFINE_CONST_FUN_OBJ_1(polar_h10_capabilities_obj,
                                 polar_h10_capabilities);
...
static mp_obj_t polar_btstack_config_snapshot_dict(void) {
  ...
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_feature_mask),
                    mp_obj_new_int_from_uint(polar_btstack_cfg_feature_mask()));

#if defined(ENABLE_LE_CENTRAL)
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_le_central),
                    mp_const_true);
#else
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_le_central),
...
#if defined(ENABLE_LE_PERIPHERAL)
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_le_peripheral),
                    mp_const_true);
#else
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_le_peripheral),
  ...
  mp_obj_dict_store(
      dict,
      MP_OBJ_NEW_QSTR(
          MP_QSTR_btstack_cfg_enable_hci_controller_to_host_flow_control),
      mp_const_true);
#else
  mp_obj_dict_store(
      dict,
...
#if defined(ENABLE_LOG_INFO)
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_log_info),
                    mp_const_true);
#else
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_log_info),
                    mp_const_false);
...
#if defined(ENABLE_LOG_ERROR)
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_log_error),
                    mp_const_true);
#else
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_log_error),
                    mp_const_false);
...
#if defined(ENABLE_LOG_DEBUG)
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_log_debug),
                    mp_const_true);
#else
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_enable_log_debug),
                    mp_const_false);
  ...
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_hci_acl_payload_size),
                    mp_obj_new_int_from_uint((uint32_t)HCI_ACL_PAYLOAD_SIZE));
#endif
#if defined(MAX_NR_HCI_CONNECTIONS)
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_hci_connections),
                    mp_obj_new_int_from_uint((uint32_t)MAX_NR_HCI_CONNECTIONS));
#endif
#if defined(MAX_NR_GATT_CLIENTS)
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_gatt_clients),
                    mp_obj_new_int_from_uint((uint32_t)MAX_NR_GATT_CLIENTS));
#endif
#if defined(MAX_NR_L2CAP_CHANNELS)
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_l2cap_channels),
                    mp_obj_new_int_from_uint((uint32_t)MAX_NR_L2CAP_CHANNELS));
#endif
#if defined(MAX_NR_L2CAP_SERVICES)
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_l2cap_services),
                    mp_obj_new_int_from_uint((uint32_t)MAX_NR_L2CAP_SERVICES));
#endif
#if defined(MAX_NR_SM_LOOKUP_ENTRIES)
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_sm_lookup_entries),
      mp_obj_new_int_from_uint((uint32_t)MAX_NR_SM_LOOKUP_ENTRIES));
#endif
#if defined(MAX_NR_WHITELIST_ENTRIES)
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_whitelist_entries),
      mp_obj_new_int_from_uint((uint32_t)MAX_NR_WHITELIST_ENTRIES));
#endif
#if defined(MAX_NR_LE_DEVICE_DB_ENTRIES)
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_nr_le_device_db_entries),
      mp_obj_new_int_from_uint((uint32_t)MAX_NR_LE_DEVICE_DB_ENTRIES));
#endif
#if defined(MAX_ATT_DB_SIZE)
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_max_att_db_size),
                    mp_obj_new_int_from_uint((uint32_t)MAX_ATT_DB_SIZE));
#endif
#if defined(HCI_RESET_RESEND_TIMEOUT_MS)
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_cfg_hci_reset_resend_timeout_ms),
      mp_obj_new_int_from_uint((uint32_t)HCI_RESET_RESEND_TIMEOUT_MS));
#endif
...
static mp_obj_t polar_h10_stats(mp_obj_t self_in) {
  ...
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_state),
      mp_obj_new_str(polar_state_strs[self->runtime_link.state],
                     strlen(polar_state_strs[self->runtime_link.state])));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_connected),
                    mp_obj_new_bool(self->runtime_link.connected));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_connect_calls),
                    mp_obj_new_int_from_uint(self->connect_calls));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_disconnect_calls),
                    mp_obj_new_int_from_uint(self->disconnect_calls));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_hr),
                    mp_obj_new_bool(POLAR_CFG_ENABLE_HR));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_ecg),
                    mp_obj_new_bool(POLAR_CFG_ENABLE_ECG));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_psftp),
                    mp_obj_new_bool(POLAR_CFG_ENABLE_PSFTP));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_required_capabilities),
      polar_capabilities_tuple_from_mask(self->required_capabilities_mask));

#if POLAR_SDK_HAS_BTSTACK
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_config),
                    polar_btstack_config_snapshot_dict());
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_polar_conn_handle),
                    mp_obj_new_int_from_uint(self->runtime_link.conn_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_connect_attempts_total),
                    mp_obj_new_int_from_uint(self->connect_attempts_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_connect_success_total),
                    mp_obj_new_int_from_uint(self->connect_success_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_reconnect_backoff_events),
                    mp_obj_new_int_from_uint(self->reconnect_backoff_events));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hci_events_total),
                    mp_obj_new_int_from_uint(self->hci_events_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_adv_reports_total),
                    mp_obj_new_int_from_uint(self->adv_reports_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_adv_match_total),
                    mp_obj_new_int_from_uint(self->adv_match_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_conn_complete_total),
      mp_obj_new_int_from_uint(self->runtime_link.conn_complete_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_conn_update_complete_total),
      mp_obj_new_int_from_uint(self->runtime_link.conn_update_complete_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_att_mtu),
                    mp_obj_new_int_from_uint(self->att_mtu));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_mtu_exchange_pending),
                    mp_obj_new_bool(self->mtu_exchange_pending));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_mtu_exchange_total),
                    mp_obj_new_int_from_uint(self->mtu_exchange_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_conn_interval_units),
      mp_obj_new_int_from_uint(self->runtime_link.last_conn_interval_units));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_conn_latency),
      mp_obj_new_int_from_uint(self->runtime_link.last_conn_latency));
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_last_conn_supervision_timeout_10ms),
                    mp_obj_new_int_from_uint(
                        self->runtime_link.last_conn_supervision_timeout_10ms));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_conn_update_status),
      mp_obj_new_int_from_uint(self->runtime_link.last_conn_update_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_conn_update_pending),
                    mp_obj_new_bool(self->runtime_link.conn_update_pending));
  ...
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_conn_encryption_key_size),
                    mp_obj_new_int_from_uint(conn_encryption_key_size));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_conn_bonded),
                    mp_obj_new_bool(conn_bonded));

  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_disconnect_events_total),
      mp_obj_new_int_from_uint(self->runtime_link.disconnect_events_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_disconnect_status),
      mp_obj_new_int_from_uint(self->runtime_link.last_disconnect_status));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_disconnect_reason),
      mp_obj_new_int_from_uint(self->runtime_link.last_disconnect_reason));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_disconnect_reason_0x08_total),
                    mp_obj_new_int_from_uint(
                        self->runtime_link.disconnect_reason_0x08_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_disconnect_reason_0x3e_total),
                    mp_obj_new_int_from_uint(
                        self->runtime_link.disconnect_reason_0x3e_total));
  mp_obj_dict_store(dict,
                    MP_OBJ_NEW_QSTR(MP_QSTR_disconnect_reason_other_total),
                    mp_obj_new_int_from_uint(
                        self->runtime_link.disconnect_reason_other_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_sm_pairing_complete_total),
                    mp_obj_new_int_from_uint(self->sm_pairing_complete_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_sm_last_pairing_status),
                    mp_obj_new_int_from_uint(self->sm_last_pairing_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_sm_last_pairing_reason),
                    mp_obj_new_int_from_uint(self->sm_last_pairing_reason));

  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_hci_status),
      mp_obj_new_int_from_uint(self->runtime_link.last_hci_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_last_att_status),
                    mp_obj_new_int_from_uint(self->last_att_status));

  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_measurement_handle),
                    mp_obj_new_int_from_uint(self->hr_measurement_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_enabled),
                    mp_obj_new_bool(self->hr_enabled));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_notifications_total),
                    mp_obj_new_int_from_uint(self->hr_notifications_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_indications_total),
                    mp_obj_new_int_from_uint(self->hr_indications_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_value_events_total),
                    mp_obj_new_int_from_uint(self->hr_value_events_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_unmatched_value_events_total),
      mp_obj_new_int_from_uint(self->hr_unmatched_value_events_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_parse_errors_total),
      mp_obj_new_int_from_uint(self->hr_state.parse_errors_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_resubscribe_total),
                    mp_obj_new_int_from_uint(self->hr_resubscribe_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_last_resubscribe_ms),
                    mp_obj_new_int_from_uint(self->hr_last_resubscribe_ms));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_last_bpm),
                    mp_obj_new_int_from_uint(self->hr_state.last_bpm));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_last_rr_count),
                    mp_obj_new_int_from_uint(self->hr_state.last_rr_count));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_hr_last_flags),
                    mp_obj_new_int_from_uint(self->hr_state.last_flags));

  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_cp_handle),
                    mp_obj_new_int_from_uint(self->pmd_cp_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_data_handle),
                    mp_obj_new_int_from_uint(self->pmd_data_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_cp_properties),
                    mp_obj_new_int_from_uint(self->pmd_cp_char.properties));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_data_properties),
                    mp_obj_new_int_from_uint(self->pmd_data_char.properties));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_cfg_att_status),
                    mp_obj_new_int_from_uint(self->pmd_cfg_att_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_write_att_status),
                    mp_obj_new_int_from_uint(self->pmd_write_att_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_cp_response_status),
                    mp_obj_new_int_from_uint(self->pmd_cp_response_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_enabled),
                    mp_obj_new_bool(self->ecg_enabled));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_available_bytes),
                    mp_obj_new_int_from_uint(self->ecg_ring.count));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_notifications_total),
      mp_obj_new_int_from_uint(self->ecg_ring.notifications_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_frames_total),
                    mp_obj_new_int_from_uint(self->ecg_ring.frames_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_samples_total),
                    mp_obj_new_int_from_uint(self->ecg_ring.samples_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_parse_errors_total),
      mp_obj_new_int_from_uint(self->ecg_ring.parse_errors_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_drop_bytes_total),
                    mp_obj_new_int_from_uint(self->ecg_ring.drop_bytes_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_ecg_ring_high_water),
                    mp_obj_new_int_from_uint(self->ecg_ring.ring_high_water));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_enabled),
                    mp_obj_new_bool(self->imu_enabled));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_available_bytes),
                    mp_obj_new_int_from_uint(self->imu_ring.count));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_notifications_total),
      mp_obj_new_int_from_uint(self->imu_ring.notifications_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_frames_total),
                    mp_obj_new_int_from_uint(self->imu_ring.frames_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_samples_total),
                    mp_obj_new_int_from_uint(self->imu_ring.samples_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_parse_errors_total),
      mp_obj_new_int_from_uint(self->imu_ring.parse_errors_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_drop_bytes_total),
                    mp_obj_new_int_from_uint(self->imu_ring.drop_bytes_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_acc_ring_high_water),
                    mp_obj_new_int_from_uint(self->imu_ring.ring_high_water));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_cp_notifications_total),
                    mp_obj_new_int_from_uint(self->pmd_cp_notifications_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_pmd_cp_response_total),
                    mp_obj_new_int_from_uint(self->pmd_cp_response_total));

  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_mtu_handle),
                    mp_obj_new_int_from_uint(self->psftp_mtu_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_d2h_handle),
                    mp_obj_new_int_from_uint(self->psftp_d2h_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_h2d_handle),
                    mp_obj_new_int_from_uint(self->psftp_h2d_handle));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_mtu_properties),
                    mp_obj_new_int_from_uint(self->psftp_mtu_char.properties));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_d2h_properties),
                    mp_obj_new_int_from_uint(self->psftp_d2h_char.properties));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_h2d_properties),
                    mp_obj_new_int_from_uint(self->psftp_h2d_char.properties));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_mtu_enabled),
                    mp_obj_new_bool(self->psftp_mtu_enabled));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_d2h_enabled),
                    mp_obj_new_bool(self->psftp_d2h_enabled));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_tx_frames_total),
                    mp_obj_new_int_from_uint(self->psftp_tx_frames_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_rx_frames_total),
                    mp_obj_new_int_from_uint(self->psftp_rx_frames_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_rx_seq_errors_total),
                    mp_obj_new_int_from_uint(self->psftp_rx_seq_errors_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_protocol_errors_total),
      mp_obj_new_int_from_uint(self->psftp_protocol_errors_total));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_overflow_errors_total),
      mp_obj_new_int_from_uint(self->psftp_overflow_errors_total));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_last_error_code),
                    mp_obj_new_int_from_uint(self->psftp_last_error_code));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_last_att_status),
                    mp_obj_new_int_from_uint(self->psftp_last_att_status));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_psftp_last_response_bytes),
                    mp_obj_new_int_from_uint(self->psftp_last_response_bytes));
#endif
...
static mp_obj_t polar_h10_connect(size_t n_args, const mp_obj_t *args,
                                  mp_map_t *kw_args) {
  enum {
    ARG_timeout_ms,
    ARG_required_capabilities,
  ...
  polar_raise_exc(
      &polar_type_Error,
      MP_ERROR_TEXT(
          "Bluetooth BTstack central mode is not available in this build"));
#else
  ...
  if (parsed_args[ARG_required_capabilities].u_obj != mp_const_none) {
    (void)polar_required_capabilities_parse(
        parsed_args[ARG_required_capabilities].u_obj, &connect_caps_mask,
        &connect_services_mask);
  }
  ...
  if (polar_active_h10 != NULL && polar_active_h10 != self) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("another polar.Device transport is active"));
  }
  ...
  polar_sdk_transport_connect_result_t connect_result =
      polar_transport_connect_with_timeout(self, (uint32_t)timeout_ms);
...
static mp_obj_t polar_h10_disconnect(size_t n_args, const mp_obj_t *args,
                                     mp_map_t *kw_args) {
  enum {
    ARG_timeout_ms,
  };
  ...
  if (self->runtime_link.connected &&
    ...
    if (err != ERROR_CODE_SUCCESS) {
      self->runtime_link.last_hci_status = err;
      polar_raise_exc(&polar_type_Error,
                      MP_ERROR_TEXT("gap_disconnect failed"));
    }
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_disconnect_obj, 1,
                                  polar_h10_disconnect);
...
static mp_obj_t polar_h10_start_hr(mp_obj_t self_in) {
  ...
  polar_raise_exc(
      &polar_type_Error,
      MP_ERROR_TEXT("Bluetooth BTstack is not available in this build"));
#else
  if (!POLAR_CFG_ENABLE_HR) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("HR feature disabled at build time"));
  }
  ...
  if (self->hr_enabled) {
    ...
    polar_raise_exc(
        &polar_type_Error,
        MP_ERROR_TEXT("HR stream already active with different format"));
  }
  self->hr_stream_format = POLAR_STREAM_FORMAT_DECODED;
  polar_hr_set_notify(self, true);
...
static mp_obj_t polar_h10_read_hr(size_t n_args, const mp_obj_t *args,
                                  mp_map_t *kw_args) {
  enum {
    ARG_timeout_ms,
  };
  ...
  if (!self->hr_enabled ||
      self->hr_stream_format != POLAR_STREAM_FORMAT_DECODED) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("decoded HR stream is not active"));
  }
...
static polar_sdk_security_result_t polar_pmd_policy_ensure_security(void *ctx) {
  ...
  return polar_sdk_btstack_security_ensure(
      &self->runtime_link.conn_handle, HCI_CON_HANDLE_INVALID, &policy,
      polar_pmd_policy_security_sleep_ms, self);
}

static int polar_pmd_policy_enable_cp_notify(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return polar_pmd_set_notify_for_char_result(
      self, &self->pmd_cp_char, &self->pmd_cp_notification,
      &self->pmd_cp_notification_listening, true);
}

static int polar_pmd_policy_enable_data_notify(void *ctx) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return polar_pmd_set_notify_for_char_result(
      self, &self->pmd_data_char, &self->pmd_data_notification,
      &self->pmd_data_notification_listening, true);
}
...
static int polar_pmd_policy_enable_notifications(void *ctx) {
  ...
  int r = polar_sdk_pmd_enable_notify_pair(
      &ops, POLAR_SDK_PMD_OP_OK, POLAR_SDK_PMD_OP_NOT_CONNECTED,
      POLAR_SDK_PMD_OP_TRANSPORT, &last_att_status);
...
static int polar_pmd_policy_ensure_minimum_mtu(void *ctx,
...
static void polar_pmd_policy_expect_response_cb(void *ctx, uint8_t opcode,
                                                uint8_t measurement_type) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  polar_pmd_expect_response(self, opcode, measurement_type);
}

static int polar_pmd_policy_write_command_cb(void *ctx, const uint8_t *cmd,
                                             uint16_t cmd_len) {
  polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
  return polar_pmd_write_command_result(self, cmd, cmd_len);
}

static bool polar_pmd_policy_wait_response_cb(void *ctx, uint32_t timeout_ms,
...
static int polar_pmd_policy_start_measurement_and_wait_response(
    void *ctx, const uint8_t *start_cmd, size_t start_cmd_len,
    uint8_t *out_status) {
  ...
  return polar_sdk_pmd_start_command_and_wait(
      &ops, start_cmd, start_cmd_len,
      POLAR_SDK_PMD_OP_REQUEST_MEASUREMENT_START, measurement_type,
      POLAR_SDK_GATT_OP_TIMEOUT_MS, POLAR_SDK_PMD_OP_OK,
      POLAR_SDK_PMD_OP_NOT_CONNECTED, POLAR_SDK_PMD_OP_TIMEOUT,
      POLAR_SDK_PMD_OP_TRANSPORT, out_status);
}

static void
polar_raise_for_pmd_start_result(polar_sdk_pmd_start_result_t start_result,
                                 bool is_ecg) {
  switch (start_result) {
  ...
  case POLAR_SDK_PMD_START_RESULT_SECURITY_TIMEOUT:
    polar_raise_exc(&polar_type_SecurityError,
                    MP_ERROR_TEXT("failed to establish secure link for PMD"));
    break;
  ...
  case POLAR_SDK_PMD_START_RESULT_MTU_FAILED:
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PMD requires larger ATT MTU"));
    break;
  case POLAR_SDK_PMD_START_RESULT_START_TIMEOUT:
    polar_raise_exc(&polar_type_TimeoutError,
                    MP_ERROR_TEXT("PMD start response timeout"));
    break;
  case POLAR_SDK_PMD_START_RESULT_START_REJECTED:
    polar_raise_exc(&polar_type_ProtocolError,
                    is_ecg ? MP_ERROR_TEXT("PMD start ECG rejected")
                           : MP_ERROR_TEXT("PMD start ACC rejected"));
    break;
  case POLAR_SDK_PMD_START_RESULT_TRANSPORT_ERROR:
  default:
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("PMD start transport failure"));
    break;
  }
}

static polar_sdk_pmd_start_result_t
polar_h10_start_ecg_raw(polar_h10_obj_t *self, uint16_t sample_rate,
  ...
  polar_sdk_pmd_start_ops_t ops = {
      ...
      .start_ecg_and_wait_response =
          polar_pmd_policy_start_measurement_and_wait_response,
  };

  uint8_t start_response_status = 0xff;
  int last_ccc_att_status = 0;
  polar_sdk_pmd_start_result_t start_result =
      polar_sdk_pmd_start_ecg_with_policy(&policy, &ops, &start_response_status,
                                          &last_ccc_att_status);
...
static void polar_h10_stop_underlying_ecg_or_raise(polar_h10_obj_t *self) {
  ...
  polar_pmd_expect_response(self, POLAR_SDK_PMD_OP_STOP_MEASUREMENT,
                            POLAR_SDK_PMD_MEAS_ECG);
  ...
  if (polar_pmd_wait_response(self, POLAR_SDK_GATT_OP_TIMEOUT_MS)) {
    if (!polar_sdk_pmd_response_status_ok(self->pmd_cp_response_status)) {
      polar_raise_exc(&polar_type_ProtocolError,
                      MP_ERROR_TEXT("PMD stop ECG rejected"));
    }
  }
}

static void
polar_h10_start_hidden_ecg_companion_or_raise(polar_h10_obj_t *self) {
  if (self == NULL || self->ecg_enabled || self->ecg_companion_active) {
    return;
  }

  polar_sdk_pmd_start_result_t start_result =
      polar_h10_start_ecg_raw(self, 130u, NULL, NULL);
...
static mp_obj_t polar_h10_start_ecg(size_t n_args, const mp_obj_t *args,
                                    mp_map_t *kw_args) {
  enum {
    ARG_sample_rate,
  };
  ...
  if (!POLAR_CFG_ENABLE_ECG) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("ECG feature disabled at build time"));
  }
  polar_require_no_active_download_handle(self);
  if (!self->runtime_link.connected ||
      self->runtime_link.state != POLAR_SDK_STATE_READY ||
  ...
  if (!self->pmd_cp_char_found || !self->pmd_data_char_found ||
      self->pmd_cp_handle == 0 || self->pmd_data_handle == 0) {
    polar_raise_exc(&polar_type_ProtocolError,
                    MP_ERROR_TEXT("PMD characteristics missing"));
  }
  ...
  if (self->ecg_enabled) {
    if (self->ecg_stream_format == POLAR_STREAM_FORMAT_DECODED &&
        self->ecg_sample_rate_hz == (uint16_t)sample_rate) {
      return mp_const_none;
    }
    polar_raise_exc(
        &polar_type_Error,
        MP_ERROR_TEXT("ECG stream already active with different config"));
  }
  ...
  if (self->ecg_companion_active) {
    ...
    if (self->imu_enabled) {
      polar_raise_exc(
          &polar_type_Error,
          MP_ERROR_TEXT("cannot change ECG config while ACC stream is active"));
    }
  ...
  polar_sdk_pmd_start_result_t start_result =
      polar_h10_start_ecg_raw(self, (uint16_t)sample_rate, NULL, NULL);
  polar_raise_for_pmd_start_result(start_result, true);
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_start_ecg_obj, 1,
                                  polar_h10_start_ecg);
...
static void polar_pmd_disable_notifications_best_effort(polar_h10_obj_t *self) {
  ...
  if (nlr_push(&nlr) == 0) {
    polar_pmd_set_notify_for_char(self, &self->pmd_cp_char,
                                  &self->pmd_cp_notification,
                                  &self->pmd_cp_notification_listening, false);
    nlr_pop();
  } else {
...
static mp_obj_t polar_h10_read_ecg(size_t n_args, const mp_obj_t *args,
                                   mp_map_t *kw_args) {
  enum {
    ARG_max_bytes,
    ARG_timeout_ms,
  ...
  if (!self->ecg_enabled) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("ECG stream is not active"));
  }
  ...
  if (polar_ecg_ring_available(self) == 0 && timeout_ms > 0 &&
      self->ecg_enabled) {
    uint32_t start_ms = polar_now_ms();
    while (!polar_elapsed_ms(start_ms, (uint32_t)timeout_ms)) {
      if (!self->runtime_link.connected ||
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_read_ecg_obj, 1,
                                  polar_h10_read_ecg);

static mp_obj_t polar_h10_start_imu(size_t n_args, const mp_obj_t *args,
                                    mp_map_t *kw_args) {
  enum {
    ARG_sample_rate,
    ARG_range,
  ...
  static const mp_arg_t allowed_args[] = {
      ...
      {MP_QSTR_range,
       ...
       {.u_int = POLAR_SDK_IMU_DEFAULT_RANGE_G}},
  };
  ...
  if (!POLAR_CFG_ENABLE_ECG) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("ACC feature disabled at build time"));
  }
  polar_require_no_active_download_handle(self);
  if (!self->runtime_link.connected ||
      self->runtime_link.state != POLAR_SDK_STATE_READY ||
  ...
  if (self->imu_enabled) {
    ...
    polar_raise_exc(
        &polar_type_Error,
        MP_ERROR_TEXT("ACC stream already active with different config"));
  }
  ...
  int last_ccc_att_status = 0;
  polar_sdk_pmd_start_result_t start_result =
      polar_sdk_pmd_start_acc_with_policy(&policy, &ops, &start_response_status,
                                          &last_ccc_att_status);
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_start_imu_obj, 1,
                                  polar_h10_start_imu);
...
static mp_obj_t polar_h10_stop_imu(mp_obj_t self_in) {
  ...
  if (self->imu_enabled) {
    ...
    if (polar_pmd_wait_response(self, POLAR_SDK_GATT_OP_TIMEOUT_MS)) {
      if (!polar_sdk_pmd_response_status_ok(self->pmd_cp_response_status)) {
        polar_raise_exc(&polar_type_ProtocolError,
                        MP_ERROR_TEXT("PMD stop ACC rejected"));
      }
    }
  }
...
static mp_obj_t polar_h10_read_imu(size_t n_args, const mp_obj_t *args,
                                   mp_map_t *kw_args) {
  enum {
    ARG_max_bytes,
    ARG_timeout_ms,
  ...
  if (!self->imu_enabled) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("ACC stream is not active"));
  }
  ...
  if (polar_imu_ring_available(self) == 0 && timeout_ms > 0 &&
      self->imu_enabled) {
    uint32_t start_ms = polar_now_ms();
    ...
    while (!polar_elapsed_ms(start_ms, (uint32_t)timeout_ms)) {
      ...
      if (!companion_checked && !self->ecg_enabled &&
          !self->ecg_companion_active && self->acc_start_ms != 0 &&
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_read_imu_obj, 1,
                                  polar_h10_read_imu);
...
static mp_obj_t polar_h10_list_dir(mp_obj_t self_in, mp_obj_t path_in) {
  ...
  if (!POLAR_CFG_ENABLE_PSFTP) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("PSFTP feature disabled at build time"));
  }
  polar_require_ready(self);
  if (self->download_handle_active) {
    polar_raise_exc(&polar_type_Error,
  ...
  polar_sdk_psftp_dir_entry_t *entries =
      m_new(polar_sdk_psftp_dir_entry_t, POLAR_SDK_PSFTP_MAX_DIR_ENTRIES);
  size_t entry_count = 0;
  polar_psftp_fetch_directory_entries_or_raise(self, normalized_path, entries,
                                               POLAR_SDK_PSFTP_MAX_DIR_ENTRIES,
                                               &entry_count);
  ...
  for (size_t i = 0; i < entry_count; ++i) {
    ...
    mp_obj_dict_store(entry, mp_obj_new_str("name", 4),
                      mp_obj_new_str(entries[i].name, name_len));
    mp_obj_dict_store(entry, mp_obj_new_str("size", 4),
                      mp_obj_new_int_from_ull(entries[i].size));
    mp_obj_dict_store(entry, mp_obj_new_str("is_dir", 6),
...
static mp_obj_t polar_h10_download(size_t n_args, const mp_obj_t *args,
                                   mp_map_t *kw_args) {
  enum {
    ARG_path,
    ARG_max_bytes,
  ...
  static const mp_arg_t allowed_args[] = {
      ...
      {MP_QSTR_max_bytes,
       MP_ARG_KW_ONLY | MP_ARG_INT,
       {.u_int = POLAR_SDK_PSFTP_DEFAULT_DOWNLOAD_MAX_BYTES}},
      {MP_QSTR_timeout_ms,
       MP_ARG_KW_ONLY | MP_ARG_INT,
       {.u_int = POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS}},
  };
  ...
  size_t path_len = 0;
  const char *path =
      mp_obj_str_get_data(parsed_args[ARG_path].u_obj, &path_len);
  if (path_len == 0 || path[0] != '/' ||
      path_len > POLAR_SDK_PSFTP_MAX_PATH_BYTES) {
  ...
  polar_sdk_psftp_trans_result_t r =
      polar_psftp_execute_get(self, path, path_len, response, (size_t)max_bytes,
                              (uint32_t)timeout_ms, &response_len);
  if (r != POLAR_SDK_PSFTP_TRANS_OK) {
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_download_obj, 2,
                                  polar_h10_download);

static mp_obj_t polar_h10_download_open(size_t n_args, const mp_obj_t *args,
                                        mp_map_t *kw_args) {
  enum {
    ARG_path,
    ARG_timeout_ms,
  ...
  polar_sdk_psftp_trans_result_t r =
      polar_psftp_begin_get_stream(self, path, path_len);
  if (r != POLAR_SDK_PSFTP_TRANS_OK) {
    polar_download_blob_reset(self);
    polar_raise_for_psftp_result(self, r);
  }
  ...
  if (timeout_ms > 0) {
    uint32_t start_ms = polar_now_ms();
    while (!polar_elapsed_ms(start_ms, (uint32_t)timeout_ms)) {
      if (polar_download_blob_available(self) > 0 ||
          self->psftp_response_done) {
        break;
      }
      if (!self->runtime_link.connected ||
          self->runtime_link.state != POLAR_SDK_STATE_READY ||
    ...
    if (self->psftp_response_done && polar_download_blob_available(self) == 0 &&
        self->psftp_response_result != POLAR_SDK_PSFTP_RX_COMPLETE) {
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_download_open_obj, 2,
                                  polar_h10_download_open);

static mp_obj_t polar_h10_download_read(size_t n_args, const mp_obj_t *args,
                                        mp_map_t *kw_args) {
  enum {
    ARG_handle,
    ARG_buf,
  ...
  if (handle <= 0 || !self->download_handle_active ||
      (uint32_t)handle != self->download_handle_id) {
  ...
  if (available == 0) {
    ...
    polar_raise_exc(&polar_type_TimeoutError,
                    MP_ERROR_TEXT("download read timeout"));
  }
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_download_read_obj, 3,
                                  polar_h10_download_read);
...
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_download_close_obj,
                                 polar_h10_download_close);

static mp_obj_t polar_h10_recording_start(size_t n_args, const mp_obj_t *args,
                                          mp_map_t *kw_args) {
  enum {
    ARG_kind,
    ARG_sample_type,
  ...
  polar_recording_kind_key_t kind =
      polar_parse_recording_kind_key(parsed_args[ARG_kind].u_obj);
  ...
  if (!polar_recording_kind_is_supported_build(kind)) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("recording kind unsupported"));
  }
  if (!polar_h10_recording_is_supported(self)) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("recording control unsupported"));
  }
  ...
  polar_sdk_h10_recording_sample_type_t sample_type =
      parsed_args[ARG_sample_type].u_obj == mp_const_none
          ...
          : polar_parse_h10_recording_sample_type(
                parsed_args[ARG_sample_type].u_obj);
  ...
  if (sample_type == POLAR_SDK_H10_RECORDING_SAMPLE_RR_INTERVAL &&
      interval_value != 1) {
    mp_raise_ValueError(
        MP_ERROR_TEXT("interval_s is unsupported for RR recording"));
  }
  ...
  if (self->recording_hr_active) {
    if (!self->recording_hr_config_known) {
      polar_raise_exc(
          &polar_type_Error,
          MP_ERROR_TEXT("recording already active with unknown config"));
    }
    if (!polar_h10_recording_local_config_matches(self, sample_type,
                                                  interval_s)) {
      polar_raise_exc(
          &polar_type_Error,
          MP_ERROR_TEXT(
              "recording already active with different or unknown config"));
    }
    return mp_const_none;
  }

  char existing_recording_id[POLAR_SDK_H10_RECORDING_ID_MAX_BYTES + 1u];
  if (polar_h10_recording_find_any_stored_id_or_raise(
          self, existing_recording_id, sizeof(existing_recording_id))) {
    polar_raise_exc(
        &polar_type_Error,
        MP_ERROR_TEXT("delete existing recording before starting a new one"));
  }

  char recording_id[POLAR_SDK_H10_RECORDING_ID_MAX_BYTES + 1u];
  polar_h10_recording_make_unique_auto_id_or_raise(self, recording_id,
                                                   sizeof(recording_id));
  ...
  polar_sdk_h10_recording_interval_t interval =
      interval_s == 5u ? POLAR_SDK_H10_RECORDING_INTERVAL_5S
                       : POLAR_SDK_H10_RECORDING_INTERVAL_1S;
  if (!polar_sdk_psftp_encode_h10_start_recording_params(
          recording_id, strlen(recording_id), sample_type, interval,
          query_payload, sizeof(query_payload), &query_payload_len)) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("failed to encode recording start"));
  }

  uint8_t response[32];
  size_t response_len = 0;
  polar_sdk_psftp_trans_result_t result = polar_psftp_execute_query(
      self, POLAR_SDK_PSFTP_QUERY_REQUEST_START_RECORDING, query_payload,
      query_payload_len, response, sizeof(response),
      POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS, &response_len);
  if (result != POLAR_SDK_PSFTP_TRANS_OK) {
    polar_raise_for_psftp_result(self, result);
  }
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_recording_start_obj, 2,
                                  polar_h10_recording_start);
...
static mp_obj_t polar_h10_recording_stop(mp_obj_t self_in, mp_obj_t kind_in) {
  ...
  size_t response_len = 0;
  polar_sdk_psftp_trans_result_t result = polar_psftp_execute_query(
      self, POLAR_SDK_PSFTP_QUERY_REQUEST_STOP_RECORDING, NULL, 0, response,
      sizeof(response), POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS, &response_len);
  if (result != POLAR_SDK_PSFTP_TRANS_OK) {
    polar_raise_for_psftp_result(self, result);
  }
...
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_recording_stop_obj,
                                 polar_h10_recording_stop);
...
static mp_obj_t polar_h10_recording_status(mp_obj_t self_in) {
  ...
  if (!polar_h10_recording_is_supported(self)) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("recording status unsupported"));
  }
  ...
  mp_obj_dict_store(by_kind, mp_obj_new_str("hr", 2),
                    mp_obj_new_bool(self->recording_hr_active));
  ...
  mp_obj_dict_store(out, mp_obj_new_str("active", 6),
                    mp_obj_new_bool(self->recording_hr_active));
  mp_obj_dict_store(out, mp_obj_new_str("by_kind", 7), by_kind);
  mp_obj_dict_store(out, mp_obj_new_str("active_kinds", 12), active_kinds);
  return out;
}
static MP_DEFINE_CONST_FUN_OBJ_1(polar_h10_recording_status_obj,
                                 polar_h10_recording_status);
...
static mp_obj_t polar_h10_recording_list(mp_obj_t self_in) {
  ...
  if (!polar_h10_recording_is_supported(self)) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("recording is unsupported"));
  }
  ...
  for (size_t i = 0; i < root_count; ++i) {
    ...
    bool is_active = self->recording_hr_active &&
                     strcmp(self->recording_hr_id, dir_name) == 0;
    mp_obj_t entry = mp_obj_new_dict(6);
    mp_obj_dict_store(entry, mp_obj_new_str("recording_id", 12),
                      mp_obj_new_str(dir_name, dir_name_len));
    mp_obj_dict_store(entry, mp_obj_new_str("kind", 4),
                      mp_obj_new_str("hr", 2));
    mp_obj_dict_store(
        entry, mp_obj_new_str("state", 5),
        mp_obj_new_str(is_active ? "active" : "stopped", is_active ? 6 : 7));
    mp_obj_dict_store(entry, mp_obj_new_str("bytes_total", 11),
                      mp_obj_new_int_from_ull(bytes_total));
    mp_obj_dict_store(entry, mp_obj_new_str("start_time_ns", 13),
                      mp_const_none);
    mp_obj_dict_store(entry, mp_obj_new_str("time_base", 9),
                      mp_obj_new_str("unknown", 7));
    ...
    if (sample_path_len <= POLAR_SDK_PSFTP_MAX_PATH_BYTES) {
      ...
      memcpy(sample_path + 1u + dir_name_len, samples_name,
             sizeof(samples_name));
      mp_obj_dict_store(entry, mp_obj_new_str("path", 4),
                        mp_obj_new_str(sample_path, sample_path_len));
    }
...
static MP_DEFINE_CONST_FUN_OBJ_1(polar_h10_recording_list_obj,
                                 polar_h10_recording_list);

static mp_obj_t polar_h10_recording_default_config(mp_obj_t self_in,
                                                   mp_obj_t kind_in) {
  polar_h10_obj_t *self = MP_OBJ_TO_PTR(self_in);
  polar_require_ready(self);
  polar_require_no_active_download_handle(self);
  ...
  switch (kind) {
  case POLAR_RECORDING_KIND_KEY_HR:
    mp_obj_dict_store(dict, mp_obj_new_str("sample_type", 11),
                      mp_obj_new_str("hr", 2));
    mp_obj_dict_store(dict, mp_obj_new_str("interval_s", 10),
                      mp_obj_new_int(1));
    return dict;
  default:
    polar_raise_exc(&polar_type_UnsupportedError,
...
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_recording_default_config_obj,
                                 polar_h10_recording_default_config);

static mp_obj_t polar_h10_recording_delete(mp_obj_t self_in,
                                           mp_obj_t recording_id_in) {
  polar_h10_obj_t *self = MP_OBJ_TO_PTR(self_in);
  polar_require_ready(self);
  polar_require_no_active_download_handle(self);
  ...
  size_t recording_id_len = 0;
  const char *recording_id =
      mp_obj_str_get_data(recording_id_in, &recording_id_len);
  if (recording_id_len == 0u ||
      recording_id_len > POLAR_SDK_H10_RECORDING_ID_MAX_BYTES) {
  ...
  if (self->recording_hr_active &&
      ...
      memcmp(self->recording_hr_id, recording_id, recording_id_len) == 0) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("cannot delete active recording"));
  }
  ...
  if (sample_path_len > POLAR_SDK_PSFTP_MAX_PATH_BYTES) {
    polar_raise_exc(&polar_type_Error,
                    MP_ERROR_TEXT("recording path too long"));
  }
  sample_path[0] = '/';
  memcpy(sample_path + 1u, recording_id, recording_id_len);
  memcpy(sample_path + 1u + recording_id_len, samples_name,
         sizeof(samples_name));

  polar_sdk_psftp_trans_result_t r = polar_psftp_execute_remove(
      self, sample_path, sample_path_len, POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS);
  if (r != POLAR_SDK_PSFTP_TRANS_OK) {
    polar_raise_for_psftp_result(self, r);
  }
  return mp_const_none;
}
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_recording_delete_obj,
                                 polar_h10_recording_delete);
...
static mp_obj_t polar_h10_delete(mp_obj_t self_in, mp_obj_t path_in) {
  ...
  if (!POLAR_CFG_ENABLE_PSFTP) {
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("filesystem delete is unsupported"));
  }

  polar_sdk_psftp_trans_result_t r = polar_psftp_execute_remove(
      self, path, path_len, POLAR_SDK_PSFTP_DEFAULT_TIMEOUT_MS);
  if (r != POLAR_SDK_PSFTP_TRANS_OK) {
    polar_raise_for_psftp_result(self, r);
  }
  return mp_const_none;
...
static mp_obj_t polar_call_typed_read_ecg(polar_h10_obj_t *self,
                                          mp_int_t max_bytes,
                                          mp_int_t timeout_ms) {
...
static mp_obj_t polar_call_typed_read_acc(polar_h10_obj_t *self,
                                          mp_int_t max_bytes,
                                          mp_int_t timeout_ms) {
  mp_obj_t argv[1] = {MP_OBJ_FROM_PTR(self)};
  mp_map_elem_t kw_table[2] = {
...
static mp_obj_t polar_h10_stream_default_config(mp_obj_t self_in,
                                                mp_obj_t kind_in) {
  polar_h10_obj_t *self = MP_OBJ_TO_PTR(self_in);
  polar_require_ready(self);
  polar_require_no_active_download_handle(self);
  ...
  case POLAR_STREAM_KIND_KEY_HR:
    return dict;
  case POLAR_STREAM_KIND_KEY_ECG:
    mp_obj_dict_store(dict, mp_obj_new_str("sample_rate_hz", 14),
                      mp_obj_new_int(130));
    return dict;
  case POLAR_STREAM_KIND_KEY_ACC:
    mp_obj_dict_store(dict, mp_obj_new_str("sample_rate_hz", 14),
                      mp_obj_new_int(50));
    mp_obj_dict_store(dict, mp_obj_new_str("range", 5),
                      mp_obj_new_int(POLAR_SDK_IMU_DEFAULT_RANGE_G));
    return dict;
  ...
  case POLAR_STREAM_KIND_KEY_MAG:
    polar_raise_exc(&polar_type_UnsupportedError,
                    MP_ERROR_TEXT("stream kind unsupported"));
    break;
...
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_stream_default_config_obj,
                                 polar_h10_stream_default_config);

static mp_obj_t polar_h10_start_stream(size_t n_args, const mp_obj_t *args,
                                       mp_map_t *kw_args) {
  enum {
    ARG_kind,
    ARG_format,
  ...
  polar_stream_kind_key_t kind =
      polar_parse_stream_kind_key(parsed_args[ARG_kind].u_obj);
  ...
  case POLAR_STREAM_KIND_KEY_HR:
    if (parsed_args[ARG_sample_rate_hz].u_obj != mp_const_none ||
        parsed_args[ARG_range].u_obj != mp_const_none) {
  ...
  case POLAR_STREAM_KIND_KEY_ECG: {
    if (requested_format != POLAR_STREAM_FORMAT_DECODED) {
      polar_raise_exc(&polar_type_UnsupportedError,
                      MP_ERROR_TEXT("ECG raw stream format not implemented"));
    }
  ...
  case POLAR_STREAM_KIND_KEY_ACC: {
    if (requested_format != POLAR_STREAM_FORMAT_DECODED) {
      polar_raise_exc(&polar_type_UnsupportedError,
                      MP_ERROR_TEXT("ACC raw stream format not implemented"));
    }
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_start_stream_obj, 1,
                                  polar_h10_start_stream);
...
static MP_DEFINE_CONST_FUN_OBJ_2(polar_h10_stop_stream_obj,
                                 polar_h10_stop_stream);

static mp_obj_t polar_h10_read_stream_core(polar_h10_obj_t *self,
                                           polar_stream_kind_key_t kind,
                                           mp_int_t max_bytes,
                                           mp_int_t timeout_ms) {
  if (max_bytes <= 0) {
  ...
  if (kind == POLAR_STREAM_KIND_KEY_HR) {
    ...
    if (!self->hr_enabled) {
      polar_raise_exc(&polar_type_Error,
                      MP_ERROR_TEXT("HR stream is not active"));
    }
    ...
    if (self->hr_stream_format == POLAR_STREAM_FORMAT_DECODED) {
      ...
      if ((size_t)max_bytes < frame_len) {
        polar_raise_exc(
            ...
            MP_ERROR_TEXT("buffer too small for decoded HR sample"));
      }
    ...
    if ((size_t)max_bytes < framed_len) {
      polar_raise_exc(&polar_type_BufferOverflowError,
                      MP_ERROR_TEXT("buffer too small for raw HR sample"));
    }
  ...
  if (kind == POLAR_STREAM_KIND_KEY_ECG || kind == POLAR_STREAM_KIND_KEY_ACC) {
    ...
    polar_stream_format_t selected =
        is_ecg ? self->ecg_stream_format : self->acc_stream_format;
    const uint32_t sample_size = is_ecg ? 4u : 6u;
    if (selected == POLAR_STREAM_FORMAT_RAW) {
      polar_raise_exc(
          &polar_type_UnsupportedError,
          MP_ERROR_TEXT("raw stream format not implemented for this kind"));
    }
    ...
    if ((size_t)max_bytes < header_len + sample_size) {
      polar_raise_exc(
          &polar_type_BufferOverflowError,
          MP_ERROR_TEXT("buffer too small for decoded stream sample"));
    }

    size_t payload_budget = (size_t)max_bytes - header_len;
    payload_budget -= (payload_budget % sample_size);

    mp_obj_t payload_obj =
        is_ecg ? polar_call_typed_read_ecg(self, (mp_int_t)payload_budget,
                                           timeout_ms)
               : polar_call_typed_read_acc(self, (mp_int_t)payload_budget,
                                           timeout_ms);

    size_t payload_len = 0;
    const uint8_t *payload =
        (const uint8_t *)mp_obj_str_get_data(payload_obj, &payload_len);
    if (payload_len == 0) {
      return payload_obj;
    }

    return polar_build_decoded_chunk_v1(is_ecg ? 1 : 2, is_ecg ? 2 : 3, 0, 0, 0,
                                        0, sample_size, payload, payload_len);
  }
...
static mp_obj_t polar_h10_read_stream(size_t n_args, const mp_obj_t *args,
                                      mp_map_t *kw_args) {
  enum {
    ARG_kind,
    ARG_max_bytes,
  ...
  return polar_h10_read_stream_core(self, kind,
                                    parsed_args[ARG_max_bytes].u_int,
                                    parsed_args[ARG_timeout_ms].u_int);
}
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_read_stream_obj, 1,
                                  polar_h10_read_stream);

static mp_obj_t polar_h10_read_stream_into(size_t n_args, const mp_obj_t *args,
                                           mp_map_t *kw_args) {
  enum {
    ARG_kind,
    ARG_buf,
  ...
  mp_obj_t chunk = polar_h10_read_stream_core(
      self, kind, (mp_int_t)bufinfo.len, parsed_args[ARG_timeout_ms].u_int);
...
static MP_DEFINE_CONST_FUN_OBJ_KW(polar_h10_read_stream_into_obj, 1,
                                  polar_h10_read_stream_into);
...
static const mp_rom_map_elem_t polar_h10_locals_dict_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_is_connected),
     MP_ROM_PTR(&polar_h10_is_connected_obj)},
    {MP_ROM_QSTR(MP_QSTR_required_capabilities),
     MP_ROM_PTR(&polar_h10_required_capabilities_obj)},
    {MP_ROM_QSTR(MP_QSTR_set_required_capabilities),
     MP_ROM_PTR(&polar_h10_set_required_capabilities_obj)},
    {MP_ROM_QSTR(MP_QSTR_capabilities),
     MP_ROM_PTR(&polar_h10_capabilities_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_stream_default_config),
     MP_ROM_PTR(&polar_h10_stream_default_config_obj)},
    {MP_ROM_QSTR(MP_QSTR_start_stream),
     MP_ROM_PTR(&polar_h10_start_stream_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_read_stream_into),
     MP_ROM_PTR(&polar_h10_read_stream_into_obj)},
    {MP_ROM_QSTR(MP_QSTR_recording_start),
     MP_ROM_PTR(&polar_h10_recording_start_obj)},
    {MP_ROM_QSTR(MP_QSTR_recording_stop),
     MP_ROM_PTR(&polar_h10_recording_stop_obj)},
    {MP_ROM_QSTR(MP_QSTR_recording_status),
     MP_ROM_PTR(&polar_h10_recording_status_obj)},
    {MP_ROM_QSTR(MP_QSTR_recording_list),
     MP_ROM_PTR(&polar_h10_recording_list_obj)},
    {MP_ROM_QSTR(MP_QSTR_recording_default_config),
     MP_ROM_PTR(&polar_h10_recording_default_config_obj)},
    {MP_ROM_QSTR(MP_QSTR_recording_delete),
     MP_ROM_PTR(&polar_h10_recording_delete_obj)},
    ...
    {MP_ROM_QSTR(MP_QSTR_download_open),
     MP_ROM_PTR(&polar_h10_download_open_obj)},
    {MP_ROM_QSTR(MP_QSTR_download_read),
     MP_ROM_PTR(&polar_h10_download_read_obj)},
    {MP_ROM_QSTR(MP_QSTR_download_close),
     MP_ROM_PTR(&polar_h10_download_close_obj)},
};
static MP_DEFINE_CONST_DICT(polar_h10_locals_dict, polar_h10_locals_dict_table);

MP_DEFINE_CONST_OBJ_TYPE(polar_h10_type, MP_QSTR_Device, MP_TYPE_FLAG_NONE,
                         make_new, polar_h10_make_new, locals_dict,
                         &polar_h10_locals_dict);

// -----------------------------------------------------------------------------
// Module-level helpers

static mp_obj_t polar_version(void) { return mp_obj_new_str("0.1.0-dev", 9); }
...
static mp_obj_t polar_build_info(void) {
  ...
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_version),
                    mp_obj_new_str("0.1.0-dev", 9));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_git_sha),
      mp_obj_new_str(POLAR_BUILD_GIT_SHA, strlen(POLAR_BUILD_GIT_SHA)));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_git_dirty),
      mp_obj_new_str(POLAR_BUILD_GIT_DIRTY, strlen(POLAR_BUILD_GIT_DIRTY)));
  mp_obj_dict_store(
      dict, MP_OBJ_NEW_QSTR(MP_QSTR_preset),
      mp_obj_new_str(POLAR_BUILD_PRESET, strlen(POLAR_BUILD_PRESET)));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_build_type),
                    mp_obj_new_str(POLAR_BUILD_TYPE, strlen(POLAR_BUILD_TYPE)));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_has_btstack),
                    mp_obj_new_int(POLAR_SDK_HAS_BTSTACK));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_hr),
                    mp_obj_new_int(POLAR_CFG_ENABLE_HR));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_ecg),
                    mp_obj_new_int(POLAR_CFG_ENABLE_ECG));
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_psftp),
                    mp_obj_new_int(POLAR_CFG_ENABLE_PSFTP));
#if POLAR_SDK_HAS_BTSTACK
  mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_btstack_config),
                    polar_btstack_config_snapshot_dict());
#endif
...
static const mp_rom_map_elem_t polar_sdk_module_globals_table[] = {
    ...
    {MP_ROM_QSTR(MP_QSTR_NotConnectedError),
     MP_ROM_PTR(&polar_type_NotConnectedError)},
    {MP_ROM_QSTR(MP_QSTR_ProtocolError), MP_ROM_PTR(&polar_type_ProtocolError)},
    {MP_ROM_QSTR(MP_QSTR_BufferOverflowError),
     MP_ROM_PTR(&polar_type_BufferOverflowError)},
    {MP_ROM_QSTR(MP_QSTR_SecurityError), MP_ROM_PTR(&polar_type_SecurityError)},
    {MP_ROM_QSTR(MP_QSTR_UnsupportedError),
     MP_ROM_PTR(&polar_type_UnsupportedError)},
    ...
    {MP_ROM_QSTR(MP_QSTR_CAP_STREAM_ECG),
     MP_ROM_PTR(&polar_cap_stream_ecg_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_STREAM_ACC),
     MP_ROM_PTR(&polar_cap_stream_acc_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_STREAM_PPG),
     MP_ROM_PTR(&polar_cap_stream_ppg_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_STREAM_PPI),
     MP_ROM_PTR(&polar_cap_stream_ppi_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_STREAM_GYRO),
     MP_ROM_PTR(&polar_cap_stream_gyro_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_STREAM_MAG),
     MP_ROM_PTR(&polar_cap_stream_mag_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_RECORDING), MP_ROM_PTR(&polar_cap_recording_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_PSFTP_READ),
     MP_ROM_PTR(&polar_cap_psftp_read_obj)},
    {MP_ROM_QSTR(MP_QSTR_CAP_PSFTP_DELETE),
     MP_ROM_PTR(&polar_cap_psftp_delete_obj)},
};
    ...
    .map =
        {
            ...
            .table = (mp_map_elem_t *)(mp_rom_map_elem_t *)
                polar_sdk_module_globals_table,
        },
};
