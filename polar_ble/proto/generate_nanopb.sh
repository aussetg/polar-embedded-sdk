...
usage() {
    cat <<'EOF'
Usage:
  polar_ble/proto/generate_nanopb.sh <polar_proto_dir> [output_dir]
...
for opt in pftp_request.options pftp_response.options structures.options; do
    if [[ -f "${repo_root}/polar_ble/proto/options/${opt}" ]]; then
        cp "${repo_root}/polar_ble/proto/options/${opt}" "${tmp_dir}/proto/${opt}"
    fi
done
