...
// MicroPython user C module: polar_ble
//
...
#include "polar_ble_driver_discovery_runtime.h"
#include "polar_ble_driver_discovery_btstack_runtime.h"
...
#include "polar_ble_driver_wait.h"
...
#if POLAR_BLE_CFG_ENABLE_PSFTP
#include "pftp_error.pb.h"
#endif

#if MICROPY_PY_BLUETOOTH && MICROPY_BLUETOOTH_BTSTACK
#define POLAR_BLE_HAS_BTSTACK (1)
#include "extmod/modbluetooth.h"
#include "btstack.h"
#else
#define POLAR_BLE_HAS_BTSTACK (0)
#endif

#ifndef POLAR_BLE_CFG_ENABLE_HR
#ifdef POLAR_CFG_ENABLE_HR
#define POLAR_BLE_CFG_ENABLE_HR (POLAR_CFG_ENABLE_HR)
#else
#define POLAR_BLE_CFG_ENABLE_HR (1)
#endif
#endif

#ifndef POLAR_BLE_CFG_ENABLE_ECG
#ifdef POLAR_CFG_ENABLE_ECG
#define POLAR_BLE_CFG_ENABLE_ECG (POLAR_CFG_ENABLE_ECG)
#else
#define POLAR_BLE_CFG_ENABLE_ECG (1)
#endif
#endif

#ifndef POLAR_BLE_CFG_ENABLE_PSFTP
#ifdef POLAR_CFG_ENABLE_PSFTP
#define POLAR_BLE_CFG_ENABLE_PSFTP (POLAR_CFG_ENABLE_PSFTP)
#else
#define POLAR_BLE_CFG_ENABLE_PSFTP (1)
#endif
#endif

#ifndef POLAR_BLE_BUILD_GIT_SHA
#define POLAR_BLE_BUILD_GIT_SHA "unknown"
#endif

#ifndef POLAR_BLE_BUILD_GIT_DIRTY
#define POLAR_BLE_BUILD_GIT_DIRTY "unknown"
#endif

#ifndef POLAR_BLE_BUILD_PRESET
#define POLAR_BLE_BUILD_PRESET "manual"
#endif

#ifndef POLAR_BLE_BUILD_TYPE
#define POLAR_BLE_BUILD_TYPE "unknown"
#endif

#define POLAR_BLE_SERVICE_HR (0x01)
#define POLAR_BLE_SERVICE_ECG (0x02)
#define POLAR_BLE_SERVICE_PSFTP (0x04)
#define POLAR_BLE_SERVICE_ALL (POLAR_BLE_SERVICE_HR | POLAR_BLE_SERVICE_ECG | POLAR_BLE_SERVICE_PSFTP)
#define POLAR_BLE_DEFAULT_REQUIRED_SERVICES \
    ((POLAR_BLE_CFG_ENABLE_HR ? POLAR_BLE_SERVICE_HR : 0) | \
    (POLAR_BLE_CFG_ENABLE_ECG ? POLAR_BLE_SERVICE_ECG : 0) | \
    (POLAR_BLE_CFG_ENABLE_PSFTP ? POLAR_BLE_SERVICE_PSFTP : 0))

// Conservative defaults for connect scan and retry behavior.
#define POLAR_BLE_SCAN_INTERVAL_UNITS (0x0030) // 30 ms
#define POLAR_BLE_SCAN_WINDOW_UNITS   (0x0030) // 30 ms
#define POLAR_BLE_CONNECT_ATTEMPT_SLICE_MS (3500)
#define POLAR_BLE_DISCONNECT_WAIT_MS (2500)
#define POLAR_BLE_GATT_OP_TIMEOUT_MS (2000)

#define POLAR_BLE_POST_CONN_INTERVAL_MIN (24)
#define POLAR_BLE_POST_CONN_INTERVAL_MAX (40)
#define POLAR_BLE_POST_CONN_LATENCY (0)
...
#define POLAR_BLE_PSFTP_DEFAULT_TIMEOUT_MS (12000)
#define POLAR_BLE_PSFTP_GATT_OP_TIMEOUT_MS (6000)
#define POLAR_BLE_PSFTP_RECONNECT_TIMEOUT_MS (12000)
#define POLAR_BLE_PSFTP_DEFAULT_DOWNLOAD_MAX_BYTES (8192u)
#define POLAR_BLE_PSFTP_MAX_DOWNLOAD_BYTES (32768u)
#define POLAR_BLE_PSFTP_MAX_DIR_ENTRIES (48u)
#define POLAR_BLE_PSFTP_MAX_PROTO_REQUEST_BYTES (320u)
#define POLAR_BLE_PSFTP_MAX_DIR_RESPONSE_BYTES (8192u)
...
#define POLAR_BLE_STATE_CONNECTING (POLAR_BLE_DRIVER_RUNTIME_STATE_CONNECTING)
#define POLAR_BLE_STATE_DISCOVERING (POLAR_BLE_DRIVER_RUNTIME_STATE_DISCOVERING)
#define POLAR_BLE_STATE_READY (POLAR_BLE_DRIVER_RUNTIME_STATE_READY)
#define POLAR_BLE_STATE_RECOVERING (POLAR_BLE_DRIVER_RUNTIME_STATE_RECOVERING)
...
static bool polar_service_mask_is_valid(uint32_t mask) {
    uint32_t allowed = POLAR_BLE_SERVICE_ALL;
    if (!POLAR_BLE_CFG_ENABLE_HR) {
        allowed &= ~POLAR_BLE_SERVICE_HR;
    }
    if (!POLAR_BLE_CFG_ENABLE_ECG) {
        allowed &= ~POLAR_BLE_SERVICE_ECG;
    }
    if (!POLAR_BLE_CFG_ENABLE_PSFTP) {
        allowed &= ~POLAR_BLE_SERVICE_PSFTP;
    }
    return polar_ble_driver_service_mask_is_valid(mask, allowed);
}

#if POLAR_BLE_HAS_BTSTACK

static const uint8_t POLAR_BLE_UUID_PMD_SERVICE_BE[16] = {
    0xFB, 0x00, 0x5C, 0x80, 0x02, 0xE7, 0xF3, 0x87,
    0x1C, 0xAD, 0x8A, 0xCD, 0x2D, 0x8D, 0xF0, 0xC8,
};

static const uint8_t POLAR_BLE_UUID_PMD_CP_BE[16] = {
    0xFB, 0x00, 0x5C, 0x81, 0x02, 0xE7, 0xF3, 0x87,
    0x1C, 0xAD, 0x8A, 0xCD, 0x2D, 0x8D, 0xF0, 0xC8,
};

static const uint8_t POLAR_BLE_UUID_PMD_DATA_BE[16] = {
    0xFB, 0x00, 0x5C, 0x82, 0x02, 0xE7, 0xF3, 0x87,
    0x1C, 0xAD, 0x8A, 0xCD, 0x2D, 0x8D, 0xF0, 0xC8,
};

static const uint8_t POLAR_BLE_UUID_PSFTP_MTU_BE[16] = {
    0xFB, 0x00, 0x5C, 0x51, 0x02, 0xE7, 0xF3, 0x87,
    0x1C, 0xAD, 0x8A, 0xCD, 0x2D, 0x8D, 0xF0, 0xC8,
};

static const uint8_t POLAR_BLE_UUID_PSFTP_D2H_BE[16] = {
    0xFB, 0x00, 0x5C, 0x52, 0x02, 0xE7, 0xF3, 0x87,
    0x1C, 0xAD, 0x8A, 0xCD, 0x2D, 0x8D, 0xF0, 0xC8,
};

static const uint8_t POLAR_BLE_UUID_PSFTP_H2D_BE[16] = {
...
typedef struct _polar_h10_obj_t {
    ...
    polar_ble_driver_runtime_link_t runtime_link;
    ...
    polar_ble_driver_hr_state_t hr_state;
    ...
    polar_ble_driver_psftp_rx_result_t psftp_response_result;
    polar_ble_driver_psftp_rx_state_t psftp_rx_state;
    ...
    polar_ble_driver_discovery_stage_t discovery_stage;
#endif
} polar_h10_obj_t;
...
static void polar_clear_discovery_cache(polar_h10_obj_t *self) {
    ...
    self->discovery_stage = POLAR_BLE_DRIVER_DISC_STAGE_IDLE;
    self->last_att_status = ATT_ERROR_SUCCESS;
}

static void polar_mark_attempt_failed(polar_h10_obj_t *self) {
    polar_ble_driver_runtime_mark_attempt_failed(&self->runtime_link);
}
...
static bool polar_wait_for_flag_or_disconnect(polar_h10_obj_t *self, bool *done_flag, uint32_t timeout_ms) {
    ...
    polar_ble_driver_wait_ops_t ops = {
        .ctx = &ctx,
    ...
    return polar_ble_driver_wait_until_done_or_disconnect(&ops, timeout_ms, 10);
}
...
static bool polar_hr_notify_is_connected_ready(void *ctx) {
    ...
    return h->self->runtime_link.connected &&
        h->self->runtime_link.state == POLAR_BLE_STATE_READY &&
...
static void polar_raise_for_notify_runtime_result(
    polar_ble_driver_gatt_notify_runtime_result_t r,
    ...
    } else if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_MISSING_CHAR) {
        polar_raise_exc(&polar_type_ProtocolError, missing_msg);
    } else if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_NO_NOTIFY_PROP) {
        polar_raise_exc(&polar_type_ProtocolError, no_prop_msg);
    } else if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_TIMEOUT) {
        polar_raise_exc(&polar_type_TimeoutError, timeout_msg);
    } else if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_ATT_REJECTED) {
...
static void polar_hr_rearm_best_effort(polar_h10_obj_t *self) {
    if (!self->runtime_link.connected || self->runtime_link.state != POLAR_BLE_STATE_READY || self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID || !self->hr_enabled) {
        return;
    }
...
static void polar_ecg_ring_reset(polar_h10_obj_t *self) {
    polar_ble_driver_ecg_ring_reset(&self->ecg_ring);
}

static uint16_t polar_ecg_ring_available(const polar_h10_obj_t *self) {
    return polar_ble_driver_ecg_ring_available(&self->ecg_ring);
}
...
static bool polar_pmd_ensure_mtu(polar_h10_obj_t *self, uint16_t minimum_mtu, uint32_t timeout_ms) {
    ...
    if (gatt_client_get_mtu(self->runtime_link.conn_handle, &current_mtu) == ERROR_CODE_SUCCESS) {
        self->att_mtu = current_mtu;
    }
    if (self->att_mtu >= minimum_mtu) {
        return true;
    }

    self->mtu_exchange_pending = true;
    self->mtu_exchange_done = false;
    gatt_client_send_mtu_negotiation(polar_hci_packet_handler_and_discovery, self->runtime_link.conn_handle);

    if (!polar_wait_for_mtu_exchange(self, timeout_ms)) {
    ...
    self->mtu_exchange_pending = false;
    return self->att_mtu >= minimum_mtu;
}
...
static bool polar_pmd_notify_is_connected_ready(void *ctx) {
    ...
    return p->self->runtime_link.connected &&
        p->self->runtime_link.state == POLAR_BLE_STATE_READY &&
...
static void polar_raise_for_pmd_notify_status(int status, bool missing_char) {
    ...
    if (status == POLAR_BLE_DRIVER_PMD_OP_NOT_CONNECTED) {
        polar_raise_exc(&polar_type_NotConnectedError, MP_ERROR_TEXT("not connected"));
    } else if (status == POLAR_BLE_DRIVER_PMD_OP_TIMEOUT) {
...
static int polar_pmd_write_command_result(polar_h10_obj_t *self, const uint8_t *cmd, uint16_t cmd_len) {
    ...
    polar_ble_driver_gatt_write_ops_t ops = {
        .ctx = &ctx,
    ...
    polar_ble_driver_gatt_write_result_t r = polar_ble_driver_gatt_write_with_wait(
        &ops,
        cmd,
        cmd_len,
        ATT_ERROR_SUCCESS,
    ...
    if (r == POLAR_BLE_DRIVER_GATT_WRITE_NOT_CONNECTED) {
        return POLAR_BLE_DRIVER_PMD_OP_NOT_CONNECTED;
    }
    if (r == POLAR_BLE_DRIVER_GATT_WRITE_TIMEOUT) {
        self->pmd_write_pending = false;
        self->pmd_write_done = false;
        return POLAR_BLE_DRIVER_PMD_OP_TIMEOUT;
    }
    if (r == POLAR_BLE_DRIVER_GATT_WRITE_ATT_REJECTED) {
        return self->pmd_write_att_status;
    }
    if (r == POLAR_BLE_DRIVER_GATT_WRITE_FAILED) {
...
static void polar_pmd_write_command(polar_h10_obj_t *self, const uint8_t *cmd, uint16_t cmd_len) {
    ...
    if (r == POLAR_BLE_DRIVER_PMD_OP_OK) {
        return;
    }
    if (r == POLAR_BLE_DRIVER_PMD_OP_NOT_CONNECTED) {
        polar_raise_exc(&polar_type_NotConnectedError, MP_ERROR_TEXT("not connected"));
    } else if (r == POLAR_BLE_DRIVER_PMD_OP_TIMEOUT) {
...
static void polar_pmd_parse_cp_notification(polar_h10_obj_t *self, const uint8_t *value, uint16_t value_len) {
    ...
    if (!polar_ble_driver_pmd_parse_cp_response(value, value_len, &response)) {
        return;
    }
...
typedef enum {
    POLAR_PSFTP_TRANS_OK = 0,
    POLAR_PSFTP_TRANS_NOT_CONNECTED,
    POLAR_PSFTP_TRANS_MISSING_CHARACTERISTICS,
    POLAR_PSFTP_TRANS_NOTIFY_FAILED,
    POLAR_PSFTP_TRANS_NOTIFY_TIMEOUT,
    POLAR_PSFTP_TRANS_NOTIFY_ATT_REJECTED,
    POLAR_PSFTP_TRANS_ENCODE_FAILED,
    POLAR_PSFTP_TRANS_WRITE_FAILED,
    POLAR_PSFTP_TRANS_WRITE_TIMEOUT,
    POLAR_PSFTP_TRANS_WRITE_ATT_REJECTED,
    POLAR_PSFTP_TRANS_RESPONSE_TIMEOUT,
    POLAR_PSFTP_TRANS_PROTOCOL_ERROR,
    POLAR_PSFTP_TRANS_SEQUENCE_ERROR,
    POLAR_PSFTP_TRANS_OVERFLOW,
    POLAR_PSFTP_TRANS_REMOTE_ERROR,
} polar_psftp_trans_result_t;

enum {
    POLAR_PSFTP_OP_OK = 0,
    POLAR_PSFTP_OP_NOT_CONNECTED = -1,
    POLAR_PSFTP_OP_TIMEOUT = -2,
    POLAR_PSFTP_OP_TRANSPORT = -3,
    POLAR_PSFTP_OP_MISSING_CHAR = -4,
    POLAR_PSFTP_OP_NO_NOTIFY_PROP = -5,
};
...
static bool polar_psftp_security_ready(polar_h10_obj_t *self) {
    if (!self->runtime_link.connected ||
        self->runtime_link.state != POLAR_BLE_STATE_READY ||
...
static int polar_psftp_set_notify_for_char_result(
    ...
    polar_ble_driver_gatt_notify_runtime_args_t args = {
        ...
        .ccc_indicate = GATT_CLIENT_CHARACTERISTICS_CONFIGURATION_INDICATION,
        .att_success = ATT_ERROR_SUCCESS,
        .timeout_ms = POLAR_BLE_PSFTP_GATT_OP_TIMEOUT_MS,
    ...
    if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_OK) {
        return POLAR_PSFTP_OP_OK;
    }
    if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_NOT_CONNECTED) {
        return POLAR_PSFTP_OP_NOT_CONNECTED;
    }
    if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_TIMEOUT) {
        return POLAR_PSFTP_OP_TIMEOUT;
    }
    ...
    if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_NO_NOTIFY_PROP) {
        return POLAR_PSFTP_OP_NO_NOTIFY_PROP;
    }
    if (r == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_ATT_REJECTED) {
        return self->psftp_cfg_att_status;
    }
    return POLAR_PSFTP_OP_TRANSPORT;
}

static int polar_psftp_enable_notify_with_security_retry(
    polar_h10_obj_t *self,
    gatt_client_characteristic_t *chr,
    gatt_client_notification_t *notification,
    bool *listening) {
    ...
    if (status > 0 && polar_ble_driver_pmd_att_status_requires_security((uint8_t)status)) {
        bool paired = polar_psftp_request_pairing_and_wait(self);
        if (!paired) {
            if (!self->runtime_link.connected || self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID) {
                return POLAR_PSFTP_OP_NOT_CONNECTED;
            }
        ...
        status = polar_psftp_set_notify_for_char_result(
            self,
            chr,
            notification,
            listening,
...
static int polar_psftp_prepare_result(polar_h10_obj_t *self) {
    ...
    if (!polar_psftp_security_ready(self)) {
        (void)polar_psftp_request_pairing_and_wait(self);
        if (!self->runtime_link.connected || self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID) {
            return POLAR_PSFTP_OP_NOT_CONNECTED;
        }
        if (!polar_psftp_security_ready(self)) {
    ...
    if (!self->psftp_mtu_enabled) {
        int status = polar_psftp_enable_notify_with_security_retry(
            self,
            &self->psftp_mtu_char,
            &self->psftp_mtu_notification,
            &self->psftp_mtu_notification_listening);
        if (status != POLAR_PSFTP_OP_OK) {
            return status;
        }
        self->psftp_mtu_enabled = true;
    }

    if (!self->psftp_d2h_enabled && self->psftp_d2h_char_found && self->psftp_d2h_handle != 0) {
        int status = polar_psftp_enable_notify_with_security_retry(
            self,
            &self->psftp_d2h_char,
        ...
        } else if (status == POLAR_PSFTP_OP_NOT_CONNECTED) {
            return status;
        }
    }
...
static int polar_psftp_write_frame_result(polar_h10_obj_t *self, const uint8_t *frame, uint16_t frame_len) {
    ...
    if (r == POLAR_BLE_DRIVER_GATT_WRITE_OK) {
        return POLAR_PSFTP_OP_OK;
    }
    if (r == POLAR_BLE_DRIVER_GATT_WRITE_NOT_CONNECTED) {
        return POLAR_PSFTP_OP_NOT_CONNECTED;
...
static polar_psftp_trans_result_t polar_psftp_execute_get(
    polar_h10_obj_t *self,
    const char *path,
    size_t path_len,
    uint8_t *response,
    ...
    int prep = POLAR_PSFTP_OP_TRANSPORT;
    bool prep_reconnected = false;
    for (;;) {
        prep = polar_psftp_prepare_result(self);
        if (prep == POLAR_PSFTP_OP_OK) {
            break;
        }

        bool security_related = false;
        if (prep > 0 && polar_ble_driver_pmd_att_status_requires_security((uint8_t)prep)) {
            security_related = true;
        } else if ((prep == POLAR_PSFTP_OP_NOT_CONNECTED || prep == POLAR_PSFTP_OP_TIMEOUT || prep == POLAR_PSFTP_OP_TRANSPORT) &&
                   !polar_psftp_security_ready(self)) {
            security_related = true;
        }

        if (!prep_reconnected && security_related) {
            prep_reconnected = true;
            if (polar_psftp_reconnect_after_security_failure(self, POLAR_BLE_PSFTP_RECONNECT_TIMEOUT_MS)) {
                continue;
            }
        }
        break;
    ...
    if (prep == POLAR_PSFTP_OP_NOT_CONNECTED) {
        return POLAR_PSFTP_TRANS_NOT_CONNECTED;
    }
    if (prep == POLAR_PSFTP_OP_MISSING_CHAR || prep == POLAR_PSFTP_OP_NO_NOTIFY_PROP) {
        return POLAR_PSFTP_TRANS_MISSING_CHARACTERISTICS;
    }
    if (prep == POLAR_PSFTP_OP_TIMEOUT) {
        return POLAR_PSFTP_TRANS_NOTIFY_TIMEOUT;
    }
    if (prep == POLAR_PSFTP_OP_TRANSPORT) {
        return POLAR_PSFTP_TRANS_NOTIFY_FAILED;
    }
    if (prep > 0) {
        self->psftp_last_att_status = (uint8_t)prep;
        return POLAR_PSFTP_TRANS_NOTIFY_ATT_REJECTED;
    }

    uint8_t proto_payload[POLAR_BLE_PSFTP_MAX_PROTO_REQUEST_BYTES];
    size_t proto_payload_len = 0;
    if (!polar_ble_driver_psftp_encode_get_operation(path, path_len, proto_payload, sizeof(proto_payload), &proto_payload_len)) {
        return POLAR_PSFTP_TRANS_ENCODE_FAILED;
    }

    uint8_t request_stream[POLAR_BLE_PSFTP_MAX_PROTO_REQUEST_BYTES + 2u];
    ...
    self->psftp_response_result = POLAR_BLE_DRIVER_PSFTP_RX_MORE;
    polar_ble_driver_psftp_rx_reset(&self->psftp_rx_state, response, response_capacity);
    ...
    while (polar_ble_driver_psftp_tx_has_more(&tx)) {
        ...
        if (write_status == POLAR_PSFTP_OP_NOT_CONNECTED) {
            self->psftp_response_waiting = false;
            return POLAR_PSFTP_TRANS_NOT_CONNECTED;
        }
        if (write_status == POLAR_PSFTP_OP_TIMEOUT) {
            self->psftp_response_waiting = false;
            return POLAR_PSFTP_TRANS_WRITE_TIMEOUT;
        }
        if (write_status == POLAR_PSFTP_OP_TRANSPORT) {
            self->psftp_response_waiting = false;
            return POLAR_PSFTP_TRANS_WRITE_FAILED;
        }
        if (write_status > 0) {
            self->psftp_response_waiting = false;
            self->psftp_last_att_status = (uint8_t)write_status;
            return POLAR_PSFTP_TRANS_WRITE_ATT_REJECTED;
        }

        self->psftp_tx_frames_total += 1;
    }

    if (!polar_psftp_wait_response(self, timeout_ms)) {
        self->psftp_response_waiting = false;
        self->psftp_response_done = false;
        return POLAR_PSFTP_TRANS_RESPONSE_TIMEOUT;
    }

    self->psftp_response_waiting = false;
    self->psftp_last_response_bytes = (uint32_t)self->psftp_rx_state.length;

    if (self->psftp_response_result == POLAR_BLE_DRIVER_PSFTP_RX_COMPLETE) {
        if (out_response_len != NULL) {
            *out_response_len = self->psftp_rx_state.length;
        }
        return POLAR_PSFTP_TRANS_OK;
    }
    if (self->psftp_response_result == POLAR_BLE_DRIVER_PSFTP_RX_ERROR_FRAME) {
        self->psftp_last_error_code = self->psftp_rx_state.error_code;
#if POLAR_BLE_CFG_ENABLE_PSFTP
        if (self->psftp_last_error_code == protocol_PbPFtpError_OPERATION_SUCCEEDED) {
            if (out_response_len != NULL) {
                *out_response_len = self->psftp_rx_state.length;
            }
            return POLAR_PSFTP_TRANS_OK;
...
#endif
        return POLAR_PSFTP_TRANS_REMOTE_ERROR;
    }
    if (self->psftp_response_result == POLAR_BLE_DRIVER_PSFTP_RX_SEQUENCE_ERROR) {
        return POLAR_PSFTP_TRANS_SEQUENCE_ERROR;
    }
    if (self->psftp_response_result == POLAR_BLE_DRIVER_PSFTP_RX_OVERFLOW) {
        return POLAR_PSFTP_TRANS_OVERFLOW;
    }

    return POLAR_PSFTP_TRANS_PROTOCOL_ERROR;
}
...
static NORETURN void polar_raise_for_psftp_result(polar_h10_obj_t *self, polar_psftp_trans_result_t result) {
    (void)self;
    if (result == POLAR_PSFTP_TRANS_NOT_CONNECTED) {
        polar_raise_exc(&polar_type_NotConnectedError, MP_ERROR_TEXT("not connected"));
    }
    if (result == POLAR_PSFTP_TRANS_MISSING_CHARACTERISTICS) {
        polar_raise_exc(&polar_type_ProtocolError, MP_ERROR_TEXT("PSFTP characteristics missing"));
    }
    if (result == POLAR_PSFTP_TRANS_NOTIFY_TIMEOUT ||
        result == POLAR_PSFTP_TRANS_WRITE_TIMEOUT ||
        result == POLAR_PSFTP_TRANS_RESPONSE_TIMEOUT) {
        polar_raise_exc(&polar_type_TimeoutError, MP_ERROR_TEXT("PSFTP timeout"));
    }
    if (result == POLAR_PSFTP_TRANS_NOTIFY_ATT_REJECTED || result == POLAR_PSFTP_TRANS_WRITE_ATT_REJECTED) {
        polar_raise_exc(&polar_type_ProtocolError, MP_ERROR_TEXT("PSFTP ATT rejected"));
    }
    if (result == POLAR_PSFTP_TRANS_OVERFLOW) {
        polar_raise_exc(&polar_type_BufferOverflowError, MP_ERROR_TEXT("PSFTP response overflow"));
    }
    if (result == POLAR_PSFTP_TRANS_SEQUENCE_ERROR) {
        polar_raise_exc(&polar_type_ProtocolError, MP_ERROR_TEXT("PSFTP sequence error"));
    }
    if (result == POLAR_PSFTP_TRANS_PROTOCOL_ERROR) {
        polar_raise_exc(&polar_type_ProtocolError, MP_ERROR_TEXT("PSFTP protocol error"));
    }
    if (result == POLAR_PSFTP_TRANS_REMOTE_ERROR) {
...
static void polar_session_reset_feature_state(polar_h10_obj_t *self) {
    ...
    polar_ble_driver_hr_reset(&self->hr_state);
    ...
    self->psftp_last_error_code = 0;
    self->psftp_last_response_bytes = 0;
    polar_ble_driver_psftp_rx_reset(&self->psftp_rx_state, NULL, 0);
...
static void polar_link_on_connected_ready(void *ctx, const polar_ble_driver_link_event_t *event) {
    (void)event;
    ...
    uint8_t conn_update_err = gap_update_connection_parameters(
        ...
        POLAR_BLE_POST_CONN_INTERVAL_MIN,
        POLAR_BLE_POST_CONN_INTERVAL_MAX,
        POLAR_BLE_POST_CONN_LATENCY,
        POLAR_BLE_POST_CONN_SUPERVISION_TIMEOUT_10MS);
...
static void polar_link_on_disconnected(void *ctx, const polar_ble_driver_link_event_t *event) {
    (void)event;
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static bool polar_adv_runtime_is_scanning(void *ctx) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
    return self->runtime_link.state == POLAR_BLE_STATE_SCANNING;
}

static void polar_adv_runtime_on_report(void *ctx, const polar_ble_driver_btstack_adv_report_t *report) {
    (void)report;
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
    self->adv_reports_total += 1;
}

static void polar_adv_runtime_on_match(void *ctx, const polar_ble_driver_btstack_adv_report_t *report) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_hci_dispatch_on_adv_report(void *ctx, const polar_ble_driver_btstack_adv_report_t *adv_report) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_hci_dispatch_on_link_event(void *ctx, const polar_ble_driver_link_event_t *link_event) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
    ...
    (void)polar_ble_driver_runtime_context_handle_link_event(
        &self->runtime_link,
...
static void polar_hci_dispatch_on_sm_event(void *ctx, const polar_ble_driver_sm_event_t *sm_event) {
    (void)ctx;
    (void)sm_event;
}
...
static void polar_sm_on_pairing_complete(void *ctx, const polar_ble_driver_sm_event_t *event) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_sm_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    (void)polar_ble_driver_sm_control_apply(
        &sm_event,
        self->runtime_link.conn_handle,
        HCI_CON_HANDLE_INVALID,
        &ops);
...
static polar_ble_driver_discovery_snapshot_t polar_discovery_snapshot_from_obj(polar_h10_obj_t *self) {
    polar_ble_driver_discovery_snapshot_t snapshot = {
...
static void polar_discovery_runtime_set_stage(void *ctx, polar_ble_driver_discovery_stage_t stage) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
...
static void polar_discovery_advance(polar_h10_obj_t *self, uint8_t att_status) {
    polar_ble_driver_discovery_snapshot_t snapshot = polar_discovery_snapshot_from_obj(self);
    polar_ble_driver_discovery_runtime_ops_t ops = {
        .ctx = self,
    ...
    polar_ble_driver_discovery_runtime_on_query_complete(
        &snapshot,
        POLAR_BLE_SERVICE_HR,
        POLAR_BLE_SERVICE_ECG,
        POLAR_BLE_SERVICE_PSFTP,
        att_status,
...
static void polar_discovery_handle_service_result(polar_h10_obj_t *self, const gatt_client_service_t *svc) {
    polar_ble_driver_disc_service_kind_t kind = polar_ble_driver_btstack_classify_service(
        ...
        POLAR_BLE_UUID_PMD_SERVICE_BE);

    if (kind == POLAR_BLE_DRIVER_DISC_SERVICE_HR) {
        self->hr_service = *svc;
    } else if (kind == POLAR_BLE_DRIVER_DISC_SERVICE_PMD) {
        self->pmd_service = *svc;
    } else if (kind == POLAR_BLE_DRIVER_DISC_SERVICE_PSFTP) {
...
static void polar_discovery_handle_char_result(polar_h10_obj_t *self, const gatt_client_characteristic_t *chr) {
    polar_ble_driver_disc_char_kind_t kind = polar_ble_driver_btstack_classify_char(
        ...
        POLAR_BLE_UUID_PMD_CP_BE,
        POLAR_BLE_UUID_PMD_DATA_BE,
        POLAR_BLE_UUID_PSFTP_MTU_BE,
        POLAR_BLE_UUID_PSFTP_D2H_BE,
        POLAR_BLE_UUID_PSFTP_H2D_BE);

    if (kind == POLAR_BLE_DRIVER_DISC_CHAR_HR_MEAS) {
        self->hr_measurement_char = *chr;
    } else if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PMD_CP) {
        self->pmd_cp_char = *chr;
    } else if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PMD_DATA) {
        self->pmd_data_char = *chr;
    } else if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PSFTP_MTU) {
...
static bool polar_handle_gatt_route_event(polar_h10_obj_t *self, const polar_ble_driver_btstack_gatt_route_result_t *route) {
    ...
    if (route->kind == POLAR_BLE_DRIVER_GATT_ROUTE_HR_VALUE) {
    ...
    if (route->kind == POLAR_BLE_DRIVER_GATT_ROUTE_PMD_CP_VALUE) {
    ...
    if (route->kind == POLAR_BLE_DRIVER_GATT_ROUTE_PSFTP_MTU_VALUE) {
        ...
        if (self->psftp_response_waiting) {
            polar_ble_driver_psftp_rx_result_t r = polar_ble_driver_psftp_rx_feed_frame(
                &self->psftp_rx_state,
                route->value.value,
                route->value.value_len);
            self->psftp_response_result = r;

            if (r == POLAR_BLE_DRIVER_PSFTP_RX_SEQUENCE_ERROR) {
                self->psftp_rx_seq_errors_total += 1;
                self->psftp_last_error_code = POLAR_BLE_DRIVER_PSFTP_PFTP_AIR_PACKET_LOST_ERROR;
                self->psftp_response_done = true;
                self->psftp_response_waiting = false;
                return true;
            }
            if (r == POLAR_BLE_DRIVER_PSFTP_RX_PROTOCOL_ERROR) {
                self->psftp_protocol_errors_total += 1;
                self->psftp_response_done = true;
                self->psftp_response_waiting = false;
                return true;
            }
            if (r == POLAR_BLE_DRIVER_PSFTP_RX_OVERFLOW) {
                self->psftp_overflow_errors_total += 1;
                self->psftp_response_done = true;
                self->psftp_response_waiting = false;
                return true;
            }
            if (r == POLAR_BLE_DRIVER_PSFTP_RX_ERROR_FRAME || r == POLAR_BLE_DRIVER_PSFTP_RX_COMPLETE) {
                self->psftp_response_done = true;
                self->psftp_response_waiting = false;
                return true;
            }
    ...
    if (route->kind == POLAR_BLE_DRIVER_GATT_ROUTE_HR_UNMATCHED_VALUE) {
        self->hr_unmatched_value_events_total += 1;
        return true;
    }

    if (route->kind == POLAR_BLE_DRIVER_GATT_ROUTE_QUERY_COMPLETE) {
        polar_ble_driver_gatt_query_slot_t slots[] = {
            {
        ...
        if (polar_ble_driver_gatt_apply_query_complete(
        ...
        if (self->runtime_link.state == POLAR_BLE_STATE_DISCOVERING && self->runtime_link.conn_handle != HCI_CON_HANDLE_INVALID) {
...
static void polar_handle_discovery_decode_apply(polar_h10_obj_t *self, uint8_t packet_type, uint8_t *packet) {
    ...
    polar_ble_driver_discovery_btstack_result_t disc_result;
    if (!polar_ble_driver_discovery_btstack_decode_result(packet_type, packet, self->discovery_stage, &disc_result)) {
        return;
    }

    if (disc_result.kind == POLAR_BLE_DRIVER_DISCOVERY_BTSTACK_SERVICE_RESULT) {
        polar_discovery_handle_service_result(self, &disc_result.service);
    } else if (disc_result.kind == POLAR_BLE_DRIVER_DISCOVERY_BTSTACK_CHAR_RESULT) {
...
static void polar_hci_packet_handler_and_discovery(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    if (!polar_ble_driver_btstack_route_gatt_event(packet_type, packet, &route_state, &route)) {
...
static void polar_discovery_start(polar_h10_obj_t *self) {
    ...
    self->discovery_stage = POLAR_BLE_DRIVER_DISC_STAGE_SERVICES;
...
static bool polar_wait_until_ready_or_fail(polar_h10_obj_t *self, uint32_t budget_ms) {
    ...
    while (!polar_elapsed_ms(start_ms, budget_ms)) {
        if (self->runtime_link.state == POLAR_BLE_STATE_READY) {
            return true;
        }
    ...
    return self->runtime_link.state == POLAR_BLE_STATE_READY;
}
...
static void polar_cleanup_after_attempt(polar_h10_obj_t *self) {
    polar_ble_driver_runtime_context_cleanup_ops_t ops = {
        .ctx = self,
    ...
    polar_ble_driver_runtime_context_cleanup_attempt(
        &self->runtime_link,
        HCI_CON_HANDLE_INVALID,
        POLAR_BLE_DISCONNECT_WAIT_MS,
        POLAR_BLE_DRIVER_RUNTIME_STATE_RECOVERING,
        &ops);
}
...
static bool polar_transport_start_attempt(void *ctx) {
    ...
    self->last_att_status = ATT_ERROR_SUCCESS;
    self->runtime_link.state = POLAR_BLE_STATE_SCANNING;

    gap_set_scan_parameters(1, POLAR_BLE_SCAN_INTERVAL_UNITS, POLAR_BLE_SCAN_WINDOW_UNITS);
    gap_start_scan();
    return true;
}

static polar_ble_driver_transport_attempt_result_t polar_transport_wait_attempt(void *ctx, uint32_t attempt_budget_ms) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
    if (polar_wait_until_ready_or_fail(self, attempt_budget_ms)) {
        return POLAR_BLE_DRIVER_TRANSPORT_ATTEMPT_READY;
    }
    if (self->runtime_link.attempt_failed) {
        return POLAR_BLE_DRIVER_TRANSPORT_ATTEMPT_FAILED;
    }
    return POLAR_BLE_DRIVER_TRANSPORT_ATTEMPT_TIMEOUT;
}

static void polar_transport_cleanup_after_attempt(void *ctx) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
    polar_cleanup_after_attempt(self);
    self->runtime_link.state = POLAR_BLE_STATE_RECOVERING;
}
...
static polar_ble_driver_transport_connect_result_t polar_transport_connect_with_timeout(
    polar_h10_obj_t *self,
    uint32_t timeout_ms) {
    self->connect_intent = true;
    self->user_disconnect_requested = false;

    polar_ble_driver_connect_policy_t connect_policy = {
        .timeout_ms = timeout_ms,
        .attempt_slice_ms = POLAR_BLE_CONNECT_ATTEMPT_SLICE_MS,
    };
    polar_ble_driver_transport_connect_ops_t transport_ops = {
        .ctx = self,
    ...
    polar_ble_driver_transport_connect_stats_t transport_stats;

    polar_ble_driver_transport_connect_result_t connect_result = polar_ble_driver_transport_connect_blocking(
        &connect_policy,
        &transport_ops,
        &transport_stats);
...
static bool polar_psftp_reconnect_after_security_failure(polar_h10_obj_t *self, uint32_t timeout_ms) {
    ...
    self->runtime_link.conn_handle = HCI_CON_HANDLE_INVALID;
    self->runtime_link.conn_update_pending = false;
    self->runtime_link.state = POLAR_BLE_STATE_IDLE;

    if (polar_active_h10 == NULL) {
        polar_active_h10 = self;
    }

    polar_ble_driver_transport_connect_result_t connect_result = polar_transport_connect_with_timeout(self, timeout_ms);
    if (connect_result == POLAR_BLE_DRIVER_TRANSPORT_CONNECT_OK) {
        return true;
    }
...
static mp_obj_t polar_h10_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    static const mp_arg_t allowed_args[] = {
        ...
        { MP_QSTR_required_services, MP_ARG_KW_ONLY | MP_ARG_INT, { .u_int = POLAR_BLE_DEFAULT_REQUIRED_SERVICES } },
    };
    ...
    polar_ble_driver_runtime_link_init(&self->runtime_link, 0xffff);
    self->runtime_link.state = POLAR_BLE_STATE_IDLE;
    self->runtime_link.connected = false;
...
static mp_obj_t polar_h10_stats(mp_obj_t self_in) {
    ...
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_hr), mp_obj_new_bool(POLAR_BLE_CFG_ENABLE_HR));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_ecg), mp_obj_new_bool(POLAR_BLE_CFG_ENABLE_ECG));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_psftp), mp_obj_new_bool(POLAR_BLE_CFG_ENABLE_PSFTP));
...
static mp_obj_t polar_h10_connect(size_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    ...
    if (self->runtime_link.connected && self->runtime_link.state == POLAR_BLE_STATE_READY) {
        return mp_const_none;
    }
    ...
    polar_ble_driver_transport_connect_result_t connect_result = polar_transport_connect_with_timeout(
        self,
        (uint32_t)timeout_ms);
    ...
    if (connect_result == POLAR_BLE_DRIVER_TRANSPORT_CONNECT_TIMEOUT) {
        polar_raise_exc(&polar_type_TimeoutError, MP_ERROR_TEXT("connect timeout"));
    }
...
static mp_obj_t polar_h10_disconnect(mp_obj_t self_in) {
    ...
    if (self->runtime_link.state == POLAR_BLE_STATE_SCANNING) {
        gap_stop_scan();
    }
    if (self->runtime_link.state == POLAR_BLE_STATE_CONNECTING) {
    ...
    if (self->runtime_link.connected && self->runtime_link.conn_handle != HCI_CON_HANDLE_INVALID) {
        ...
        uint32_t start_ms = polar_now_ms();
        while (!polar_elapsed_ms(start_ms, POLAR_BLE_DISCONNECT_WAIT_MS)) {
            if (!self->runtime_link.connected || self->runtime_link.conn_handle == HCI_CON_HANDLE_INVALID) {
                break;
            }
            mp_event_wait_ms(10);
...
static bool polar_pmd_policy_is_connected(void *ctx) {
    polar_h10_obj_t *self = (polar_h10_obj_t *)ctx;
    return self->runtime_link.connected && self->runtime_link.state == POLAR_BLE_STATE_READY && self->runtime_link.conn_handle != HCI_CON_HANDLE_INVALID;
}
...
static int polar_pmd_policy_enable_notifications(void *ctx) {
    ...
    polar_ble_driver_pmd_notify_pair_ops_t ops = {
        .ctx = ctx,
    ...
    int r = polar_ble_driver_pmd_enable_notify_pair(
        &ops,
        POLAR_BLE_DRIVER_PMD_OP_OK,
        POLAR_BLE_DRIVER_PMD_OP_NOT_CONNECTED,
        POLAR_BLE_DRIVER_PMD_OP_TRANSPORT,
        &last_att_status);
    if (r == POLAR_BLE_DRIVER_PMD_OP_TRANSPORT && last_att_status > 0) {
...
static int polar_pmd_policy_ensure_minimum_mtu(void *ctx, uint16_t minimum_mtu) {
    ...
    if (polar_pmd_ensure_mtu(self, minimum_mtu, POLAR_BLE_GATT_OP_TIMEOUT_MS)) {
        return POLAR_BLE_DRIVER_PMD_OP_OK;
    }
    if (!polar_pmd_policy_is_connected(ctx)) {
        return POLAR_BLE_DRIVER_PMD_OP_NOT_CONNECTED;
...
static int polar_pmd_policy_start_measurement_and_wait_response(
    ...
    return polar_ble_driver_pmd_start_command_and_wait(
        &ops,
        start_cmd,
        start_cmd_len,
...
static mp_obj_t polar_h10_start_ecg(size_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    ...
    polar_ble_driver_pmd_start_result_t start_result = polar_ble_driver_pmd_start_ecg_with_policy(
        &policy,
        &ops,
...
static mp_obj_t polar_h10_stop_ecg(mp_obj_t self_in) {
    ...
    if (self->ecg_enabled) {
        ...
        polar_pmd_expect_response(self, POLAR_BLE_PMD_OP_STOP_MEASUREMENT, POLAR_BLE_PMD_MEAS_ECG);
...
static mp_obj_t polar_h10_read_ecg(size_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    ...
    if (max_bytes > POLAR_BLE_ECG_RING_BYTES) {
        max_bytes = POLAR_BLE_ECG_RING_BYTES;
    }
    max_bytes &= ~0x03;
    if (max_bytes <= 0) {
        return mp_obj_new_bytes((const uint8_t *)"", 0);
...
static mp_obj_t polar_h10_list_dir(mp_obj_t self_in, mp_obj_t path_in) {
    ...
    if (normalized_len > POLAR_BLE_DRIVER_PSFTP_MAX_PATH_BYTES) {
        polar_raise_exc(&polar_type_Error, MP_ERROR_TEXT("path too long"));
    }

    char normalized_path[POLAR_BLE_DRIVER_PSFTP_MAX_PATH_BYTES + 1u];
    ...
    uint8_t *response = m_new(uint8_t, POLAR_BLE_PSFTP_MAX_DIR_RESPONSE_BYTES);
    size_t response_len = 0;
    polar_psftp_trans_result_t r = polar_psftp_execute_get(
        self,
        normalized_path,
        normalized_len,
        response,
        POLAR_BLE_PSFTP_MAX_DIR_RESPONSE_BYTES,
        POLAR_BLE_PSFTP_DEFAULT_TIMEOUT_MS,
        &response_len);
    if (r != POLAR_PSFTP_TRANS_OK) {
        m_del(uint8_t, response, POLAR_BLE_PSFTP_MAX_DIR_RESPONSE_BYTES);
        polar_raise_for_psftp_result(self, r);
    }

    polar_ble_driver_psftp_dir_entry_t *entries = m_new(
        polar_ble_driver_psftp_dir_entry_t,
        POLAR_BLE_PSFTP_MAX_DIR_ENTRIES);
    size_t entry_count = 0;
    polar_ble_driver_psftp_dir_decode_result_t decode_result = polar_ble_driver_psftp_decode_directory(
        response,
        response_len,
        entries,
        POLAR_BLE_PSFTP_MAX_DIR_ENTRIES,
    ...
    if (decode_result == POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_TOO_MANY_ENTRIES) {
        m_del(polar_ble_driver_psftp_dir_entry_t, entries, POLAR_BLE_PSFTP_MAX_DIR_ENTRIES);
        polar_raise_exc(&polar_type_BufferOverflowError, MP_ERROR_TEXT("PSFTP directory entry overflow"));
    }
    if (decode_result != POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_OK) {
        m_del(polar_ble_driver_psftp_dir_entry_t, entries, POLAR_BLE_PSFTP_MAX_DIR_ENTRIES);
...
static mp_obj_t polar_h10_download(size_t n_args, const mp_obj_t *args, mp_map_t *kw_args) {
    ...
    static const mp_arg_t allowed_args[] = {
        ...
        { MP_QSTR_max_bytes, MP_ARG_KW_ONLY | MP_ARG_INT, { .u_int = POLAR_BLE_PSFTP_DEFAULT_DOWNLOAD_MAX_BYTES } },
        { MP_QSTR_timeout_ms, MP_ARG_KW_ONLY | MP_ARG_INT, { .u_int = POLAR_BLE_PSFTP_DEFAULT_TIMEOUT_MS } },
    };
    ...
    mp_int_t max_bytes = parsed_args[ARG_max_bytes].u_int;
    if (max_bytes <= 0 || max_bytes > POLAR_BLE_PSFTP_MAX_DOWNLOAD_BYTES) {
    ...
    if (path_len == 0 || path_len > POLAR_BLE_DRIVER_PSFTP_MAX_PATH_BYTES) {
...
static mp_obj_t polar_build_info(void) {
    ...
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_git_sha), mp_obj_new_str(POLAR_BLE_BUILD_GIT_SHA, strlen(POLAR_BLE_BUILD_GIT_SHA)));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_git_dirty), mp_obj_new_str(POLAR_BLE_BUILD_GIT_DIRTY, strlen(POLAR_BLE_BUILD_GIT_DIRTY)));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_preset), mp_obj_new_str(POLAR_BLE_BUILD_PRESET, strlen(POLAR_BLE_BUILD_PRESET)));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_build_type), mp_obj_new_str(POLAR_BLE_BUILD_TYPE, strlen(POLAR_BLE_BUILD_TYPE)));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_has_btstack), mp_obj_new_int(POLAR_BLE_HAS_BTSTACK));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_hr), mp_obj_new_int(POLAR_BLE_CFG_ENABLE_HR));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_ecg), mp_obj_new_int(POLAR_BLE_CFG_ENABLE_ECG));
    mp_obj_dict_store(dict, MP_OBJ_NEW_QSTR(MP_QSTR_feature_psftp), mp_obj_new_int(POLAR_BLE_CFG_ENABLE_PSFTP));
    return dict;
}
...
static const mp_rom_map_elem_t polar_ble_module_globals_table[] = {
    { MP_ROM_QSTR(MP_QSTR___name__), MP_ROM_QSTR(MP_QSTR_polar_ble) },
    ...
    { MP_ROM_QSTR(MP_QSTR_FEATURE_HR), MP_ROM_INT(POLAR_BLE_CFG_ENABLE_HR) },
    { MP_ROM_QSTR(MP_QSTR_FEATURE_ECG), MP_ROM_INT(POLAR_BLE_CFG_ENABLE_ECG) },
    { MP_ROM_QSTR(MP_QSTR_FEATURE_PSFTP), MP_ROM_INT(POLAR_BLE_CFG_ENABLE_PSFTP) },
    { MP_ROM_QSTR(MP_QSTR_HAS_BTSTACK), MP_ROM_INT(POLAR_BLE_HAS_BTSTACK) },

    // Runtime service mask bits
    { MP_ROM_QSTR(MP_QSTR_SERVICE_HR), MP_ROM_INT(POLAR_BLE_SERVICE_HR) },
    { MP_ROM_QSTR(MP_QSTR_SERVICE_ECG), MP_ROM_INT(POLAR_BLE_SERVICE_ECG) },
    { MP_ROM_QSTR(MP_QSTR_SERVICE_PSFTP), MP_ROM_INT(POLAR_BLE_SERVICE_PSFTP) },
    { MP_ROM_QSTR(MP_QSTR_SERVICE_ALL), MP_ROM_INT(POLAR_BLE_SERVICE_ALL) },
};
static MP_DEFINE_CONST_DICT(polar_ble_module_globals, polar_ble_module_globals_table);

const mp_obj_module_t polar_ble_user_cmodule = {
    .base = { &mp_type_module },
    .globals = (mp_obj_dict_t *)&polar_ble_module_globals,
};

MP_REGISTER_MODULE(MP_QSTR_polar_ble, polar_ble_user_cmodule);
