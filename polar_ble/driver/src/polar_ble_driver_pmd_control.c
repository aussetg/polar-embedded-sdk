...
int polar_ble_driver_pmd_map_notify_result(
    polar_ble_driver_gatt_notify_runtime_result_t result,
    uint8_t att_status,
    int op_ok,
    int op_not_connected,
    int op_timeout,
    int op_transport) {
    if (result == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_OK) {
        return op_ok;
    }
    if (result == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_NOT_CONNECTED) {
        return op_not_connected;
    }
    if (result == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_TIMEOUT) {
        return op_timeout;
    }
    if (result == POLAR_BLE_DRIVER_GATT_NOTIFY_RUNTIME_ATT_REJECTED) {
        return att_status > 0 ? (int)att_status : op_transport;
    }
    return op_transport;
}

int polar_ble_driver_pmd_enable_notify_pair(
    const polar_ble_driver_pmd_notify_pair_ops_t *ops,
    int op_ok,
    int op_not_connected,
    int op_transport,
...
int polar_ble_driver_pmd_start_command_and_wait(
    const polar_ble_driver_pmd_start_cmd_ops_t *ops,
    const uint8_t *cmd,
    size_t cmd_len,
    uint8_t opcode,
