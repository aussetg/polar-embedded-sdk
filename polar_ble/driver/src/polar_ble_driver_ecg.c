...
static void polar_ble_driver_ecg_ring_push_bytes(polar_ble_driver_ecg_ring_t *ring, const uint8_t *data, uint16_t len) {
...
void polar_ble_driver_ecg_ring_init(
    polar_ble_driver_ecg_ring_t *ring,
    uint8_t *storage,
    uint16_t capacity) {
    if (ring == 0) {
        return;
...
void polar_ble_driver_ecg_ring_reset(polar_ble_driver_ecg_ring_t *ring) {
    if (ring == 0) {
        return;
    }
    ring->head = 0;
...
uint16_t polar_ble_driver_ecg_ring_available(const polar_ble_driver_ecg_ring_t *ring) {
    return ring == 0 ? 0 : ring->count;
}

uint16_t polar_ble_driver_ecg_ring_pop_bytes(
    polar_ble_driver_ecg_ring_t *ring,
    uint8_t *out,
    uint16_t max_len) {
...
void polar_ble_driver_ecg_parse_pmd_notification(
    polar_ble_driver_ecg_ring_t *ring,
    ...
    for (uint16_t i = 0; i < sample_count; ++i) {
        ...
        polar_ble_driver_ecg_ring_push_bytes(ring, packed, 4);
    }
