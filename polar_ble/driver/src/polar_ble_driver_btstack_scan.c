...
static bool polar_ble_driver_btstack_adv_name_contains_pair(
...
bool polar_ble_driver_btstack_decode_adv_report(
    uint8_t packet_type,
    uint8_t *packet,
    polar_ble_driver_btstack_adv_report_t *out_report) {
...
bool polar_ble_driver_btstack_adv_matches_filter(
    ...
    if (filter->use_name_prefix) {
        return polar_ble_driver_btstack_adv_name_matches_prefix(
            adv_data,
            adv_len,
            filter->name_prefix,
    ...
    if (filter->use_name_contains_pair) {
        return polar_ble_driver_btstack_adv_name_contains_pair(
            adv_data,
            adv_len,
            filter->name_contains_a,
            filter->name_contains_b);
    }

    return true;
}

bool polar_ble_driver_btstack_adv_prepare_connect(
    polar_ble_driver_runtime_link_t *link,
    const polar_ble_driver_btstack_scan_filter_t *filter,
    const uint8_t *adv_addr,
    const uint8_t *adv_data,
    uint8_t adv_len) {
    if (!polar_ble_driver_btstack_adv_matches_filter(filter, adv_addr, adv_data, adv_len)) {
        return false;
    }
    if (link != 0) {
        link->state = POLAR_BLE_DRIVER_RUNTIME_STATE_CONNECTING;
    }
    return true;
}
