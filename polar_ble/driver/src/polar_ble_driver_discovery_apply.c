...
void polar_ble_driver_discovery_apply_char_kind(
    ...
    if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PMD_CP) {
    ...
    if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PMD_DATA) {
    ...
    if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PSFTP_D2H) {
        if (psftp_d2h_handle) {
    ...
    if (kind == POLAR_BLE_DRIVER_DISC_CHAR_PSFTP_H2D) {
        if (psftp_h2d_handle) {
