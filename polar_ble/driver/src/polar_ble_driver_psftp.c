...
static size_t polar_ble_driver_psftp_min_size(size_t a, size_t b) {
    return a < b ? a : b;
}

#if POLAR_CFG_ENABLE_PSFTP
static size_t polar_ble_driver_psftp_bounded_strlen(const char *s, size_t max_len) {
    if (s == 0) {
        return 0;
    }
...
size_t polar_ble_driver_psftp_tx_build_next_frame(
    ...
    if (!polar_ble_driver_psftp_tx_has_more(state)) {
        return 0;
    }
    ...
    if (frame_capacity > 1u && out_frame_capacity > 1u) {
        max_payload = polar_ble_driver_psftp_min_size(frame_capacity - 1u, out_frame_capacity - 1u);
    }
    ...
    size_t chunk_len = polar_ble_driver_psftp_min_size(remaining, max_payload);
...
polar_ble_driver_psftp_rx_result_t polar_ble_driver_psftp_rx_feed_frame(
    ...
    if (state == 0 || frame == 0 || frame_len == 0) {
        return POLAR_BLE_DRIVER_PSFTP_RX_PROTOCOL_ERROR;
    }
    ...
    if (sequence != state->expected_sequence) {
        return POLAR_BLE_DRIVER_PSFTP_RX_SEQUENCE_ERROR;
    }
    ...
    if (status == 0u) {
        if (frame_len == 3u) {
            ...
            return POLAR_BLE_DRIVER_PSFTP_RX_ERROR_FRAME;
        }
        return POLAR_BLE_DRIVER_PSFTP_RX_PROTOCOL_ERROR;
    }
    ...
    if (payload_len > 0u) {
        ...
        if (state->length + payload_len > state->capacity) {
            return POLAR_BLE_DRIVER_PSFTP_RX_OVERFLOW;
        }
    ...
    return status == 0x01u ? POLAR_BLE_DRIVER_PSFTP_RX_COMPLETE : POLAR_BLE_DRIVER_PSFTP_RX_MORE;
}
...
bool polar_ble_driver_psftp_encode_get_operation(
    ...
    if (path == 0 || out_payload == 0 || path_len > POLAR_BLE_DRIVER_PSFTP_MAX_PATH_BYTES) {
        return false;
    }
...
#if POLAR_CFG_ENABLE_PSFTP
typedef struct {
    polar_ble_driver_psftp_dir_entry_t *entries;
    size_t capacity;
    size_t count;
    bool overflow;
} polar_ble_driver_psftp_dir_decode_ctx_t;

static bool polar_ble_driver_psftp_decode_directory_entry_cb(
    pb_istream_t *stream,
    ...
    polar_ble_driver_psftp_dir_decode_ctx_t *ctx = (polar_ble_driver_psftp_dir_decode_ctx_t *)(*arg);
    ...
    polar_ble_driver_psftp_dir_entry_t *out = &ctx->entries[ctx->count];
    size_t name_len = polar_ble_driver_psftp_bounded_strlen(entry.name, sizeof(entry.name));
    if (name_len > POLAR_BLE_DRIVER_PSFTP_MAX_ENTRY_NAME_BYTES) {
        name_len = POLAR_BLE_DRIVER_PSFTP_MAX_ENTRY_NAME_BYTES;
    }
...
polar_ble_driver_psftp_dir_decode_result_t polar_ble_driver_psftp_decode_directory(
    ...
    if ((payload_len > 0u && payload == 0) || (out_entries_capacity > 0u && out_entries == 0)) {
        return POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_INVALID_ARGS;
    }
    ...
    return POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_FAILED;
#else
    polar_ble_driver_psftp_dir_decode_ctx_t ctx = {
        .entries = out_entries,
    ...
    directory.entries.funcs.decode = polar_ble_driver_psftp_decode_directory_entry_cb;
    ...
    if (!pb_decode(&stream, protocol_PbPFtpDirectory_fields, &directory)) {
        if (ctx.overflow) {
            return POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_TOO_MANY_ENTRIES;
        }
        return POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_FAILED;
    }
    ...
    return POLAR_BLE_DRIVER_PSFTP_DIR_DECODE_OK;
#endif
}
