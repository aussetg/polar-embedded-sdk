...
static void polar_ble_driver_imu_ring_push_bytes(polar_ble_driver_imu_ring_t *ring, const uint8_t *data, uint16_t len) {
...
void polar_ble_driver_imu_ring_init(
    polar_ble_driver_imu_ring_t *ring,
    uint8_t *storage,
    uint16_t capacity) {
    if (ring == 0) {
        return;
...
void polar_ble_driver_imu_ring_reset(polar_ble_driver_imu_ring_t *ring) {
    if (ring == 0) {
        return;
    }
    ring->head = 0;
...
uint16_t polar_ble_driver_imu_ring_available(const polar_ble_driver_imu_ring_t *ring) {
    return ring == 0 ? 0 : ring->count;
}

uint16_t polar_ble_driver_imu_ring_pop_bytes(
    polar_ble_driver_imu_ring_t *ring,
    uint8_t *out,
    uint16_t max_len) {
...
void polar_ble_driver_imu_parse_pmd_notification(
    polar_ble_driver_imu_ring_t *ring,
    ...
    if (value[0] != pmd_measurement_type_acc) {
        return;
    }
    ...
    if (compressed || frame_type != 0x01u) {
        ring->parse_errors_total += 1;
        return;
    }
    ...
    uint16_t sample_count = (uint16_t)(payload_len / 6u);
    if ((payload_len % 6u) != 0u) {
        ring->parse_errors_total += 1;
    }

    for (uint16_t i = 0; i < sample_count; ++i) {
        uint16_t o = (uint16_t)(i * 6u);
        polar_ble_driver_imu_ring_push_bytes(ring, payload + o, 6);
    }
