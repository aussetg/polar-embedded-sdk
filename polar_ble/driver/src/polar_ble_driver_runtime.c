...
void polar_ble_driver_runtime_link_init(
    ...
    link->state = POLAR_BLE_DRIVER_RUNTIME_STATE_IDLE;
...
void polar_ble_driver_runtime_mark_attempt_failed(
    ...
    if (!link->connected) {
        link->state = POLAR_BLE_DRIVER_RUNTIME_STATE_RECOVERING;
    }
}
...
void polar_ble_driver_runtime_on_connection_complete(
    ...
    if (status != 0) {
        ...
        link->connected = false;
        link->conn_handle = invalid_conn_handle;
        polar_ble_driver_runtime_mark_attempt_failed(link);
        return;
    }

    link->connected = true;
    link->conn_handle = handle;
    link->state = POLAR_BLE_DRIVER_RUNTIME_STATE_DISCOVERING;
    link->attempt_failed = false;
}
