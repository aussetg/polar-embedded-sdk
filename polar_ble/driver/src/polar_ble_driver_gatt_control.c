...
static bool polar_ble_driver_gatt_notify_ops_ready(const polar_ble_driver_gatt_notify_ops_t *ops) {
    return ops != 0 &&
        ops->is_connected_ready != 0 &&
        ops->listener_active != 0 &&
        ops->start_listener != 0 &&
        ops->stop_listener != 0 &&
        ops->write_ccc != 0 &&
        ops->wait_complete != 0;
}

polar_ble_driver_gatt_notify_result_t polar_ble_driver_gatt_set_notify(
    ...
    uint8_t att_success,
    uint32_t timeout_ms) {
    if (!polar_ble_driver_gatt_notify_ops_ready(ops)) {
        return POLAR_BLE_DRIVER_GATT_NOTIFY_WRITE_FAILED;
    }
    if (!ops->is_connected_ready(ops->ctx)) {
        return POLAR_BLE_DRIVER_GATT_NOTIFY_NOT_CONNECTED;
    }
    ...
    if (enable) {
        if ((characteristic_properties & notify_property_mask) != 0) {
            ...
            return POLAR_BLE_DRIVER_GATT_NOTIFY_NO_NOTIFY_PROP;
        }
    }
    ...
    if (!ops->wait_complete(ops->ctx, timeout_ms, &att_status)) {
        return POLAR_BLE_DRIVER_GATT_NOTIFY_TIMEOUT;
    }
    if (att_status != att_success) {
        return POLAR_BLE_DRIVER_GATT_NOTIFY_ATT_REJECTED;
    }
    ...
    return POLAR_BLE_DRIVER_GATT_NOTIFY_OK;
}
