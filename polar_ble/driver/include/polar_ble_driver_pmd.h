// SPDX-License-Identifier: MIT
#ifndef POLAR_BLE_DRIVER_PMD_H
#define POLAR_BLE_DRIVER_PMD_H

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

#ifdef __cplusplus
extern "C" {
#endif

// PMD opcodes/measurement types used by Polar H10 paths.
enum {
    POLAR_BLE_DRIVER_PMD_OPCODE_GET_MEASUREMENT_SETTINGS = 0x01,
    POLAR_BLE_DRIVER_PMD_OPCODE_START_MEASUREMENT = 0x02,
    POLAR_BLE_DRIVER_PMD_OPCODE_STOP_MEASUREMENT = 0x03,
    POLAR_BLE_DRIVER_PMD_MEASUREMENT_ECG = 0x00,
    POLAR_BLE_DRIVER_PMD_MEASUREMENT_ACC = 0x02,
    POLAR_BLE_DRIVER_PMD_CP_RESPONSE_CODE = 0xF0,
};

enum {
    POLAR_BLE_DRIVER_PMD_RESPONSE_SUCCESS = 0x00,
    POLAR_BLE_DRIVER_PMD_RESPONSE_ALREADY_IN_STATE = 0x06,
    POLAR_BLE_DRIVER_PMD_RESPONSE_INVALID_SAMPLE_RATE = 0x08,
    POLAR_BLE_DRIVER_PMD_RESPONSE_INVALID_MTU = 0x0A,
};

typedef struct {
    uint16_t sample_rate;
    bool include_resolution;
    uint16_t resolution;
} polar_ble_driver_pmd_ecg_start_config_t;

typedef struct {
    uint16_t sample_rate;
    bool include_resolution;
    uint16_t resolution;
    bool include_range;
    uint16_t range;
} polar_ble_driver_pmd_acc_start_config_t;

typedef struct {
    uint8_t opcode;
    uint8_t measurement_type;
    uint8_t status;
    uint8_t more;
    bool has_more;
} polar_ble_driver_pmd_cp_response_t;

bool polar_ble_driver_pmd_att_status_requires_security(uint8_t att_status);
bool polar_ble_driver_pmd_security_ready(uint8_t encryption_key_size);
bool polar_ble_driver_pmd_response_status_ok(uint8_t status);

size_t polar_ble_driver_pmd_build_ecg_start_command(
    const polar_ble_driver_pmd_ecg_start_config_t *cfg,
    uint8_t *out,
    size_t out_capacity);

size_t polar_ble_driver_pmd_build_acc_start_command(
    const polar_ble_driver_pmd_acc_start_config_t *cfg,
    uint8_t *out,
    size_t out_capacity);

bool polar_ble_driver_pmd_parse_cp_response(
    const uint8_t *value,
    size_t value_len,
    polar_ble_driver_pmd_cp_response_t *out);

typedef enum {
    POLAR_BLE_DRIVER_PMD_START_RESULT_OK = 0,
    POLAR_BLE_DRIVER_PMD_START_RESULT_NOT_CONNECTED,
    POLAR_BLE_DRIVER_PMD_START_RESULT_SECURITY_TIMEOUT,
    POLAR_BLE_DRIVER_PMD_START_RESULT_CCC_REJECTED,
    POLAR_BLE_DRIVER_PMD_START_RESULT_CCC_TIMEOUT,
    POLAR_BLE_DRIVER_PMD_START_RESULT_MTU_FAILED,
    POLAR_BLE_DRIVER_PMD_START_RESULT_START_TIMEOUT,
    POLAR_BLE_DRIVER_PMD_START_RESULT_START_REJECTED,
    POLAR_BLE_DRIVER_PMD_START_RESULT_TRANSPORT_ERROR,
} polar_ble_driver_pmd_start_result_t;

typedef struct {
    size_t ccc_attempts;
    size_t security_rounds_per_attempt;
    uint32_t security_wait_ms;
    uint16_t minimum_mtu;
    uint16_t sample_rate;
    bool include_resolution;
    uint16_t resolution;
    bool include_range;
    uint16_t range;
} polar_ble_driver_pmd_start_policy_t;

typedef enum {
    POLAR_BLE_DRIVER_PMD_OP_OK = 0,
    POLAR_BLE_DRIVER_PMD_OP_NOT_CONNECTED = -1,
    POLAR_BLE_DRIVER_PMD_OP_TIMEOUT = -2,
    POLAR_BLE_DRIVER_PMD_OP_TRANSPORT = -3,
} polar_ble_driver_pmd_op_status_t;

typedef struct {
    void *ctx;
    bool (*is_connected)(void *ctx);
    uint8_t (*encryption_key_size)(void *ctx);
    void (*request_pairing)(void *ctx);
    void (*sleep_ms)(void *ctx, uint32_t ms);

    // 0 on success, >0 ATT status on ATT-level failure, <0 on transport errors.
    int (*enable_notifications)(void *ctx);
    int (*ensure_minimum_mtu)(void *ctx, uint16_t minimum_mtu);

    // return 0 on transport success and set *out_status to PMD CP response status.
    int (*start_ecg_and_wait_response)(
        void *ctx,
        const uint8_t *start_cmd,
        size_t start_cmd_len,
        uint8_t *out_status);
} polar_ble_driver_pmd_start_ops_t;

polar_ble_driver_pmd_start_result_t polar_ble_driver_pmd_start_ecg_with_policy(
    const polar_ble_driver_pmd_start_policy_t *policy,
    const polar_ble_driver_pmd_start_ops_t *ops,
    uint8_t *out_pmd_response_status,
    int *out_last_ccc_att_status);

polar_ble_driver_pmd_start_result_t polar_ble_driver_pmd_start_acc_with_policy(
    const polar_ble_driver_pmd_start_policy_t *policy,
    const polar_ble_driver_pmd_start_ops_t *ops,
    uint8_t *out_pmd_response_status,
    int *out_last_ccc_att_status);

#ifdef __cplusplus
}
#endif

#endif // POLAR_BLE_DRIVER_PMD_H
