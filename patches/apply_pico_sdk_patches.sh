...
FORCE=0
DRY_RUN=0
TARGET_PATH="vendors/pico-sdk"

usage() {
  cat <<'EOF'
Apply local pico-sdk format-patch series.

Usage:
  patches/apply_pico_sdk_patches.sh [--dry-run] [--force] [--target <path>]

Options:
  --dry-run         Validate patch applicability without applying
  --force           Apply even if target tree has local changes
  --target <path>   Target pico-sdk checkout (default: vendors/pico-sdk)
                    Example for MicroPython vendored pico-sdk:
                    --target vendors/micropython/lib/pico-sdk
...
PATCH_DIR="$REPO_ROOT/patches/pico-sdk/"
