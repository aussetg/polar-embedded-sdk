...
static bool logger_upload_bundle_copy_name(char dst[LOGGER_UPLOAD_BUNDLE_NAME_MAX], const char *src) {
...
static int logger_upload_bundle_tar_write(mtar_t *tar, const void *data, unsigned size) {
    logger_upload_bundle_stream_t *stream = (logger_upload_bundle_stream_t *)tar->stream;
    if (stream == NULL || data == NULL || (stream->emit_buf_len + size) > sizeof(stream->emit_buf)) {
...
static void logger_upload_bundle_tar_init(logger_upload_bundle_stream_t *stream) {
...
static bool logger_upload_bundle_write_header(
...
static bool logger_upload_bundle_write_root_dir_header(logger_upload_bundle_stream_t *stream) {
...
static bool logger_upload_bundle_write_manifest_header(logger_upload_bundle_stream_t *stream) {
    char path[LOGGER_STORAGE_PATH_MAX];
    if (!logger_path_join2(path, sizeof(path), stream->dir_name, "/manifest.json")) {
        return false;
    }
    return logger_upload_bundle_write_header(stream, path, (unsigned)stream->manifest_len, 0644u, MTAR_TREG);
}

static bool logger_upload_bundle_write_journal_header(logger_upload_bundle_stream_t *stream) {
    char path[LOGGER_STORAGE_PATH_MAX];
    if (!logger_path_join2(path, sizeof(path), stream->dir_name, "/journal.bin")) {
        return false;
    }
    return logger_upload_bundle_write_header(stream, path, (unsigned)stream->journal_size_bytes, 0644u, MTAR_TREG);
}

static bool logger_upload_bundle_fill_emit(logger_upload_bundle_stream_t *stream) {
    ...
    switch (stream->state) {
        ...
        case LOGGER_UPLOAD_BUNDLE_STATE_MANIFEST_DATA: {
            ...
            const unsigned chunk = remaining > sizeof(stream->io_buf) ? (unsigned)sizeof(stream->io_buf) : (unsigned)remaining;
            if (mtar_write_data(&stream->tar, stream->manifest_buf + stream->manifest_offset, chunk) != MTAR_ESUCCESS) {
                return false;
            }
        ...
        case LOGGER_UPLOAD_BUNDLE_STATE_JOURNAL_HEADER:
            if (!stream->journal_file_open) {
                if (f_open(&stream->journal_file, stream->journal_path, FA_READ) != FR_OK) {
                    return false;
                }
        ...
        case LOGGER_UPLOAD_BUNDLE_STATE_JOURNAL_DATA: {
            const uint64_t remaining64 = stream->journal_size_bytes - stream->journal_bytes_streamed;
            const unsigned want = remaining64 > sizeof(stream->io_buf) ? (unsigned)sizeof(stream->io_buf) : (unsigned)remaining64;
            UINT read_bytes = 0u;
            if (want > 0u) {
                if (f_read(&stream->journal_file, stream->io_buf, want, &read_bytes) != FR_OK || read_bytes != want) {
                    return false;
                }
            }
            if (mtar_write_data(&stream->tar, stream->io_buf, read_bytes) != MTAR_ESUCCESS) {
                return false;
            }
...
bool logger_upload_bundle_stream_open(
    logger_upload_bundle_stream_t *stream,
    const char *dir_name,
    ...
    logger_copy_string(stream->manifest_path, sizeof(stream->manifest_path), manifest_path);
    logger_copy_string(stream->journal_path, sizeof(stream->journal_path), journal_path);

    if (!logger_storage_read_file(
...
bool logger_upload_bundle_stream_read(
    logger_upload_bundle_stream_t *stream,
    void *dst,
    size_t cap,
    size_t *len_out) {
    ...
    while (written < cap) {
        if (stream->emit_buf_offset < stream->emit_buf_len) {
            ...
            const size_t chunk = (cap - written) < remaining ? (cap - written) : remaining;
...
bool logger_upload_bundle_compute(
    ...
    logger_upload_bundle_stream_t stream;
    if (!logger_upload_bundle_stream_open(&stream, dir_name, manifest_path, journal_path)) {
        return false;
    }
    ...
    for (;;) {
        size_t len = 0u;
        if (!logger_upload_bundle_stream_read(&stream, chunk, sizeof(chunk), &len)) {
            ok = false;
            break;
        }
        if (len == 0u) {
