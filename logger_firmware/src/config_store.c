...
static void logger_config_normalize(const logger_config_t *src, logger_config_t *dst) {
    ...
    logger_copy_string(dst->upload_api_key, sizeof(dst->upload_api_key), src->upload_api_key);
...
bool logger_config_set_upload_api_key(logger_persisted_state_t *state, const char *value) {
    if (!logger_write_if_changed(state->config.upload_api_key, sizeof(state->config.upload_api_key), value)) {
        return false;
    }
    return logger_config_store_save(state);
}
