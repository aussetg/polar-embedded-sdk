...
static uint64_t logger_fnv1a64_with_salt(const uint8_t *data, size_t len, uint64_t salt) {
...
void logger_identity_read_hardware_id_hex(char out_hex[LOGGER_HARDWARE_ID_HEX_LEN + 1]) {
    ...
    const uint64_t h0 = logger_fnv1a64_with_salt(board_id.id, sizeof(board_id.id), 0x6c6f676765723031ull);
    const uint64_t h1 = logger_fnv1a64_with_salt(board_id.id, sizeof(board_id.id), 0x6c6f676765723032ull);
