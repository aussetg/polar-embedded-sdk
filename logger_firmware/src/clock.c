...
static void logger_clock_dns_found_cb(const char *hostname,
                                      const ip_addr_t *ipaddr, void *arg) {
  (void)hostname;
  logger_clock_dns_resolution_t *resolution =
      (logger_clock_dns_resolution_t *)arg;
  if (resolution == NULL) {
    return;
  }
...
static void logger_clock_ntp_recv_cb(void *arg, struct udp_pcb *pcb,
  ...
  if (addr == NULL || !ip_addr_cmp(addr, &response->expected_address) ||
      port != LOGGER_CLOCK_NTP_PORT) {
    pbuf_free(p);
    return;
  }
...
static bool logger_clock_resolve_ntp_server(
  ...
  err_t dns_err = dns_gethostbyname(hostname, address_out,
                                    logger_clock_dns_found_cb, &resolution);
  if (dns_err == ERR_OK) {
    ipaddr_ntoa_r(address_out, remote_address_out, 48);
    return true;
  }
  if (dns_err != ERR_INPROGRESS) {
    snprintf(message_out, LOGGER_CLOCK_NTP_MESSAGE_MAX + 1u,
             "DNS lookup failed err=%d", (int)dns_err);
    return false;
  }
  ...
  if (resolution.err != ERR_OK) {
    snprintf(message_out, LOGGER_CLOCK_NTP_MESSAGE_MAX + 1u,
             "DNS lookup failed err=%d", (int)resolution.err);
    return false;
  }
...
logger_clock_ntp_exchange(const char *server, uint32_t originate_sec,
  ...
  request[0] =
      (uint8_t)((LOGGER_CLOCK_NTP_VERSION << 3) | LOGGER_CLOCK_NTP_CLIENT_MODE);
  ...
  const err_t send_err =
      udp_sendto(pcb, packet, &server_address, LOGGER_CLOCK_NTP_PORT);
  pbuf_free(packet);
  if (send_err != ERR_OK) {
    snprintf(message_out, LOGGER_CLOCK_NTP_MESSAGE_MAX + 1u,
             "failed to send NTP request err=%d", (int)send_err);
    udp_remove(pcb);
    return false;
  }
  ...
  if (response_state.truncated ||
      response_state.len < LOGGER_CLOCK_NTP_PACKET_LEN) {
    logger_copy_string(message_out, LOGGER_CLOCK_NTP_MESSAGE_MAX + 1u,
  ...
  const uint32_t echoed_frac =
      logger_clock_read_be32(response_state.data + 28u);
  const uint32_t transmit_sec =
      logger_clock_read_be32(response_state.data + 40u);
  const uint32_t transmit_frac =
      logger_clock_read_be32(response_state.data + 44u);
  ...
  if (echoed_sec != originate_sec || echoed_frac != originate_frac) {
    logger_copy_string(
        message_out, LOGGER_CLOCK_NTP_MESSAGE_MAX + 1u,
...
static bool logger_clock_format_unix_seconds_rfc3339(
    int64_t unix_seconds, char out_rfc3339[LOGGER_CLOCK_RFC3339_UTC_LEN + 1]) {
...
static bool
logger_clock_datetime_reasonable(const logger_clock_status_t *status) {
  return logger_clock_datetime_reasonable_parts(status->year, status->month,
...
static bool logger_timezone_is_europe_paris(const char *timezone) {
...
logger_clock_status_to_observed_utc_ns(const logger_clock_status_t *status,
  ...
  const int64_t seconds = days * 86400ll + ((int64_t)status->hour * 3600ll) +
...
static bool
logger_clock_status_to_observed_utc_seconds(const logger_clock_status_t *status,
                                            int64_t *utc_seconds_out) {
  if (utc_seconds_out == NULL || !logger_clock_datetime_reasonable(status)) {
    return false;
  }
  const int64_t days =
      logger_days_from_civil(status->year, status->month, status->day);
...
static int logger_last_sunday_of_month(int year, int month) {
  const int last_day = logger_days_in_month(year, month);
  const int weekday = logger_weekday_from_date(year, month, last_day);
...
static bool
logger_clock_europe_paris_dst_active_utc(const logger_clock_status_t *status) {
  ...
  const int transition_day = logger_last_sunday_of_month(status->year, month);
  if (month == 3) {
...
static bool
logger_timezone_utc_offset_seconds(const logger_clock_status_t *utc_status,
                                   const char *timezone,
                                   int32_t *offset_seconds_out) {
  if (utc_status == NULL || offset_seconds_out == NULL ||
      !logger_clock_datetime_reasonable(utc_status)) {
    return false;
  }
  ...
  if (logger_timezone_is_europe_paris(timezone)) {
    *offset_seconds_out =
        logger_clock_europe_paris_dst_active_utc(utc_status) ? 7200 : 3600;
    return true;
  }
  return false;
}
...
static bool
logger_clock_datetime_from_unix_seconds(int64_t seconds,
  ...
  dt_out->weekday =
      logger_weekday_from_date(dt_out->year, dt_out->month, dt_out->day);
  return logger_clock_datetime_reasonable_parts(dt_out->year, dt_out->month,
                                                dt_out->day, dt_out->hour,
                                                dt_out->minute, dt_out->second);
}

static void logger_clock_sync_posix_time(const logger_clock_status_t *status) {
  if (status == NULL || !status->valid ||
      !logger_clock_datetime_reasonable(status)) {
    return;
  }
  const int64_t days =
      logger_days_from_civil(status->year, status->month, status->day);
...
static bool logger_clock_write_reg(uint8_t reg, uint8_t value) {
  ...
  return i2c_write_blocking(i2c0, LOGGER_RTC_I2C_ADDR, buf, sizeof(buf),
                            false) == (int)sizeof(buf);
}
...
void logger_clock_sample(logger_clock_status_t *status) {
  ...
  status->valid =
      !status->lost_power && logger_clock_datetime_reasonable(status);
  if (logger_clock_datetime_reasonable(status)) {
...
bool logger_clock_set_utc(const char *rfc3339_utc,
  ...
  if (sscanf(rfc3339_utc, "%4d-%2d-%2dT%2d:%2d:%2dZ", &dt.year, &dt.month,
             &dt.day, &dt.hour, &dt.minute, &dt.second) != 6) {
    return false;
  }
  if (!logger_clock_datetime_reasonable_parts(dt.year, dt.month, dt.day,
  ...
  if (!logger_clock_update_reg(LOGGER_PCF8523_REG_CONTROL3,
                               LOGGER_PCF8523_REG_CONTROL3_PM_MASK, 0u)) {
    return false;
  }
  if (i2c_write_blocking(i2c0, LOGGER_RTC_I2C_ADDR, buf, sizeof(buf), false) !=
      (int)sizeof(buf)) {
    return false;
  }
  if (!logger_clock_update_reg(LOGGER_PCF8523_REG_CONTROL1,
                               LOGGER_PCF8523_REG_CONTROL1_STOP, 0u)) {
    return false;
  }
...
bool logger_clock_ntp_sync(const logger_clock_status_t *current_status,
  ...
  for (size_t i = 0u; i < sizeof(LOGGER_CLOCK_NTP_SERVERS) /
                              sizeof(LOGGER_CLOCK_NTP_SERVERS[0]);
       ++i) {
    ...
    if (!logger_clock_format_unix_seconds_rfc3339(applied_unix_seconds,
                                                  result->applied_utc)) {
    ...
    if (status_out != NULL && status_out->now_utc[0] != '\0') {
      logger_copy_string(result->applied_utc, sizeof(result->applied_utc),
                         status_out->now_utc);
    }
    if (have_previous_utc) {
      result->correction_seconds =
          applied_unix_seconds - (previous_utc_ns / 1000000000ll);
      result->large_correction = result->correction_seconds < -300ll ||
                                 result->correction_seconds > 300ll;
    }

    result->applied = true;
    snprintf(result->message, sizeof(result->message),
             "applied NTP time from %s", result->server);
    return true;
  }
...
static bool logger_clock_derive_study_day_from_fields(int year, int month,
                                                      int day, int hour,
...
bool logger_clock_derive_study_day_local_observed(
  ...
  if (!logger_timezone_present(timezone)) {
    return false;
  }
...
bool logger_clock_observed_local_datetime(const logger_clock_status_t *status,
  ...
  if (status == NULL || local_out == NULL ||
      !logger_timezone_present(timezone)) {
    return false;
  }

  int64_t utc_seconds = 0;
  int32_t offset_seconds = 0;
  if (!logger_clock_status_to_observed_utc_seconds(status, &utc_seconds) ||
      !logger_timezone_utc_offset_seconds(status, timezone, &offset_seconds)) {
    return false;
  }
  return logger_clock_datetime_from_unix_seconds(
