...
bool logger_battery_low_start_blocked(const logger_battery_status_t *status) {
    return !status->vbus_present && status->voltage_mv < LOGGER_BATTERY_LOW_START_BLOCK_MV;
}

bool logger_battery_off_charger_upload_allowed(const logger_battery_status_t *status) {
