...
logger_button_event_t logger_button_poll(logger_button_t *button, uint32_t now_ms) {
    ...
    if (button->debounced_pressed != button->raw_pressed) {
        ...
        if (duration_ms >= LOGGER_BUTTON_SHORT_MIN_MS && duration_ms < LOGGER_BUTTON_SHORT_MAX_MS) {
